...
Section clean_ca.
  ...
  Proof.
    ...
    - case: hd => [|[a [|//]] gs] /=.
      ...
      by rewrite save_goals_cons /catr/= cat0s.
  Qed.
  ...
  Proof.
    ...
    - move=> fA /andP[vA bB].
      ...
      have {HA HB} := HA s3 (t2l B sm no_alt) _ _ _ _ _ fv fA vA.
    ...
    - move=> /[!failed_or_None] fB vB.
      ...
      by rewrite H HB/= clean_ca_add_ca1//.
    - rewrite failed_and; case fA: failed => //= + /andP[vA].
      ...
      rewrite -{2}(cat0s bt).
      have HH := @clean_ca_add_deep_gs no_alt bt (a2g B0) gs (EA _).
      rewrite cat0s in HH.
      ...
      rewrite /save_alts/=/r2a/= map_cons.
      rewrite cat_cons.
      ...
      have:= @add_deep_altsP (a2g B0) (r2a zs) T1 no_alt T2 (EA _) (empty_ca_atoms1 _).
      rewrite !cats0//.
  Qed.
