...
Section NurProp.
  ...
  Section t2l_base.
    ...
    Proof.
      elim: A => //=.
      - by eexists.
      - move=> s; case: s => //=[p a|] _ B0 B HB/andP[/eqP ? bB]; subst; have [hd H]/= := HB bB.
    ...
    Proof.
      ...
      move=> a; case: a => //=[p a|] _ B0 B HB /=C /andP[? bB]; subst.
  ...
  Proof.
    ...
    - move=> A; case: A => //=[p a|] _ B0 B HB s1 /andP[/eqP? bB]; subst.
        have [hd H]:= base_and_t2l bB.
  ...
  Proof.
    ...
    - move=> A; case: A => //=[p c|] _ [p1 l] B HB C bt /andP[/eqP? bB][<-]; subst => //.
  Qed.
  ...
  Proof. case: r => //[B|B|B]vA H/=; try (move=> _; apply: failed_t2l vA (step_not_failed _ H notF)). Qed.
  ...
  Proof.
    ...
    - move=> A; case: A => //=[p a|] _ [p' l'] B HB s s1 l//=.
  Qed.
  ...
  Proof.
    ...
    - move=> A; case: A => //[p a|] _ [p' l'] B HB s2 C l/=/andP[? bB][<-]//.
  Qed.
  ...
  Proof.
    ...
    - move=> A HA B0 B HB sA s1 C l1/=/andP[vA].
      case eA: step => //[A'|A'].
        ...
        - rewrite H3/= s2l_big_and/= take0 drop0 make_lB0_empty1//.
  ...
  Proof.
    ...
    - move=> A HA B0 B HB l l1 ca tl alts s1 s2/andP[vA].
      ...
      case: z SA => //=.
        rewrite cat0s => H1 H2.
        case e: step => [A'|A'|A'|A']/=.
        - have [] := s2l_empty_hd_success vA (step_not_failed _ e notF) H1.
          by rewrite (step_not_solved _ e)//.
        - have []:= s2l_empty_hd_success vA (step_not_failed _ e notF) H1.
          by rewrite (step_not_solved _ e)//.
  ...
  Proof.
    ...
    - move=> /= A HA B0 B HB s1 C s4 ca x tl l1 /andP[vA].
      ...
      rewrite !make_lB01_empty2.
      have [s[y[ys]]] := failed_t2l vB (step_not_failed _ eB notF) (get_substS s1 A') (make_lB0 SA (r2l B0.1 B0.2) ++ l1).
      move=>H4; rewrite H4/=.
      move=>[???]; subst.
  ...
  Proof.
    ...
    - move=> A HA s B HB C s1 s3 l p t gs xs.
      ...
      rewrite (valid_tree_is_dead (valid_tree_expand _ vA e)); last first.
        ...
        move=>->//.
      have [s5 [y[ys sA]]]:= failed_t2l vA (step_not_failed _ e notF) s1 SB.
    ...
    - move=> A HA B0 B HB C s1 s3 l p t gs xs /andP[vA].
      ...
      have [s[y[ys sB]]]:= failed_t2l vB (step_not_failed _ e1 notF) Y (X++l).
