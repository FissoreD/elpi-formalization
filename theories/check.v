...
Section merge.
  Definition update (s:sigV) '((k, v): (V * _)) : typecheck (sigV) :=
    ...
    | Some e => 
        map_ty' (fun m => add k m s) (min v e)
    end.
  ...
  Proof.
    ...
    rewrite (valid_sig_cat H) min_refl//= add_cat//=.
...
Fixpoint det_tree_aux (sP:sigT) (sV : sigV) A (dd:D) : typecheck (D * sigV)%type :=
  ...
  | And A B0 B =>
    ...
    if (failed A && nB0) || nA || (nB0 && nB) then ty_ok (dd, sV)
      ...
      if failed A then 
        map_ty (fun '(dd', sV') => det_tree_aux sP sV' B0 dd') rA
      (* add case: if next_alt_B0 == None then cat A and B *)
      else
      if success A && (next_alt true A == None) then
        if nB then ty_ok (dd, sV)
        else map_ty (fun '(dd', sV') => det_tree_aux sP sV' B dd') rA
      else
      map_ty (fun '(dd', sV') =>
        ...
        map_ty (fun '(ddB0, sVB0) =>
          map_ty (fun '(ddB, sVB) =>
            map_ty' (fun x => 
            (maxD ddB0 ddB, x)) (merge_sig sVB sVB0)) rB) rB0) rA
  | Or A _ B =>
      ...
      map_ty (fun '(sVA, X) =>
        map_ty (fun '(sVB, Y) =>
          map_ty' (fun x => (if has_cut A then maxD sVA sVB else Pred, x)) (merge_sig X Y)) dtB) dtA
  end.
...
Section less_precise.
  ...
  Proof.
    ...
    rewrite key_absent_remove//.
      by apply: IH.
    rewrite key_absent_add_diff//; congruence.
  Qed.
  ...
  Lemma merge_less_precise {A B C}:
    valid_sig A -> valid_sig B -> merge_sig A B = ty_ok C -> less_precise A C.
  Proof.
    elim: B A C => //=.
    - move=> A C vA _ [<-]; apply: less_precise_refl vA.
    - move=> [k v] xs IH A C vA /= /andP[H1 H2].
      case L: lookup => [A'|]//=.
        case M: min => /=[m|]// H.
        have:= IH _ _ (valid_sig_add _ vA) H2 H.
        by apply: less_precise_add_Some_left L M.
      move=> H.
      have:= IH _ _ (valid_sig_add _ vA) H2 H.
      apply: less_precise_add_None_left L.
  Qed.
  ...
  Proof.
    ...
    - move=> A HA s B HB sv1 sv2 d ign vs1v.
      ...
      have [V H] := HA _ _ _ _ vs1v dtA.
      have [V' H'] := HB _ _ _ _ vs1v dtB.
      rewrite (valid_sig_merge V V' M); repeat split.
      apply: less_precise_trans H _.
      by apply: merge_less_precise M.
    - move=> A HA B0 HB0 B HB sv1 sv2 d ign sv1s.
      case fA: failed => /=.
        ...
        have [H1 H2] := HA _ _ _ _ sv1s dtA.
        have [H3 H4] := HB0 _ _ _ _ H1 dtB.
        split => //.
        apply: less_precise_trans H2 H4.
      case:eqP => nA/=.
        move=>[??]; subst.
        by rewrite less_precise_refl//.
      case:eqP => nB0/=.
        ...
        case: ifP.
          ...
          have [H1 H2] := HA _ _ _ _ sv1s dtA.
          have [H5 H6] := HB _ _ _ _ H1 dtB.
          rewrite H5; repeat split.
          apply: less_precise_trans H2 H6.
        move=> H.
        ...
        have [H1 H2] := HA _ _ _ _ sv1s dtA.
        have [H3 H4] := HB0 _ _ _ _ H1 dtB0.
        have [H5 H6] := HB _ _ _ _ H1 dtB.
        have V := valid_sig_merge H5 H3 X.
        rewrite V; repeat split.
        apply: less_precise_trans H2 _.
        apply: less_precise_trans H6 _.
        apply: merge_less_precise H5 H3 X.
      case:ifP.
      (* case dtA: (det_tree_aux _ _ A) => /=[[DA sVA]|]//=; rewrite?if_same//.
        ...
        case:ifP. *)
          ...
          case nB: next_alt => [B'|]//=.
            ...
            apply: less_precise_trans H2 H6.
          move=> [??]; subst.
          rewrite sv1s less_precise_refl//.
  ...
  Proof.
    ...
    - move=> A HA B0 HB0 B HB d1 sV sV' ign.
      ...
      case:ifP.
        move=> /andP[sA /eqP nA'].
        rewrite success_failed//= in fA, H.
        move: fA H.
        ...
        case nB: (next_alt _ B) => //=[B'|].
          ...
          rewrite H2/=.
          destruct d2; simpl in H1; subst.
            by rewrite H6; repeat eexists; auto.
          rewrite dtB; repeat eexists; rewrite minD_refl//.
        rewrite andbT.
        move=> _ /eqP nB0 [??]; subst.
        by repeat eexists => //.
      case dtA: (det_tree_aux _ _ A) => //= [[dA sVA]].
      case dtB0: (det_tree_aux _ _ B0) => //= [[dB0 sVB0]].
      case dtB: (det_tree_aux _ _ B) => //= [[dB sVB]].
      case X: merge_sig => //=[S] Hi [??]; subst.
      have {HA}[d2[H1 H2]] := HA _ _ _ _ dtA.
      rewrite H2/=.
...
Proof.
  ...
  case YY : A X => //=[|c].
    ...
    move=> {}/IH H.
    have {H}:= H V.
    case dt: det_tree_aux V => //=[[[b|]]]//= V _.
    rewrite merge_refl//=.
    by have [] := det_tree_aux_less_precise V dt.
  rewrite/check_callable.
  case X: check_tm => //[[[d b]|]]//=; last first.
    move=> [??]; subst => /=; rewrite maxD_comm/=.
    move=> /IH/= -/(_ V); case dtA: det_tree_aux => //=[[[b|]]]//=.
    rewrite merge_refl//=.
    by have [] := det_tree_aux_less_precise V dtA.
  case: d X => //-[]//=d.
  case Y: get_callable_hd_sig => //[s|].
    case: b => //=.
      case X: assume_call => //=[ts] H [??]; subst.
      have [H1 H2] := assume_call_less_precise V X.
      move=> /IH -/(_ H1).
      rewrite maxD_comm maxD_assoc maxD_refl.
      case dt: det_tree_aux => //=[[[]S]]//= _.
      rewrite merge_refl//=.
      by have [] := det_tree_aux_less_precise H1 dt.
    move=> H [??]; subst => /IH -/(_ V).
    rewrite maxD_comm/=.
    case dt: det_tree_aux => //[[[]b]]//=.
    rewrite merge_refl//=.
    by have [] := det_tree_aux_less_precise V dt.
  case: b => //=.
    move=> H [??]; subst.
...
Proof.
  ...
  - move=> A HA s B HB d1 d2 sV.
    ...
    case:ifP => kB.
      apply: HA.
    have {HA} := HA d1 d2 sV.
    case X: det_tree_aux => //=[[x y]|]; case Y: det_tree_aux => /=[[z w]|]//=.
    move=> /eqP[?]; subst.
    have {HB} := HB d1 d2 sV.
    case Z: det_tree_aux => //=[[a b]|]; case T: det_tree_aux => /=[[c d]|]//=.
    move=> /eqP[?]; subst.
    case M: merge_sig => //=.
  - move=> A HA B0 HB0 B HB d1 d2 sV.
    ...
    case dtB0: (det_tree_aux _ _ B0) => //=[[x y]|]; case dtB0': (det_tree_aux _ _ B0) => /=[[z w]|]//=; repeat case: ifP => //.
    move=> H1 H2 /eqP[?]; subst.
    have {HB}/= := HB dA dA' sA'.
    case dtB: det_tree_aux => //=[[a b]|]; case dtB': det_tree_aux => /=[[c d]|]//=; repeat case: ifP => //=.
    move=> /eqP[?]; subst.
    case X: merge_sig => //=.
Qed.
...
(* Lemma kkk {sP sV A A' i1 i2 DA sVA}:
  det_tree_aux sP sV A i1 = ty_ok (DA, sVA) ->
  next_alt false A = Some A' ->
  exists d', det_tree_aux sP sV A' i2 = ty_ok (d', sVA).
Proof.
  elim: A sV A' i1 i2 DA sVA => //=.
  - move=> ?????? [_<-][<-]; by eexists.
  - admit.
  - move=> ?????? [_<-][<-]; by eexists.
  - admit.
...
(* Lemma success_det_tree_next_alt {sP A A' d sV1 sV2 ign ign1}:
  success A -> (det_tree_aux sP sV1 A ign) = ty_ok (d, sV2) ->
    (next_alt true A) = Some A' ->
    minD ign1 ign = ign1 ->
    exists d1, minD d1 d = d1 /\ 
    (det_tree_aux sP sV1 A' ign1) = ty_ok (d1, sV2).
Proof.
  simpl in sV2, d.
  elim: A A' sV1 sV2 ign d ign1 => //=.
  - move=> A HA s B HB C S1 S2 ign d ign1.
    case:ifP => [dA sB|dA sA].
      rewrite (is_dead_next_alt _ dA)//=.
      case X: det_tree_aux => //=[[dB sVB]][??]; subst.
      rewrite !(is_dead_next_alt _ dA)//=.
      have := (HB _ _ _ _ _ _ sB X); subst.
      case nB: (next_alt _ B) => [B'|]//=.
      move=> /(_ _ _ erefl) {}HB[<-]{C}/= H.
      have /=fB' := next_alt_failed nB.
      rewrite (next_alt_not_failed _ fB') !(is_dead_next_alt _ dA)//=.
      have [d1[H1 H2]] := HB _ H.
    ...
    case nB: next_alt => [B'|]; last first => /=.
      move=> /(HA _ _ _ _ _ _ sA).
      case nA: (next_alt _ A) => [A'|]//=.
      move=> {}HB [<-]{C} H/=.
      have /=fA' := next_alt_failed nA.
      rewrite nB/= next_alt_not_failed//=.
      have [d1[H1 H2]] := HB _ _ erefl H.
      by repeat eexists; eauto.
    case dtA: (det_tree_aux _ _ A) => //=[[DA sVA]].
    case dtB: (det_tree_aux _ _ B) => //=[[DB sVB]]//.
    move=> [??]; subst.
    case nA: (next_alt _ A) => [A'|]//=.
      ...
      have /=fA' := next_alt_failed nA.
      rewrite nB/= next_alt_not_failed//=.
      rewrite -(has_cutF_next_alt nA) success_has_cut//=.
      have [d1[H1 H2]] := HA _ _ _ _ _ _ sA dtA nA H.
      rewrite H2/=.
      destruct ign1; simpl in H; subst => //=.
        have [d2[H3 H4]] := det_tree_aux_func2 dtB.
        by rewrite H4/=; repeat eexists => //.
      by rewrite dtB//=; repeat eexists.
    move=> [<-]{C} H/=.
...
Proof.
  ...
  - move=> A HA B0 HB0 B HB sV1 sV2 ign /andP[sA sB].
    ...
    case nB: (next_alt _ B) => [B'|]/=.
      by rewrite sA fA (next_alt_not_failed _ (next_alt_failed nB))//.
    have DA := @is_dead_dead A. 
    rewrite is_dead_success//is_dead_failed// is_dead_next_alt//?is_dead_dead//=.
    rewrite is_dead_next_alt//=.
Qed.

(* Lemma det_tree_aux_next_alt_None_same {sP sVA sVB b B DA DB}:
  next_alt b B = None ->
  det_tree_aux sP sVA B DA = ty_ok (DB, sVB) ->
  sVB = sVA.
Proof.
  elim: B sVA sVB b DA DB => //=; try congruence.
  - move=> A HA s B HB sVA sVB b DA DB.
    destruct b; last first. 
      case nA: next_alt => //=.
      case nB: next_alt => //= _.
      apply: HB nB.
    case nA: next_alt => //=.
    case sA: (success A).
      rewrite success_has_cut// success_is_dead//=.
      case nB: next_alt => //= _.
      rewrite next_alt_not_failed?success_failed//=.
      apply: HA nA.
    rewrite next_alt_false_true// in nA.
    rewrite nA/=; case nB: next_alt => //= _.
    apply: HB nB.
  - move=> A HA B0 HB0 B HB sVA sVB b DA DB.
    case: ifP => fA.
      case nA: next_alt => //=[A'|].
        case nB0: next_alt => //= _; congruence.
      case nB0: next_alt => //= _; congruence.
    case: ifP => sA/=.
      rewrite (next_alt_not_failed _ fA)/=.
      case nB: next_alt => //=.
      case nA: next_alt => //=[A'|].
        ...
        case sB: (success B).
          rewrite next_alt_not_failed?success_failed//=.
          case dtA: det_tree_aux => //=[[k v]].
          case dtB0: (det_tree_aux _ _ B0) => //=[[k1 v1]].
          case dtB: (det_tree_aux _ _ B) => //=[[k2 v2]].
          case M: merge_sig => //=[S][??]; subst.
          have ? := HB _ _ _ _ _ nB dtB; subst.
          have ? := HB0 _ _ _ _ _ nB0 dtB0; subst.
          rewrite 
          case dtA: det_tree_aux => //=[[k v]]. *)
...
Lemma failed_det_tree_next_alt {sP A sV1 sV2 d ign b}:
  valid_sig sV1 ->
  det_tree_aux sP sV1 A ign = ty_ok (d, sV2) ->
    match next_alt b A with
      | Some B => exists d', minD d' d = d' /\ (det_tree_aux sP sV1 B ign) = ty_ok (d', sV2)
      | None =>  sV2 = sV1
    end.
Proof.
  elim: A sV1 sV2 d ign b => //=; last first; [|admit..].
  move=> A HA B0 HB0 B HB sv1 sv2 d ign b V.
  case fA: failed => /=.
    case nA: (next_alt _ A) => //=[A'|].
      case nB0: (next_alt _ B0) => //=[B0'|]; [|congruence].
      have /= fA' := next_alt_failed nA.
      have /= fB0' := next_alt_failed nB0.
      ...
      case dtB0: (det_tree_aux _ _ B0) => /=[[DB0 sVB0]|]//=.
      move=>[??]; subst.
      have {HA} := (HA _ _ _ _ false V dtA).
      rewrite nA/= => -[d'[H1 H2]].
      rewrite H2/=.
      have [V' H] := det_tree_aux_less_precise V dtA.
      have {HB0} := HB0 _ _ _ _ false V' dtB0.
      rewrite nB0/=.
      move=> [d''[H3 H4]].
      have [V'' _] := det_tree_aux_less_precise V' H4.
      destruct d'; simpl in H1; subst.
        have [dx [Hx Hy]]:= det_tree_aux_func2 dtB0.
        rewrite Hy/=.
        have [dw [Hw Hz]]:= det_tree_aux_func2 H4.
        rewrite Hz/= merge_refl//=.
        case:ifP => _; by repeat eexists; destruct dx, dw, d, d'' => //=.
      rewrite dtB0/= H4/= merge_refl//=.
      by case: ifP => _; repeat eexists; destruct d, d'' => //.
    by case nB0: next_alt => [B0'|]//=; congruence.
  rewrite next_alt_not_failed//=.
  case nB0: (next_alt _ B0) => //=[B0'|].
    case:ifP.
      move=> /andP[sA /eqP nA]; rewrite sA nA.
      case nB: next_alt => [B'|]//=.
        case dtA: (det_tree_aux _ _ A) => /=[[DA sVA]|]//= dtB.
        have [V' H] := det_tree_aux_less_precise V dtA.
        move=> {B' nB}.
        case nB: next_alt => [B'|]//=.
          have/=fB' := next_alt_failed nB.
          rewrite fA nB0/= !next_alt_not_failed//=dtA/=.
          rewrite sA nA/=.
          have := (HB _ _ _ _ b V' dtB).
          by rewrite nB.
        have := (HB _ _ _ _ b V' dtB).
        rewrite nB => ?; subst.
        have := (HA _ _ _ _ true V dtA).
        by rewrite nA//.
      move=> [??]; subst.
      have fB := next_alt_None_failed nB.
      rewrite next_alt_false_true//=?failed_success//= nB//.
    case dtA: (det_tree_aux _ _ A) => /=[[DA sVA]|]//=.
    have [V' H] := det_tree_aux_less_precise V dtA.
    case dtB0: (det_tree_aux _ _ B0) => /=[[DB0 sVB0]|]//=.
    case dtB: (det_tree_aux _ _ B) => /=[[DB sVB]|]//=.
    ...
    case: ifP => //= sA.
      ...
      case nB: (next_alt _ B) => [B'|]//=.
        have/=fB' := next_alt_failed nB.
        rewrite fA nB0/= !next_alt_not_failed//=dtA/=dtB0/= sA/= nA/=.
        have {HB0} := (HB _ _ _ _ b V' dtB).
        rewrite nB.
        move=> [d''[H1 {}HB]].
        rewrite HB/= M/=; repeat eexists.
        by destruct DB0, DB, d''.

      have := HB _ _ _ _ b V' dtB.
      rewrite nB => ?; subst.
      have/=fB0' := next_alt_failed nB0.
      have := HB0 _ _ _ _ false V' dtB0.
      rewrite nB0 => -[d2[H3 HH]].
      have [Hx Hy] := det_tree_aux_less_precise V' HH.
        (* TODO: by Hy and M, sv2 = sVB0 *)
      have ? : sv2 = sVB0; subst.
        move: V' Hy M.
        admit.
      have [d3[H4 H5]] := det_tree_aux_func2 dtB0.
      have [d4[H6 H7]] := det_tree_aux_func2 HH.
      have := (HA _ _ _ _ true V dtA).
      rewrite nA.
      move => -[d''[H2 {}HA]].
      have/=fA' := next_alt_failed nA.
      ...
      case:ifP.
        move=> /andP[sA' /eqP nA'].
        destruct d''; simpl in H2; subst.
          rewrite H7.
          repeat eexists.
          by destruct d4, DB0, DB, d2 => //=.
        rewrite HH.
        repeat eexists.
        by destruct d2, DB0, DB => //=.
      destruct d''; simpl in H2; subst.
        rewrite H5/=H7/=merge_refl//=.
        repeat eexists.
        destruct d3, d4, DB0, DB => //=.
        by rewrite minD_comm/= in H3; subst.
      rewrite dtB0/= HH/=merge_refl//=.
      repeat eexists.
      by destruct d2, DB0, DB => //=.
    move=> _.
    rewrite fA/=nB0 /= sA/= next_alt_not_failed//=dtA/= dtB0/= dtB/=M/=.
  ...
  case nB: (next_alt _ B) => [B'|]//=.
    ...
    case: ifP => //=.
      ...
      move=> {B' nB fB'}.
      case nB: next_alt => [B'|]//=.
        have/=fB0' := next_alt_failed nB.
        rewrite fA/= sA/= nA/= nB0 next_alt_not_failed?success_failed//=.
        rewrite next_alt_not_failed//= dtA/=.
        have [V' H] := det_tree_aux_less_precise V dtA.
        have {HB0} := (HB _ _ _ _ b V' dtB).
        rewrite nB => -[d'[H1 H2]].
    ...
    case: ifP => sA//=.
      ...
      destruct b; last first.
        rewrite nB/=.
        have /= nB' := next_alt_failed nB.
        rewrite fA/= nA nB0/= sA/=.
        rewrite !next_alt_not_failed//=.
        rewrite dtA/= dtB0 /= .
        have:= HB _ _ _ _ false V' dtB.
        rewrite nB/= => -[d' [H1 H2]].
        rewrite H2/= M/=.
        repeat eexists.
        by destruct DB0, DB, d' => //=.
      case nB': next_alt => [B''|]//=.
        have/=fB0' := next_alt_failed nB'.
        rewrite fA/= sA/= nA/= nB0 !next_alt_not_failed//?success_failed//=.
        have:= HB _ _ _ _ true V' dtB.
        rewrite nB'/= => -[d' [H1 H2]].
        rewrite dtA /= dtB0/= H2/= M/=.
        repeat eexists.
        by destruct DB0, DB, d' => //=.
      have:= HB _ _ _ _ true V' dtB.
      rewrite nB' => ?; subst.
      have:= HB0 _ _ _ _ false V' dtB0.
      rewrite nB0 => ?; subst.
      move: M; rewrite merge_refl// => -[?]; subst.
      admit.
    move=> _.
    rewrite fA/= nB0 nB/= next_alt_not_failed//= sA/= dtA/= dtB0/= dtB/= M/=.
    repeat eexists; rewrite minD_refl//.
  move=> [??]; subst.
  have fB:= next_alt_None_failed nB.
  rewrite (@next_alt_false_true B)//=?(failed_success _ fB)//nB.
  case: ifP => sA/=.
    by case nA: next_alt => //=.
  rewrite fA sA nB0 nB/= next_alt_not_failed//=.
  repeat eexists; rewrite minD_refl//.

Admitted.
      (*have := HB0 _ _ _ _ false V' dtB0.
      rewrite  nB0.
      have := (HB _ _ _ _ false _ dtA).

        move => -[d'[H2 {}HA]].

            by destruct d2; simpl in H4; subst => //.
      ...
      case nA': next_alt => {nA}[{}A'|]//=[<-]/=.
      have /=fA':= next_alt_failed nA'.
      have /=fB0':= next_alt_failed nB0.
      rewrite fA'/= nB0 !next_alt_not_failed//=.
      {
        case X: det_tree_aux => /=[[d' s']|]//=.
          have?: s' = sVA; subst.
            admit.
          (* have {}HA := (HA _ _ _ _ _ V dtA nA). *)
          have [V' H] := det_tree_aux_less_precise V dtA.
          have {}HB0 := HB0 _ _ _ _ _ V' dtB0 nB0.
          rewrite HA/= dtB0/= HB0/= maxD_refl merge_refl//=.


      }
      admit.
    move=> [<-]/=; rewrite fA/=nB0 nB next_alt_not_failed//= dtA//=.
    case nB: next_alt => [B'|]//=.
      case dtA: (det_tree_aux _ _ A) => /=[[DA sVA]|]//=.
      case dtB0: (det_tree_aux _ _ B0) => /=[[DB0 sVB0]|]//=.
      case dtB: (det_tree_aux _ _ B) => /=[[DB sVB]|]//=.
      ...
      have /=fB := next_alt_failed nB.
      have [V' H] := det_tree_aux_less_precise V dtA.
      case: ifP => //=sA[<-]/=; rewrite nA nB0/= ?(next_alt_not_failed _ fB) dtA/= dtB0 /=.
        rewrite success_failed//=.
        have {}HB := (HB _ _ _ _ _ V' dtB nB).
        by rewrite HB/=M//=.
      rewrite nB !orbF/=fA/=.
      by rewrite dtB/=M//.
    move=> [??]; subst.
    ...
    have /=fA':= next_alt_failed nA.
    have /=fB0':= next_alt_failed nB0.
    by rewrite fA/= nA nB0 nB//=.
  move=> [??]; subst.
  by rewrite (next_alt_None_failed nA) in fA. *)
(* Admitted. *)

(*
Lemma next_alt_flow {sP sV1 A A' B0 ign1 ign2 DA sVA sV2 d sX d1}:
  valid_sig sV1 ->
  det_tree_aux sP sV1 A ign1 = ty_ok (DA, sVA) ->
  minD DA d1 = d1 -> minD ign1 ign2 = ign2 ->
  next_alt false A = Some A' ->
  det_tree_aux sP sV1 A' ign2 = ty_ok (d1, sX) ->
  det_tree_aux sP sVA B0 DA = ty_ok (d, sV2) ->
  exists d' sV2', 
    minD d d' = d' /\ less_precise sV2 sV2' /\
    det_tree_aux sP sX B0 d1 = ty_ok (d', sV2').
Proof.
  simpl in *.
  elim: A sV1 A' B0 ign1 ign2 DA sVA sV2 d sX d1 => //=.
  - move=> sV1 []// B0 ign1 ign2 DA sVA sV2 d sX d1 V [??]/=; subst.
    move=> H1 H2 H3 [??] H4; subst.
    rewrite -H1; destruct DA => /=.
      rewrite H4/=; repeat eexists; rewrite ?minD_refl//less_precise_refl//.
      by have []:= det_tree_aux_less_precise V H4.
    have [d'[<- H5]] := det_tree_aux_func2 H4.
    have [H6 H7] := det_tree_aux_less_precise V H4.
    destruct d1; rewrite ?H4?H5; repeat eexists; rewrite?less_precise_refl//=?minD_refl//.
    rewrite minD_comm -minD_assoc minD_refl//.
  - move=> p c sV1 A// B0 ign1 ign2 DA sVA sV2 d sX d1 V.
    case C: check_callable => //=[[D S]][??]; subst.
    move=> <-<-[?]; subst => /=.
    have [H1 [V1 H2]] := check_callable_less_precise V C. 
    destruct ign1 => /=.
      rewrite C/= => -[+?]; subst.
      move=> + /[dup]/det_tree_aux_less_precise -/(_ V1) [H3 H4].
      destruct D => //=.
        move=> _  ->; repeat eexists; rewrite ?minD_refl//less_precise_refl//.
      move=> <-{d1}->; repeat eexists; rewrite ?minD_refl//less_precise_refl//.
    rewrite maxD_comm/= in H1; subst.
    have [d2 [<- H3]]:= check_callable_func2 C.
    rewrite minD_comm/= in H3.
    move=> + /[dup]/det_tree_aux_less_precise -/(_ V1) [H4 H5].
    destruct ign2; rewrite ?C?H3/= => -[??]; subst => H.
      have [d2[<- H6]] := det_tree_aux_func2 H.
      destruct d1; rewrite ?H?H6/=; repeat eexists; rewrite?less_precise_refl//?minD_refl//.
      rewrite minD_comm -minD_assoc minD_refl//.
    rewrite H/=; repeat eexists; rewrite ?minD_refl//less_precise_refl//.
  - move=> sV1 []//= B ign1 ign2 []// sVA sV2 d sX []// V [<-]{sVA} _ _ _ [<-]{sX}.
    move=> /[dup]/det_tree_aux_less_precise -/(_ V) [H4 H5].
    move=> ->; repeat eexists; rewrite ?minD_refl//less_precise_refl//.
  - move=> A HA s B HB sV1 C B0 ign1 ign2 D sV sV2 d sX d1 H.
    case nA: next_alt => //=[A'|].
      move=> +++[<-]/=.  
      have /=fA' := next_alt_failed nA.
      rewrite (next_alt_not_failed _ fA').
      ...
      case nB: next_alt => //=[B'|].
        ...
        case dtA': (det_tree_aux _ _ A') => /=[[DA' sVA']|]//=.
        case dtB': (det_tree_aux _ _ B) => /=[[DB' sVB']|]//= => -[] + ?; subst.
        have := HA _ _ _ _ _ _ _ _ _ _ _ H dtA _ _ nA dtA'.
        rewrite minD_assoc minD_refl.
        move => -/(_ _ _ _ _ erefl) {}HA.
        case:ifP => cA/=.
          move=> H1 H2.
          destruct DA; simpl in *.
            destruct DB; simpl in *.
              rewrite H2; repeat eexists; admit.
            have [d2 [<- H3]] := det_tree_aux_func2 H2.
            destruct d1; simpl in *; rewrite ?H3?H2/=; repeat eexists; admit.
          have [d2 [<- H3]] := det_tree_aux_func2 H2.
          destruct d1; simpl in *; rewrite ?H3?H2/=; repeat eexists; admit.
        move=> ?; subst.
        move=> H2; rewrite H2/=; repeat eexists; admit.
      move=> H1 H2 H3 H4 H5.
      by apply: HA H H1 _ H3 nA H4 H5.
    move=> H1 H2 H3.
    rewrite if_same.
    case nB: next_alt => //[B'][<-]/=.
    rewrite (fun_if (fun x => next_alt false x)) nA (is_dead_next_alt _ is_dead_dead) if_same//=.
    apply: HB H H1 H2 H3 nB.
  - move=> A HA B0 HB0 B HB sV1 C E ign1 ign2 D sV sV2 d sX d1 H.
    case fA: failed => //=.
      case nB0: next_alt => //=[B0'|].
        ...
        have /= fB0' := next_alt_failed nB0.
        move=> H1 H2 [<-]{C}/=.
        rewrite (next_alt_not_failed _ fA') fA'/= nB0/=.
        rewrite (next_alt_not_failed _ fB0')/=.
        case dtA': (det_tree_aux _ _ A') => /=[[DA' sVA']|]//=.
        case dtB: (det_tree_aux _ _ B0) => /=[[DB sVB]|]//=.
        case dtB': (det_tree_aux _ _ B0') => /=[[DB' sVB']|]//= => -[] + ?; subst.
        move=> H3 H4.
        have [V L] := det_tree_aux_less_precise H dtA'.
        have {}HA := HA _ _ _ _ _ _ _ _ _ _ _ H dtA _ _ nA dtA'.
        apply: HB0 nB0 _ H4.
        have {}HB0 := HB0 _ _ _ _ _ _ _ _ _ _ _ V _ _ _ nB0 _ H4.
        apply: HB0. dtB0'.
        destruct DB; simpl in *; subst.
          destruct D; simpl in *; subst.
            apply: HB0; eauto.
            destruct ign1; simpl in *; subst.
              destruct DA'; subst; auto.
...
Lemma failed_det_tree_next_alt {sP A B sV1 sV2 d ign ign1 b}:
  valid_sig sV1 ->
  det_tree_aux sP sV1 A ign = ty_ok (d, sV2) ->
    (* is_ty_ok (det_tree_aux sP sV1' A ign1) -> *)
    (* less_precise sV1 sV1' -> *)
    next_alt b A = Some B ->
      minD ign ign1 = ign1 ->
      exists d1 sV2', minD d d1 = d1 /\ (less_precise sV2 sV2') /\
        (det_tree_aux sP sV1 B ign1) = ty_ok (d1, sV2') .
Proof.
  elim: A b B sV1 sV2 ign ign1 d => //=; last first; [|admit..].
  - move=> A HA B0 HB0 B HB b C sV1 sV2 ign1 ign2 d sV1v.
    case fA: failed => /=.
      case nA: (next_alt _ A) => //=[A']; rewrite orbF.
      case nB0: (next_alt _ B0) => //=[B0'] + [<-]{C}/=.
      have /= fA' := next_alt_failed nA.
      have /= fB0' := next_alt_failed nB0.
      rewrite fA'/= (next_alt_not_failed _ fA')//=.
      rewrite (next_alt_not_failed _ fB0')/= andbF.
      ...
      have {HA}:= HA _ _ _ _ _ _ _ sV1v dtA nA H.
      move => -[d1 [sX[H3 [H4 H5]]]].
      rewrite H5/=.
      have [sVAv L] := det_tree_aux_less_precise sV1v dtA.
      have {HB0} := HB0 _ _ _ _ _ _ _ sVAv dtB0 nB0 H3.
      move => -[d2 [sY[H6 [H7 H8]]]].
      rewrite  H8.
      (* rewrite H8. *)
      (* sV1 <= sVA <= sV2
...
(*
      ...
      have {HB HB0} := (HB0 _ _ _ _ _ _ (det_tree_aux_less_precise sv1v dtA).1 dtB0 nB0).
      move=> HB0.
      destruct DA; simpl in H1, HB0; subst.
        admit.
      admit.
    rewrite !(next_alt_not_failed _ fA)/=.
    case:ifP.
      move=> /andP[/eqP nB0 /eqP nB] [??]; subst.
      case:ifP => sA.
        have fB := next_alt_None_failed nB.
        have sB := failed_success _ fB.
        have fB0 := next_alt_None_failed nB0.
        rewrite next_alt_false_true//nB nB0.
        by case nA: (next_alt _ A) => //=[A'].
      move=> [<-]/=.
      rewrite fA/= next_alt_not_failed//=nB0 nB/= => H.
      repeat eexists; eauto.
      by apply: less_precise_refl.
    case nB0: (next_alt _ B0) => //=[B0'|].
      case nB: (next_alt _ B) => //=[B'|] _.
        case dtA: (det_tree_aux _ _ A) => /=[[DA sVA]|]//=.
        case dtB0: (det_tree_aux _ _ B0) => /=[[DB0 sVB0]|]//=.
        case dtB: (det_tree_aux _ _ B) => /=[[DB sVB]|]//=.
        move=> [??]; subst.
        case:ifP => sA {B' nB}.
          case nB: (next_alt _ B) => //=[B'|].
            ...
            rewrite success_failed//=.
            rewrite next_alt_not_failed?success_failed//=.
            rewrite nB0//=(next_alt_not_failed _ (next_alt_failed nB))//=.
            have [d2[H1 H2]] := det_tree_aux_func2 dtA.
            have {HB} := (HB _ _ _ _ _ _ (det_tree_aux_less_precise sv1v dtA).1 dtB nB).
            destruct ign; simpl in H; subst.
              rewrite H2/=.
              admit.
            admit.
          case nA: (next_alt _ A) => //=[A'][<-] H.
          have /={HA}[d1 [sv2[H1 [H2 H3]]]]:= (HA _ _ _ _ _ _ sv1v dtA nA H).
          have /=fA' := next_alt_failed nA.
          have /=fB0' := next_alt_failed nB0.
          rewrite H3/= fA'/= next_alt_not_failed//= nB0/=.
          rewrite next_alt_not_failed//=.
          have {HB}HB0 := (HB0 _ _ _ _ _ _ (det_tree_aux_less_precise sv1v dtA).1 dtB0 nB0).
          admit.
        move=> [<-]/= H.
      ...
      have sB:= failed_success _ fB.
      rewrite next_alt_false_true//=nB.
      have HH:= det_tree_aux_less_precise sv1v dtA.
      case:ifP => sA//= + H.
        case nA: (next_alt _ A) => //=[A'][<-]{C}/=.
        have /= fA' := next_alt_failed nA.
        have /= fB0' := next_alt_failed nB0.
        rewrite fA'/= (next_alt_not_failed _ fA')//=.
        rewrite (next_alt_not_failed _ fB0')/= andbF.
        have {HA}[d1 [sv2[H1 [H2 H3]]]]:= (HA _ _ _ _ _ _ sv1v dtA nA H).
        rewrite H3/=.
        have {HB HB0} := (HB0 _ _ _ _ _ _  HH.1 dtB0 nB0).
        admit.
      move=> [<-]/=.
      rewrite fA/= next_alt_not_failed//=.
      rewrite nB0 nB/=.
...
Proof.
  ...
  - move=> s1 s2 A B r n fA nA _ dtB dtA.
    ...
    have:= failed_det_tree_next_alt _ dtA.
    move=> /= /(_ false isT).
    rewrite nA => -[d'[H1 ->]].
    by rewrite minD_comm/= in H1; subst.
    (* have := (failed_det_tree_next_alt _ dtA nA erefl) => /(_ isT). *)
    (* rewrite dtA less_precise_refl//= => /(_ isT isT). *)
