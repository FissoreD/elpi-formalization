...
Section NurEqiv.
  ...
  (* Lemma expandedb_failure_next_alt_state_to_list_cons {s1 A B C b1}:
    valid_state A -> expandedb u s1 A (Failed B) b1 -> 
      next_alt false B = Some C -> state_to_list_cons C -> 
  ...
  Proof.
    ...
    - move=> A HA B0 HB0 B HB s2 b/=/and5P[_ vA _]++++l.
  ...
  Proof.
    ...
    - move=> A HA s B HB s3 C l b/=.
      ...
      rewrite (state_to_list_dead is_dead_dead)//.
    - move=> A HA B0 HB0 B HB s1 C l b /=/and5P[_ vA _].
  ...
  Lemma expand_solved {s A} l sx:
    ...
    { x & { xs &
      ...
      (nur u (get_substS s A) x xs (get_substS s A) (state_to_list (odflt Bot (next_alt true A)) sx l)))%type } }.
  Proof.
    move=> vA sA.
    do 2 eexists; split.
  ...
  Lemma expand_cb_failedF {s1 s2 A B}:
    valid_state A ->
    expand u s1 A = Expanded true s2 B -> failed B = false.
  Proof.
    elim: A B s1 s2 => //=.
    - move=> B s1 s2 _ [_<-]//.
    ...
    - move=> A HA B0 _ B HB C s1 s2 /and5P[_ vA _].
      ...
      case e1: expand => //[b s3 B'] vB bB0 [? _<-]/=; subst.
  ...
  (* TODO: put this prop inside s2l_CutBrothers *)
    valid_state A -> expand u s1 A = Expanded true s2 B -> ((s2 = get_substS s1 A) * (get_substS s1 A = get_substS s1 B)).
  Proof.
    elim: A B s1 s2 => //=.
    - move=> B s1 s2 _ [<-<-]//.
    - move=> A HA s B HB C s1 s2; case: ifP => dA; case: expand => //.
    - move=> A HA B0 _ B HB C s1 s2 /and5P[_ vA _].
      case e: expand => //[b s' A'|s' A'].
        rewrite (expand_not_solved_not_success _ e)//=(expand_not_failed _ e)//=.
        move=>/eqP-> bB [? <-<-]/=; rewrite (get_substS_base_and bB)// if_same; subst.
        rewrite !(HA _ _ _ vA e)//.
      have [[??]sA] := expand_solved_same _ e; subst.
      rewrite sA/= => vB bB.
      case e1: expand => //=[b s1' B'][?<-<-]/=; subst; rewrite success_cut sA ges_subst_cutl.
      rewrite !(HB _ _ _ vB e1)//.
  Qed.

  Lemma s2l_CutBrothers {s1 A s2 B} sA l1:
    valid_state A -> expand u s1 A = Expanded true s2 B -> 
      Texists x tl, 
  ...
  Proof.
    ...
    - move=> A HA B0 _ B HB sA s1 s2 C l1/=/and5P[oA vA aB].
      ...
      have [H2|[hd[H2 H3]]] := bbAnd_state_to_list bB0; rewrite H2/=.
        ...
        rewrite (base_and_ko_state_to_list (bbAnd_cutl bB0))//=H1.
        rewrite/make_lB01 map_cons; do 2 f_equal.
        rewrite success_cut sAx ges_subst_cutl//.
        rewrite H1//.
      set X:= make_lB0 _ _.
      have [x[tl [H [H1 H4]]]] := HB (get_substS sA A') _ _ _ (X++l1) vB eB.
      rewrite H//=.
      ...
      rewrite H1///make_lB01 map_cons; repeat f_equal.
      rewrite success_cut sAx ges_subst_cutl//.
  Qed.

  (* Lemma s2l_Expanded_nil {A B s1 s2 s4 l1 b ws}: valid_state A ->
    state_to_list A s1 l1 = (s4, nilC) ::: ws -> expand u s1 A = Expanded b s2 B -> 
      ((failed B = false) * (state_to_list B s1 l1 = (s4, nilC) ::: (if b then nilC else ws)) * (s4 = s2) * (s2 = get_substS s1 A) * (forall x, get_substS x A = get_substS x B))%type.
  Proof.
    elim: A B s1 s2 s4 l1 ws b => //=; auto.
    - move=> B s1 s2 s4 l1 ws b _ []?? [?<-<-]//; subst => //.
    - move=> A HA s B HB C s1 s2 s4 l1 ws b/=.
      case:ifP => [dA vB|dA/andP[vA bB]].
        rewrite state_to_list_dead//=.
        case sB: state_to_list => //[[s5 []] xs]//=[??]; subst.
        case e: expand => //[b1 s' B'][???]; subst; rewrite/=dA (state_to_list_dead dA).
        rewrite !(HB _ _ _ _ _ _ _ vB sB e); repeat split.
        case: b1 e => //= e.
        have [x[tl[H1 H2]]]:= s2l_CutBrothers s nilC vB e.
        by rewrite H1// in sB.
      case eA: expand => //[b' s' A'].
      have [s5 [x[xs H]]]:= failed_state_to_list vA (expand_not_failed _ eA notF) s1 (state_to_list B s nilC).
      rewrite H; case: x H => H //=[??][???]; subst.
      have [{}HA X] := (HA _ _ _ _ _ _ _ vA H eA).
      rewrite!HA//= (expand_not_dead _ dA eA) !HA//.
      repeat split => //.
      case: b' eA HA => //eA HA; rewrite ?HA//.
      have [x[tl[H1 H2]]]:= s2l_CutBrothers s1 ((state_to_list B s nilC)) vA eA.
      by move: H; rewrite H1 //.
    - move=> A HA B0 _ B HB C s1 s2 s4 l1 ws b/=/and5P[oA vA aB].
      case eA: expand => //[b'' s1' A'|s1' A'].
        rewrite (expand_not_solved_not_success _ eA notF)(expand_not_failed _ eA notF)/=.
        move=> /eqP->bB.
        ...
        case: hd H => //= H [??][???]; subst.
        have [{}HA X] := (HA _ _ _ _ _ _ _ vA sA eA).
        rewrite !HA/=H/=!HA/=H base_and_failed//andbF//.
        repeat split.
          by case: b eA HA => //.
        by move=> x; rewrite (get_substS_base_and bB) if_same//.
      have [[??] sA] := expand_solved_same _ eA; subst.
      case eB: expand => //[b'' s1'' B']; rewrite sA/= => vB bB0.
      move=>+ [???]; subst => /=.
      rewrite (success_state_to_list empty)//=.
      case: b eB => eB; last first.
        rewrite (success_state_to_list empty _ sA)//= sA/=.
        have [H|[hd [H H1]]] := bbAnd_state_to_list bB0; rewrite H/=.
          rewrite !make_lB01_empty2 => H1.
          have [{}HB X]:= HB _ _ _ _ _ _ _ vB H1 eB.
        ...
        rewrite H2/= => -[???]; subst.
        have [{}HB H3] := HB _ _ _ _ _ _ _ vB H2 eB.
        by rewrite !HB success_failed//.
      rewrite -success_cut in sA.
      rewrite sA success_failed//=.
      have vAc := valid_state_cut vA.
      rewrite (success_state_to_list empty _ sA)//=.
      have bBcl:= bbAnd_cutl bB0.
      have kB0 : is_ko ((cutl B0)) by rewrite base_and_ko_is_ko.
      rewrite (is_ko_state_to_list kB0)/=.
      rewrite !make_lB01_empty2.
      have [H|[hd [H H1]]] := bbAnd_state_to_list bB0; rewrite H/=.
        move=>H1.
        have [{}HB X]:= HB _ _ _ _ _ _ _ vB H1 eB.
        rewrite !HB ?success_failed; repeat split => //*; rewrite ges_subst_cutl//.
        by rewrite !HB//.
      set m := make_lB0 _ _ ++ _.
      have [s[x[xs H2]]]:= failed_state_to_list vB (expand_not_failed _ eB notF) (get_substS s1 A') m.
      rewrite !make_lB01_empty2.
      rewrite H2/= => -[???]; subst.
      have [{}HB H3] := HB _ _ _ _ _ _ _ vB H2 eB.
      rewrite !HB; repeat split; last first.
        by move=> *; rewrite ges_subst_cutl//.
      ...
      have:= [elaborate @s2l_size B' X (Y++l1) X l1].
      case W: state_to_list => //= [|[sz z]zs];
      case Z: state_to_list => //=[[sk k]ks].
  ...
  Proof.
    ...
    - move=> A HA s B HB C s1 s2 s3 l p t gs xs.
      case: ifP => //[dA vB|dA /andP[vA bB]].
        ...
        move=> [?]; subst.
        move=>[]; last first.
          by move=>[?]H; subst; rewrite !H; split => //; right; rewrite (state_to_list_dead dA)//.
        move=> /=HB; split => //; left; move: HB.
        rewrite cats0.
      ...
      move=>[fA' H1]; rewrite fA'; split; auto; left; split => //.
    ...
    - move=> A HA B0 _ B HB C s1 s2 s3 l p t gs xs /and5P[_ vA _].
      case e: expand => //[s1' A'|s1' A'].
        ...
        move=> H1; rewrite !H1; split; auto; left; split => //=.
  ...
  (* Lemma s2l_Expanded_expanded_Done {s s' s1 A B C b sx x xs} l:
    ...
    s2lA = (sx, x) ::: xs ->
      (((s2lA = state_to_list B s l)
        +
      (sx = get_substS s B /\ Texists ca tl, x = (cut ca) ::: tl /\state_to_list B s l ++ l = (sx, tl) ::: ca)%type2) /\ get_substS s B = get_substS s A)%type2.
  Proof.
    ...
    - move=> A HA s B HB C D s1 s2 s3 b l sx x xs.
      case: ifP => /=[dA vB|dA/andP[vA bB]].
        ...
        have [y[ys [H2 [H2' H3]]]] := s2l_CutBrothers s' nilC vB e.
      ...
      move=> [b1 [H1 H2]]; subst.
        ...
        by do 2 eexists => //.
      have [y[ys[H3 [H3' H4]]]]:= s2l_CutBrothers s1 (state_to_list B s' nilC) vA e.
      have?:= expand_cb_same_subst _ e; subst.
      rewrite (expand_cb_same_subst1 vA e)//.
      split; auto.
      move: H; rewrite !H3/=H3' => -[???]; subst; right.
  ...
  *)
  (* Lemma s2l_expanded_Done {s s' A B b} sx:
    ...
    Texists x xs,
      ...
      (nur u (get_substS s A) x xs s' (state_to_list ((odflt Bot (next_alt true B))) sx nilC))).
  Proof.
  ...
  (* Lemma s2l_expanded_Failed_not_failed {A B x xs s b1 l y ys s1 s3} :
    ...
    state_to_list A s l = (s3, x) ::: xs -> expandedb u s A (Failed B) b1 ->
      state_to_list B s l = (s1, y) ::: ys ->
        ...
        Texists p t ca gs, (will_call x (xs++l)) = Some (p,t,ca,gs) /\
  ...
  (* Lemma ges_subst2 {s1 A B}: valid_state A ->
    get_substS s1 A = get_substS s1 B ->
     get_substS (get_substS s1 A) B = get_substS s1 B.
  Proof.
    elim: A B s1 => //=.
      ...
      valid_state A ->
        runb u s A s1 B b -> 
          Texists x xs,
            ...
            (nur u x.1 x.2 xs s1 (state_to_list (odflt Bot B) sIgn nilC))).

  Lemma runElpiP: forall A, runElpi A.
  Proof.
    move=> A s B s1 b ++ H.
    elim: H; clear.
    + move=> s1 s2 r A B /expand_solved_same [[??]sA] ? sIgn vA; subst.
      rewrite (success_state_to_list sIgn)//.
      repeat eexists.
      apply: StopE.
    + move=> s1 s2 s3 r A B n eA rB IH sIgn vA.
      have {IH} /= [[sy y]/=[ys [+ H4]]]:= IH sIgn (valid_state_expand _ vA eA).
    ...
    + move=> s1 s2 s3 r A B n eA rB IH sIgn vA. 
      ...
      case: g => [p c|ca] H.
        ...
        move=> [].
          ...
          rewrite cats0 in IH.
          apply: CallE X IH.
        by move=> [H1[H2 H3]]; rewrite H3 => -[???]; subst.
      have [[][]H1 H2 ? []H3] := s2l_Expanded_cut vA eA H; subst.
        rewrite H3 => -[???]; subst => //.
      rewrite cats0 in H3.
      rewrite H3 => -[???]; subst.
      by apply: CutE.
    + move=> s1 s2 A B C r n /expand_failed_same [? fB] nB rC IH sIgn vA; subst.
      have vC := valid_state_next_alt vA nB.
      have {IH} /= [[sx x][xs [H1 H2]]]:= IH sIgn vC.
      have H := failed_next_alt_some_state_to_list _ vA fB nB.
      rewrite H H1.
      by repeat eexists.
  Qed.
