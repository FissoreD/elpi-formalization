...
Theorem runS2T:
  forall p v a r, runS p v a r -> 
    forall s t, sld_tree t -> tree_to_stack t s [::] = a -> exists b v',
      match r with
      | None => runT p v s t Zero b v'
      | Some (s', [::]) => runT p v s t (One s') b v'
      | Some (s', a') => 
        exists t', runT p v s t (Many s' t') b v' /\ tree_to_stack t' s [::] = a'
      end.
