...
Proof.
  ...
  - move=> A HA s B HB d0 s0 d1 s1 H1 /tc_orP[tOA tOB tOs].
    ...
    have {}HB := HB _ _ _ _ (sigma2ctx_sub tOs) tOB tB.
...
Proof.
  ...
  - move=> A HA s B HB d0 s0 d1 s1 H1 /tc_orP[tOA tOB tOs].
    ...
    case tB: tc_tree_aux => [dB sB].
    have {}HA := HA _ _ _ _ H1 tOA tA.
    have {}HB := HB _ _ _ _ (compat_sig_sigma2ctx tOs) tOB tB.
...
Proof.
  ...
  - move=> A HA s B HB s0 d1 d2 dA dB N1 N2 ++ DR CS.
    ...
    have ? := HB _ _ _ _ _ _ _ tOB tNB (sigma2ctx_sub tOs) (compat_sig_sigma2ctx tOs) (bbOr_valid bB) tB tB'.
    subst. 
    have sax : compat_sig SB X.
      have CSB := tc_tree_aux_compat_sig (compat_sig_sigma2ctx tOs) tOB tB.
      have DBO := tc_tree_aux_sub (sigma2ctx_sub tOs) tOB tB.
...
Proof.
  ...
  - move=> A HA s B HB d0 s0 d1 d2 N1 N2 ++ DR CS.
    ...
    have [??] := HB _ _ _ _ _ _ tOB tNB (sigma2ctx_sub tOs) (compat_sig_sigma2ctx tOs) (bbOr_valid bB) tB tB'.
    subst. 
...
Lemma check_callable_big_or u X tyO O p c sP s1 N1 N2 d0 d1:
  ...
  tc_tree_aux sP (big_or u p s1 c) (X + tyO) (Func, O) = (d1, N2) -> 
  minD d0 d1 = d1.
Proof.
  move=> /= ME + CM.
  rewrite /check_callable.
...
Lemma check_tm_mp sP T tyO O N D1 S1 D2 S2:
  closed_in tyO T ->
  all_weak tyO ->
  compat_sig N tyO ->
  more_precise N O ->
  check_tm sP (tyO + O) T = (S1, D1) ->
  check_tm sP (tyO + N) T = (S2, D2) ->
  [/\ compat_type S1 S2, incl S2 S1 & (D2 || ~~D1)].
Proof.
  move=> + wO CS /and3P[D C I].
  elim: T S1 D1 S2 D2 => //.
    ...
    rewrite !lookup_cat.
    case: (fndP O) => kO.
      have kN := fsubsetP D _ kO.
      rewrite kN (in_fnd kN)/= => -[<-<-][<-<-].
      have:= forallP I (Sub v kO); rewrite valPE/= in_fnd//=; auto.
      by have:= forallP C (Sub v kO); rewrite valPE/= in_fnd//=; auto.
    rewrite/= in_fnd; case: fndP => kN/=; mv_sbst_catfA; last by [].
    have /eqP := forallP wO (Sub v vO); rewrite valPE => ->.
    have:= forallP CS (Sub v vO); rewrite in_fnd/=valPE.
    by move=> /comp_weak ->; rewrite incl_weakr//compat_type_weakL.
...
(* Lemma assume_call_mp_incl sP X tyO N O1 T A B:
  ...
  compat_type A B ->
   incl A B ->
  more_precise (assume_call sP (X + (tyO + N)) T A)
  (assume_call sP (tyO + O1) T B).
Proof.
  rewrite/assume_call.
  move=> + /andP[].
...
Lemma check_callable_mp {sP T X tyO O1 N d0 d1 dA dB N1 N2}:
  ...
  more_precise N O1 ->
  check_callable sP (tyO + O1) T d0 = (d1, N1) ->
...
Proof.
  ...
  have CC : closed_in (tyO + N) (Callable2Tm T).
    by rewrite closed_in_cat//  (andP C).2.
  have CX: compat_sig N tyO.
    by apply: compat_sig_cat1 CS.
  rewrite -catfA .
  rewrite (change_only_in_tm_ck_tm_ CC) //.
  rewrite /get_callable_hd_sig/get_tm_hd_sig.
  case c1: check_tm => [S1 B1].
  case c2: check_tm => [S2 B2].
  have {c1 c2}[] := check_tm_mp (andP C).2 (andP C).1 CX MP c1 c2.
  ...
  destruct B1 => + _; last first.
    have XX:forall xxx, more_precise (assume_call sP (X + (tyO + N)) T xxx)
    (tyO + O1) by move=> *; apply/more_precise_trans/HH/assume_tm_mp.
    move=> + [<-<-].
...
(* TODO: here *)
Lemma tc_tree_aux_cat_mp sP tyO X A O O1 N N1 d1 d2 dA dB:
  complete_sig X tyO ->
  let tyN := X + tyO in
  tc sP tyO A -> tc sP tyN A ->
  domf N `<=` domf tyN ->
  ...
  (minD d1 dA = dA -> minD d2 dB = dB) /\ more_precise N1 O1.
Proof.
  move=> CM H; rewrite/H domf_cat {H}.
  elim: A O O1 N N1 d1 d2 dA dB;
  only 1-3,5: by (move=> O O1 N N1 d1 d2 dA dB/= tO _ DR CS _ MP; mv_sbst_catfA; split; last apply: mp_cat21 (andP tO).1).
    move=> p c O O1 N N1 d1 d2 dA dB/= /tc_callP tO /tc_callP tc DR CS _ MP.
    case c1: check_callable => [D1 S1].
    case c2: check_callable => [D2 S2].
    mv_sbst_catfA.
  ...
  - move=> A HA s B HB O O1 N N1 d1 d2 dA dB /tc_orP[tOA tOB tOs]/tc_orP[tNA tNB tNs].
    ...
    case kA: is_ko => /=.
      ...
      case tB: tc_tree_aux.
      case tB': tc_tree_aux => /=.
      have [] := HB _ _ _ _ _ _ _ _ tOB tNB _ _ _ _ tB tB'.
    ...
    case: ifP => kB.
      ...
      case tB: tc_tree_aux.
      case tB': tc_tree_aux => /=.
      have [] := HA _ _ _ _ _ _ _ _ tOA tNA _ _ _ _ tB tB' => //.
      by destruct d1, dA.
    mv_sbst_catfA.
    case tA:  (tc_tree_aux _ A) => [DA sA].
    case tA': (tc_tree_aux _ A) => [DA' sA'].
    have [MIN1 MP1] := HA _ _ _ _ _ _ _ _ tOA tNA DR CS vA MP tA tA'.
    case tB:  (tc_tree_aux _ B) => [DB sB].
    case tB': (tc_tree_aux _ B) => [DB' sB'].
    have [] := HB _ _ _ _ _ _ _ _ tOB tNB _ _ _ _ tB tB'.
      by rewrite fsubsetU// sigma2ctx_sub//orbT.
  ...
  move=> A HA B0 HB0 B HB O O1 N N1 d1 d2 dA dB /tc_andP[tOA tOB0 tOB]/tc_andP[tNA tNB0 tNB].
  move=> DR CS/= + MP.
  move=> /and4P[vA ++ cB].
  case: ifP => /=[SA VB bB|SA /eqP -> {B0 HB0 tOB0 tNB0 cB}].
    rewrite success_is_ko//=.
    case: ifP => kB0.
      apply: HB => //.
    ...
    case tA:  (tc_tree_aux _ A) => [DA sA].
    case tA': (tc_tree_aux _ A) => [DA' sA'].
    have [MINA MP1] := HA _ _ _ _ _ _ _ _ tOA tNA DR CS vA MP tA tA'.
    case tB:  (tc_tree_aux _ B) => [DB sB].
    case tB': (tc_tree_aux _ B) => [DB' sB'].
    case tB0: (tc_tree_aux _ B0) => [DB0 sB0].
    case tB0': (tc_tree_aux _ B0) => [DB0' sB0'].
    ...
    have [] := HB _ _ _ _ _ _ _ _ tOB tNB _ _ _ _ tB tB' => //.
      ...
      by rewrite (get_ctxS_tcE _ tOA tNA) more_precise_get_ctxS.
    have [] := HB0 _ _ _ _ _ _ _ _ tOB0 tNB0 _ _ _ _ tB0 tB0' => //.
      by rewrite -domf_cat; apply: tc_tree_aux_sub tA'; rewrite//domf_cat.
      by apply: tc_tree_aux_compat_sig tA' => //.
      by apply: bbAnd_valid.
    move=> M1 MPB M2 MP2; split; last by apply: more_precise_merge2.
    destruct DB, DB0 => //=.
    by move=> /MINA /[dup] /M1 <-/M2 <-//.
  case kA: is_ko.
    rewrite is_ko_failed// => bB; mv_sbst_catfA.
    split; first by [].
    by apply: mp_cat21 (andP tOA).1.
  case kB: is_ko.
    move=> bB; mv_sbst_catfA.
    split; first by [].
    by apply: mp_cat21 (andP tOA).1.
  move=> bB.
  case tA:  (tc_tree_aux _ A) => [DA sA].
  ...
  have [] := HB _ _ _ _ _ _ _ _ tOB tNB _ _ _ _ tB tB' => //.
    by rewrite -domf_cat; apply: tc_tree_aux_sub tA'; rewrite//domf_cat.
    by apply: tc_tree_aux_compat_sig tA' => //.
    by move: bB; case: ifP => [_ /bbAnd_valid|_ /base_and_valid].
  by auto.
Qed.
...
Lemma tc_tree_aux_cat_mp_same_te sP tyO A O O1 N N1 d1 d2 dA dB:
  tc sP tyO A ->
  domf N `<=` domf tyO ->
  compat_sig N tyO ->
  valid_tree A ->
  more_precise N O ->
  tc_tree_aux sP A tyO (d1,O) = (d2, O1) ->
  tc_tree_aux sP A tyO (dA,N) = (dB, N1) ->
  (minD d1 dA = dA -> minD d2 dB = dB) /\ more_precise N1 O1.
Proof.
  move=> /[dup].
  rewrite -{2 3 4 6}(cat0f tyO).
  apply/tc_tree_aux_cat_mp /complete_sig_empty.
Qed.
...
Lemma get_ctxS_step_succ sP tyO A O N DA SA d0 :
  valid_tree A ->
  tc sP tyO A ->
  success A ->
  more_precise N O ->
  ...
  domf N `<=` domf tyO ->
  compat_sig N tyO ->
  tc_tree_aux sP A tyO (d0, O) = (DA, SA) ->
  more_precise (tyO + get_ctxS sP tyO N A) SA.
Proof.
  elim: A O N DA SA d0 => //=.
  ...
  move=> ++ MP wO DR CS.
  rewrite sA success_failed//= success_is_ko// => vB bB0.
  case: ifP => kB0.
    apply: HB => //=.
...
Lemma tc_tree_aux_step_cb' u sP tyO X A B s1 O1 O2 N1 N2 d0 d1 dA dB:
  complete_sig X tyO ->
  let tyN := X + tyO in
...
Proof.
  ...
  elim: A B s1 O1 O2 N1 N2 d0 d1 dA dB => //.
    move=> B s1 O1 O2 N1 N2 d0 d1 dA dB _ [<-]/= tOA _ DR1 CS1 DR2 CS2 MP; mv_sbst_catfA.
    split; first by [].
  ...
  move=> +++ /tc_andP[tOA tOB0 tOB] + DR1 CS1 DR2 CS2 MP.
  case: ifP => sA.
    ...
    case: ifP => kB0.
      ...
      apply: HB tB tB'; try eassumption.
        ...
        (* rewrite fsubsetU//get_ctxS_domf//. *)
      by apply: more_precise_get_ctxS.
    case tA: tc_tree_aux => [DA SA].
    case tB: tc_tree_aux => [DB SB]/=.
    case tB0: tc_tree_aux => [DB0 SB0]/=.
    ...
    have [] := HB _ _ _ _ _ _ _ _ _ _ vB eB tOB tNB _ _ _ _ _ tB tB'.
      by apply: get_ctxS_domf.
      by apply: compat_sig_get_ctxS.
      by rewrite -(get_ctxS_tcE _ tOA tNA) -domf_cat get_ctxS_domf//(domf_cat,tc_cat)//(andP CM).1.
      by rewrite -(get_ctxS_tcE _ tOA tNA) compat_sig_get_ctxS//tc_cat//(andP CM).1.
  ...
  case tA': tc_tree_aux => [DA' SA'].
  have {HA}[MIN MP1] := HA _ _ _ _ _ _ _ _ _ _ vA eA tOA tNA DR1 CS1 DR2 CS2 MP tA tA'.
  case tB': (tc_tree_aux _ B _ (_, SA')) => [DB' SB']/=.
  have [] := tc_tree_aux_cat_mp CM tOB tNB _ _ _ _ tB tB' => //.
...
Proof.
  ...
  elim: A B s1 O1 O2 N1 N2 d0 d1 dA dB => //=.
    ...
    split.
      destruct d0 => //=.
      destruct dA; rewrite minD_comm/= => ?; subst => //.
      (* have:= check_callable_big_or ME CkP CM cc tOr _ _ _ c1 tA. *)
      admit.
    admit.
  - move=> A HA s B HB C s1 O1 O2 N1 N2 d0 d1 dA dB /andP[ckA ckB].
    ...
    case: ifP => [deadA vB|deadA /andP[vA bB]].
      ...
      rewrite is_dead_is_ko//= (step_is_ko _ eB)//=;
      rewrite (kox_is_ko _ (step_is_ko _ eB _) eB)//; mv_sbst_catfA;
      case tB: tc_tree_aux => [DB SB];
      rewrite sigma2ctx_cat//;
      case tB': tc_tree_aux => [DB' SB']/=;
    ...
    case tA': (tc_tree_aux _ A') => [DA' SA']/=.
      ...
      case tB: (tc_tree_aux _ B) => [DB SB].
      case tB': (tc_tree_aux _ B) => [DB' SB']/=.
      have := tc_tree_aux_cat_mp _ _ _ _ _ _ _ tB tB'.
      move=> []//.
        by rewrite domf_cat fsubsetU// sigma2ctx_sub//orbT.
        by rewrite compat_sig_catR//(sigma2ctx_sub,compat_sig_sigma2ctx)//.
        by apply: bbOr_valid.
      move=> MINB MPB; split; last by apply: more_precise_merge2.
      case: ifP => cA; last by [].
  ...
  move=> A HA B0 HB0 B HB C s1 O1 O2 N1 N2 d0 d1 dA dB /and3P[ckA ckB0 ckB].
  move=> +++ /tc_andP[tOA tOB0 tOB] + DR1 CS1 DR2 CS2 MP SP.
  move=> /and4P[vA ++ _].
  case: ifP => /=[sA vB bB0|sA /eqP->{B0 HB0 ckB0 tOB0}].
    ...
    case: ifP => kB0.
      move=> tB tB'.
      apply: HB ckB vB eB kBB' tOB tNB _ _ _ _ _ _ tB tB'.
        by apply: get_ctxS_domf.
        by apply: compat_sig_get_ctxS.
    ...
    case tA: (tc_tree_aux _ A) => [DA SA].
    case tA': (tc_tree_aux _ A) => [DA' SA']/=.
    have [] := tc_tree_aux_cat_mp _ _ _ _ _ _ _ tA tA' => //.
      by rewrite domf_cat.
    move=> MINA MPA.
    case tB: (tc_tree_aux _ B) => [DB SB].
    case tB': (tc_tree_aux _ B') => [DB' SB']/=.
    ...
    case tB0': (tc_tree_aux _ B0) => [DB0' SB0']/=.
    mv_sbst_catfA.
    have [] := tc_tree_aux_cat_mp _ _ _ _ _ _ _ tB0 tB0'; try eassumption.
      by apply: tc_tree_aux_sub tNA tA'; rewrite domf_cat.
      by apply: tc_tree_aux_compat_sig tNA tA'.
      by apply: bbAnd_valid.
    move=> MINB0 MPB0.
    split; last by apply: more_precise_merge2.
    ...
    destruct DB' => //= _ _.
    (* wrong hyps: tB and tB' *)
    admit.
  case: ifP => fA bB; first by rewrite failed_expand.
  case eA: step => //[A'|A']; last by rewrite !(expand_solved_same _ eA) in sA.
  move=> [<-]{C}/= kAA' /tc_andP[tNA tNB0 tNB].
  rewrite failed_is_ko//base_and_is_ko// (kox_is_ko _ (step_is_ko _ eA _) eA)//.
  case tA: (tc_tree_aux _ A) => [DA SA].
  case tA': (tc_tree_aux _ A') => [DA' SA']/=.
  have := HA _ _ _ _ _ _ _ _ _ _ ckA vA eA kAA' tOA tNA _ _ _ _ _ _ tA tA'.
  move=> []//MINA MPA.
...
Proof.
  move=> ME CkP CM/= vA + tOA + DR CS SP tA.
  case S: step => [A'|A'|A'|A'] <- {r}/=tNA tA'; last first.
  - have [? sA] := expand_solved_same _ S; subst A'.
    have [] := tc_tree_aux_cat_min CM tOA tNA DR CS vA tA tA'.
    destruct d1, dA => //.
    by rewrite minD_comm => /(_ erefl)//.
  - have [? sA] := expand_failed_same _ S; subst A'.
    have [] := tc_tree_aux_cat_min CM tOA tNA DR CS vA tA tA'.
    destruct d1, dA => //.
    by rewrite minD_comm => /(_ erefl)//.
  - have [] := tc_tree_aux_step_cb' CM vA S tOA tNA DR _ _ _ _ tA tA' => //.
      by rewrite domf_cat fsubsetU//DR orbT.
    by apply: compat_sig_catR => //.
  - have [] := tc_tree_aux_step_exp ME CkP CM vA S _ tOA tNA DR CS _ _ _ _ tA tA' => //.
    admit.
      by rewrite domf_cat fsubsetU//DR orbT.
    by apply: compat_sig_catR => //.
Admitted.
