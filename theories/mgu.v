...
Definition is_mgu (mgu s : Sigma) :=
  exists2 r : Sigma, idempotent r &
      forall t, deref r (deref mgu t) = deref s t.

Definition mgu_help base mgu l :=
  forall s, idempotent s -> unifier s l -> is_mgu base s -> is_mgu mgu s.
...
Lemma in_vars x r t: idempotent r -> x \in domf r -> x \in vars (deref r t) = false.
Proof.
  move=> A xr.
...
Lemma in_vars_V x r v (vx : v \in domf r): idempotent r -> x \in domf r -> x \in vars r.[vx] = false.
Proof.
...
Proof.
  ...
  clear l Ul; move: M Ur => [r Ar M]; case: fndP => vx/=; last first.
    ...
    exists (deref_sigma v' (Tm_V v) r).
      apply: idempotent_deref_sigma => //; first by rewrite inE eq_sym.
...
Lemma montanariPmgu b l s mgu: idempotent s -> disjoint_L s l ->
...
Proof.
  ...
  - rewrite disjoint_L_cons/= => /and3P[D1 D2 D3] M.
    have {M IH} := IH _ (idempotent_deref_sigma _ _ A) (disjoint_deref_sigma_deref_list A _ _ _) M.
    rewrite inE in vt.
  ...
  - rewrite disjoint_L_cons/= => /and3P[D1 D2 D3] M.
    have {M}IH := IH _ (idempotent_deref_sigma vt D2 A) (disjoint_deref_sigma_deref_list A D2 vt D3) M.
    by apply: mgu_help_deref_sigma IH; rewrite fdisjointX1 in D1. 
Qed.

Definition mgu m t1 t2 :=
  idempotent m /\ deref m t1 = deref m t2 /\ forall s, idempotent s -> deref s t1 = deref s t2 -> 
    exists s', idempotent s' /\ forall t, deref s' (deref m t) = (deref s t).

Lemma ren_empty t: ren fmap0 t = t.
Proof. elim: t => //=[v|f -> a ->]//; rewrite not_fnd//. Qed.

Lemma is_mgu0 s: idempotent s -> is_mgu fmap0 s.
...
Proof.
  move=> U.
  have H := montanariPmgu idempotent_0 (disjoint_Lempty _) U.
  rewrite !deref_empty in H; repeat split.
    by apply: unif_idempotent idempotent_0 U.
    by have:= unify_P idempotent_0 U.
  move=> s' A D.
...
Lemma unify_complete:
  forall t1 t2, (exists s, idempotent s /\ deref s t1 = deref s t2) -> exists s', unify t1 t2 fmap0 = Some s'.
