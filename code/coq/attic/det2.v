...
Fixpoint inHelp prog n g l :=
  match l with
  | [::] => False
  | (x::xs) =>
      forall a,
      (exists n', run prog n g a = run prog (n+n') x a) 
      \/
      inHelp prog n g xs
  end.
...
Lemma all_goals_with_cut_more_general'' prog n r:
  ...
  (* first_empty g -> *)
    ...
    all_cut_alts gs ->
      run prog n g gs = r ->
      exists n' g', inHelp prog n' g' (g::gs) /\ n' <= n /\ run prog n' g' [::] = r.
Proof.
  intro.
  induction n.
  ...
  + destruct g.
    ...
    + 
      ...
      exists 0, (Goal (call p) ca :: g0); simpl; auto.
      constructor.
      left; exists 0; auto.
      auto.
      intros.
...
Lemma aa prog x x0 g tl gs:
   inHelp prog x x0 (not_alt_goal (g ++ tl) :: [seq [seq Goal x [::]  | x <- b] ++ not_alt_goal tl | b <- gs]) -> exists y, not_alt_goal y = x0.
Proof.
  simpl.
  intro.
  specialize (H [::]).
...
(* 
Lemma all_not_alt_goal_IND prog g tl gs x0:
   IN_IND prog x0
      (not_alt_goal (g ++ tl) :: [seq [seq Goal x [::]  | x <- b] ++ not_alt_goal tl  | b <- gs]) -> exists y, not_alt_goal y = x0.
Proof.
  intros.
  remember ((not_alt_goal (g ++ tl) :: [seq [seq Goal x [::]  | x <- b] ++ not_alt_goal tl  | b <- gs])).
  revert gs g tl Heql.
  induction H; intros; inversion Heql; clear Heql.
  exists (g0 ++ tl); auto.
  ...
  specialize (IHIN_IND gs0 l tl H2).
  auto.
Admitted. *)
