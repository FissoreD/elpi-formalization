...
(*   Definition g2a g := match g with Goal _ a => a end.
  ...
  Lemma forall2_alts_refl {P} l : (forall x, P x x) -> forall2  (fun x y : goal => g2p x = g2p y /\ P (g2a x) (g2a y)) l l.
  Proof. elim l; repeat constructor; auto. Qed.
  ...
  (* Fixpoint my_prefix_refl l :=
    match l return my_prefix l l with
    | nil => my_prefix_nil [::]
    | x::xs => my_prefix_con ((forall2_alts_refl x (fun x => my_prefix_refl x)) : forall2_alts my_prefix x x) (my_prefix_refl xs)
    end. *)


  Lemma my_prefix_refl l: my_prefix l l.
  Proof. 
      have X := (my_prefix_refl_help).
      have Y := forall2_refl (fun x y : goal => g2p x = g2p y /\ my_prefix (g2a x) (g2a y)).
      induction l.
        constructor.
      constructor; auto.
      (* Print and. *)
      (* pose x:= 
        fix f l :=
          match l return (forall2_alts my_prefix l l) with
          | nil => forall2_nil f
          | x :: xs => Y x (conj erefl (fun y => X (f x) y))
          end. *)
