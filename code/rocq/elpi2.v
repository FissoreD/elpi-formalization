  ...
  *)
  ...
  Section t2l.
    ...
    Section makers.
      ...
      Definition ad l A As := 
        if A == [::] then 
          A :: (add_deep (size As) l) As
        else (add_deep (size As).+1 l) (A::As) .
      ...
      *)
      ...
      Lemma size_ad h x xs: size (ad h x xs) = (size xs).+1.
      Proof. rewrite/ad; case:ifP; rewrite/= ?size_add_deep ?size_map//. Qed.
      ...
      Lemma add_alt_empty1 xs lB0 lB:
        ...
        match lB0 with
          | [::hd] => 
            let: tl := make_lB0 (add_deep (size xs) hd xs) hd in
            let: lB := make_lB lB tl in
            lB ++ tl
          | [::] => lB
          | _ => [::] (*unreachable*)
      ...
      Proof. 
        case:lB0 => [|y []]//=; rewrite/ad; case:ifP;
        rewrite /=make_lB_empty2 cats0//.
      Qed.
  ...
  Section tests.
    ...
    Goal forall s1 s2 A B C0 C p,
      ...
      (* (((! \/ A) \/ B)) /\ (! \/ C)*)
      state_to_list 
        (And (Or ((Or (Goal p Cut) s1 (f A))) s1 (f B)) (f C0) (Or (Goal p Cut) s2 (f C))) [::] = 
        [:: 
          [::cut [:: [:: call p B; call p C0]]; cut [::[:: call p A; call p C0]; [:: call p B; call p C0]]];
          [::cut [:: [:: call p B; call p C0]]; call p C];
          [:: call p A; call p C0]; 
          [:: call p B; call p C0]].
    Proof.
    ...
    Goal forall A B0 p s1,
        (* (OK \/ A) /\_B0 OK *)
      let f x := (Goal p (Call x)) in
      state_to_list (And (Or OK s1 (f A)) (f B0) OK) [::] =
      [::[::]; [::call p A; call p B0]].
    Proof.
      move=> A B0 p s1.
      rewrite/state_to_list//=.
    ...
    Goal forall A B0 p s1,
      (* (Bot \/ B) /\_b0 B0  *)
      let f x := (Goal p (Call x)) in
      state_to_list (And (Or Bot s1 (f A)) (f B0) (f B0)) [::] =
      [::[::call p A; call p B0]].
    Proof.
      move=> A B0 p s1.
      rewrite/state_to_list//=.
    ...
    Goal forall s A B C p,
      let f x := (Goal p (Call x)) in
      state_to_list (And (Or (f A) s (f B)) (Bot) (f C)) [::] =
      [:: [:: call p A; call p C]].
    Proof.
      move=> s A B C p.
      rewrite/state_to_list/=.
    ...
    *)
    ...
    Goal forall s1 s2 A B C p,
      ...
      (* ((! \/ ! \/ A) \/ B) \/ C *)
      state_to_list 
        (Or (Or (Or (And (Goal p Cut) Top Top) s1 ((Or (Goal p Cut) s1 (f A)))) s1 (f B)) s2 (f C)) [::] = 
        [:: 
          [::cut [:: [:: call p B]; [::call p C]]];
          [::cut [:: [:: call p B]; [::call p C]]];
          [:: call p A]; 
    ...
    Goal forall s1 s2 A B C D0 D p,
      ...
      (* (((! \/ ! \/ A) \/ B) \/ C) /\ D*)
      state_to_list 
        (And (Or (Or (Or (Goal p Cut) s1 ((Or (Goal p Cut) s1 (f A)))) s1 (f B)) s2 (f C)) (f D0) (f D)) [::] = 
        [:: 
          [::cut [:: [:: call p B; call p D0]; [::call p C; call p D0]]; call p D];
          [::cut [:: [:: call p B; call p D0]; [::call p C; call p D0]]; call p D0];
          [:: call p A; call p D0]; 
          [:: call p B; call p D0];
          [:: call p C; call p D0] ].
    Proof.
    ...
    *)
    Goal forall X s1 s2 A B C D0 D p,
      ...
      (* ((X \/ ((! \/ ! \/ A) \/ B) \/ C)) /\ D*)
      state_to_list 
        (And (Or (f X) s1 (Or (Or (Or (Goal p Cut) s1 ((Or (Goal p Cut) s1 (f A)))) s1 (f B)) s2 (f C))) (f D0) (f D)) [::] = 
        [:: 
          [:: call p X; call p D];
          [::cut [:: [:: call p B; call p D0]; [::call p C; call p D0]]; call p D0];
          [::cut [:: [:: call p B; call p D0]; [::call p C; call p D0]]; call p D0];
    ...
    (* Goal add_deep 1000 [::] [::[::cut' true [::[::cut' false [::]]]]] = [::].
    Proof.
      move=>/=.  *)
...
[seq apply_cut1
  ...
  Proof.
    ...
    - move=> A HA B0 _ B HB n l/=/and5P[oA vA aB].
      case:ifP => /=[sA vB bB0|sA/eqP->].
        ...
        rewrite add_alt_empty1 H !size_cat size_lB size_lB0 size_add_deep => H1.
        rewrite valid_ca_split//; last first.
          rewrite size_cat size_lB size_lB0 size_add_deep//.
        rewrite size_lB.
      ...
      case:ifP.
        ...
        rewrite valid_ca_split_cons empty_ca_valid//=.
        rewrite/make_lB0.
        admit.
      move=>/=.
      rewrite (all_lvlS_add_ca_false (base_and_lvlS bB (H [::]))).
  ...
  Section shape.
    ...
    Lemma size_add_deep_map xs ys z w:
      [seq size j | j <- xs] = [seq size j | j <- ys] ->
      [seq size j | j <- add_deep (size xs) z xs] =
      [seq size j | j <- add_deep (size ys) w ys].
    Proof.
      elim: xs ys => //=[|x xs IH][|y ys]//=[H1 H2].
      rewrite !size_map H1; f_equal.
      rewrite /add_deep_help.
      rewrite 
      f_equal.
        rewrite
       rewrite size_add_deep_help.
      rewrite IH.
    ...
    Proof.
      ...
      - move=> A HA B0 HB0 B HB l1 l2/=.
        ...
        case: (state_to_list_aux B) => [|t ts]; case: (state_to_list_aux B) => [|r rs]//=.
          case: x X H1 => //=; case: y Y => //=.
            rewrite -2!map_comp => H4 H5 _ _.
            apply : size_o_cat_fix_map => //.
            Search size add_deep.

            admit.
          move=> t ts _ m ms _ H4 _.
          rewrite -2!(map_comp size).
          apply: size_o_cat_fix_map => //.
          rewrite -2!map_comp !size_o_map_map//.
        move=>[H5 H6].
        case: x X H1 => //=; case: y Y => //=.
          move=> _ _ _; f_equal.
            rewrite !size_map//.
          rewrite !map_id !map_cat.
          f_equal.
            rewrite -2!map_comp !size_o_map_map//.
          rewrite -!map_comp.
          apply: size_o_cat_fix_map => //.
          admit.
        move=> m ms _ n ns _ [H].
        rewrite !size_cat !size_map H5 H; f_equal.
        rewrite !map_cat.
        f_equal.
