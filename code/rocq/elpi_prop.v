...
Module NurProp (U : Unif).
  ...
  Import Nur VS RunP Run Language.
    ...
    Proof. elim: l => /=[|x xs H]; rewrite ?cats0 ?H if_same//. Qed.
    ...
    Section props.
      ...
      Proof.
        move: n.
        have H := list_induction _ _ 
          (fun gs => forall n, (forall l : seq alt', add_deep bt n [::] l = l) ->
          [seq [seq add_deep_help add_deep bt n [::] j | j <- j] | j <- gs] = gs).
        apply: H; [auto| |apply:is_list_inhab id _].
        move=> /=g _ {}gs Hgs n H.
      ...
      (* Lemma size_lB lB tl: size (add_ca1 tl lB) = size lB.
      Proof. case: lB => //= _ lB; rewrite size_map//. Qed. *)
      ...
      Section make_lB_empty2.

        Lemma add_ca_deep_empty1 {n lB} : add_ca_deep n [::] lB = lB.
        Proof.
          elim: n lB => //= ++ lB.
          have H := list_induction _ _ (fun lB => forall n : nat,
          (forall (lB0 : seq (seq G)), add_ca_deep n [::] lB0 = lB0) ->
          [seq [seq add_ca'  [::] (apply_cut1 (add_ca_deep n [::]) x) | x <- j] | j <- lB] = lB).
          apply: H => //=; try by apply: is_list_inhab id _.
          move=> g _ gs IH n H; rewrite IH//; f_equal.
          move: n H; clear.
          have H := list_induction _ _ (fun g => forall (n : nat),
            (forall (lB0 : seq (seq G)), add_ca_deep n [::] lB0 = lB0) ->
            [seq add_ca' [::] (apply_cut1 (add_ca_deep n [::]) x) | x <- g] = g).
          apply: H => //=; try by apply: is_list_inhab id _.
          move=> {}g _ gs IH n H; rewrite IH//=; f_equal.
          case: g => //= l1; f_equal.
          rewrite cats0//.
  ...
  Section t2l_base.
    ...
    Proof.
      ...
      - move=> []//p a _ B0 _ B HB/=/andP[/eqP->bB].
        have [hd H]/= := HB bB.
        rewrite/=.
        case: a => [|t]; eexists => l; rewrite H//=if_same//.
    Qed.
  ...
  Lemma apply_cut1_id l: apply_cut1 id l = l.
  Proof. case: l => //=. Qed.

  Lemma apply_cut1_id_map {T: Type} F (l: list T): map (fun x => apply_cut1 id (F x)) l = map F l.
  ...
  Fixpoint valid_ca_aux n L1 L2 :=
    ...
    | 0 => true
    | n.+1 =>
      all_tail (fun xs ys => all (if_cut1 (fun alts => valid_ca_aux n alts alts && suffix alts ys)) xs) L1 L2
    end.
  ...
  Lemma empty_ca_if_cut n r hd:
    all empty_ca hd -> 
    all (if_cut1 (fun alts => valid_ca_aux n alts alts && suffix (alts) r)) hd.
  Proof.
  ...
  Section valid_empty_2_empty_ca.
    Lemma valid_cas_empty2_help1 {n gs}:
      ...
      (all (if_cut1 (fun alts => valid_ca_aux n alts alts && suffix (alts) [::])) gs) -> 
      all empty_ca gs.
    Proof.
      move: n.
      have H := list_induction _ _
    ...
    Lemma valid_cas_empty2_help {n gs}:
      ...
      size gs < n.+1 ->
      all_tail (fun gs ys =>
    all (if_cut1 (fun alts => valid_ca_aux n alts alts && suffix (alts) (ys))) gs) gs [::] -> 
    all (all empty_ca) gs.
    Proof.
      move: n.
      have H := list_induction _ _ 
        ...
        size gs < n.+1 ->
        all_tail
          (fun xs0 ys => all (if_cut1 (fun alts => valid_ca_aux n alts alts && suffix (alts) ( ys))) xs0)
          gs [::] -> all (all empty_ca) gs).
      apply: H => //.
  ...
  Section valid_ca_mn.
    Lemma valid_ca_mn_help3 n m1 l g:
      ...
      if_cut1 (fun alts => valid_ca_aux (n + m1) alts alts && suffix (alts) (l)) g =
      if_cut1 (fun alts => valid_ca_aux n alts alts && suffix (alts) (l)) g.
    Proof.
      destruct g => //=.
      move=> H1 H2.
    ...
    Lemma valid_ca_mn_help2 {n m1} {xs gs}:
      (forall m1 (x l : seq (seq G)),
        ...
        size xs <= n ->
          all (if_cut1 (fun alts => valid_ca_aux (n + m1) alts alts && suffix (alts) (xs))) gs =
          all (if_cut1 (fun alts => valid_ca_aux n alts alts && suffix (alts) (xs))) gs.
    Proof.
      move: n m1 xs.
    ...
    Lemma valid_ca_mn_help1 n m1 gs l:
      ...
      size l < n.+1 ->
      all_tail (fun xs0 ys => 
        all (if_cut1 (fun alts => valid_ca_aux (n + m1) alts alts && suffix (alts) (ys))) xs0) gs l =
      all_tail (fun xs0 ys => 
        all (if_cut1 (fun alts => valid_ca_aux n alts alts && suffix (alts) (ys))) xs0) gs l.
    Proof.
      move: n m1 l.
      have H := list_induction _ _ 
        (fun gs => forall n m1 l,
          ...
          size gs < (size l).+1 ->
          size l < n.+1 ->
          all_tail (fun xs0 ys => all (if_cut1 (fun alts => valid_ca_aux (n + m1) alts alts && suffix (alts) (ys))) xs0) gs l =
          all_tail (fun xs0 ys => all (if_cut1 (fun alts => valid_ca_aux n alts alts && suffix (alts) (ys))) xs0) gs l).
      apply: H => //.
  ...
  Section valid_ca_split_cat.
    ...
    Lemma valid_ca_split_gs_help_1 n x y l:
      all (if_cut1 (fun alts => valid_ca_aux n alts alts && suffix (alts) l)) (x ++ y) =
      all (if_cut1 (fun alts => valid_ca_aux n alts alts && suffix (alts) l)) x &&
      all (if_cut1 (fun alts => valid_ca_aux n alts alts && suffix (alts) l)) y.
    Proof.
      move: n y l.
      have H := list_induction _ _
      (fun x => forall n y l,
  ...
  Lemma add_ca_deep_more_less_help1 cB:
      forall n : nat,
        ...
        n0 <= n.+1 ->
        valid_ca cB ->
        [seq [seq add_ca' l (apply_cut1 (add_ca_deep n l) x) | x <- j] | j <- cB] = add_ca_deep n0 l cB.
  Proof.
    have H := list_induction _ _ (fun cB => forall n : nat,
      (forall (n0 : nat) (cB0 l : seq (seq G)),
      size cB0 <= n0 -> n0 <= n -> valid_ca cB0 -> add_ca_deep n l cB0 = add_ca_deep n0 l cB0) ->
      forall (n0 : nat) (l : seq (seq G)),
      size cB <= n0 ->
    ...
    move=> g _ gs Hgs n IH []//m l H1 H2.
    ...
    move: gs n m l.
    have H := list_induction _ _ (fun g => forall (gs : seq (seq G)) (n m : nat) (l : seq (seq G)),
      (forall (n0 : nat) (cB0 l0 : seq (seq G)),
      size cB0 <= n0 -> n0 <= n -> valid_ca cB0 -> add_ca_deep n l0 cB0 = add_ca_deep n0 l0 cB0) ->
      size gs < m.+1 ->
      m < n.+1 ->
      all
        (if_cut1
          (fun alts : seq (seq G) => valid_ca_aux (size gs) alts alts && suffix (alts) (gs)))
        g ->
      [seq add_ca' l (apply_cut1 (add_ca_deep n l) x) | x <- g] =
      [seq add_ca' l (apply_cut1 (add_ca_deep m l) x) | x <- g]).
    apply: H => //=; try by apply: is_list_inhab id _.
    move=> {}g _ gs Hgs gs1 n m l IH H1 H2 /andP[H3 H4].
  ...
  Lemma add_deep_more_less bt l1 l2 n m hd:
  ...
  Proof.
    ...
    have H := list_induction _ _ 
      ...
      size l2 <= size l3 ->
      size l3 <= n ->
      valid_ca_aux n l2 l3 -> add_deep bt (n + m) hd l2 = add_deep bt n hd l2) ->
      forall l2 : seq (seq G),
      size l1 <= size l2 ->
      size l2 <= n.+1 ->
      all_tail
        ...
        all
          (if_cut1
              (fun alts : seq (seq G) =>
              valid_ca_aux n alts alts && suffix alts ys))
          xs)
        l1 l2 ->
      [seq [seq add_deep_help add_deep bt (n + m) hd j | j <- j] | j <- l1] =
      [seq [seq add_deep_help add_deep bt n hd j | j <- j] | j <- l1]).
          apply: H => //=; try by apply: is_list_inhab id _.
      ...
      all
        (if_cut1
          (fun alts : seq (seq G) =>
            valid_ca_aux n alts alts && suffix (alts) ((behead l2))))
        g ->
      (forall l3 l4 : seq (seq G),
      size l3 <= size l4 ->
      size l4 <= n ->
      valid_ca_aux n l3 l4 -> add_deep bt (n + m) hd l3 = add_deep bt n hd l3) ->
      size l2 <= n.+1 ->
      [seq add_deep_help add_deep bt (n + m) hd j | j <- g] =
      [seq add_deep_help add_deep bt n hd j | j <- g]).
    apply: H => //=; try by apply: is_list_inhab id _.
    move=> {}g _ gs Hgs n l2 /andP[H1 H2] IH H3.
    rewrite (Hgs _ l2)//; f_equal.
    case: g H1 => //= l /andP[H4 H5]; f_equal.
    rewrite (IH _ l)//.
    have:= size_suffix H5.
    destruct l2 => //=.
      destruct l => //.
  ...
  Lemma add_deep_more_less1 bt l1 l2 m hd:
    size l1 <= size l2 -> size l2 <= m -> valid_ca_aux (size l2) l1 l2 ->
      add_deep bt m hd l1 = add_deep bt (size l2) hd l1.
  Proof.
    move=> H1 H2 H3.
    have [t <-]:= size_exists _ _ H2; rewrite addnC.
    apply: add_deep_more_less H1 (leqnn _) H3.
  Qed.

  Lemma suffix_add_deep bt hd ys l:
    suffix l ys -> valid_ca_aux (size l) l l ->
    suffix ((add_deep bt (size ys) hd l)) 
      ((add_deep bt (size ys) hd ys)).
  Proof.
    move=> H1 H4.
    have H5 := size_suffix H1.
    rewrite -(add_deep_more_less _ l l _ 1)//; last first.
      rewrite valid_ca_mn => //.
  ...
  Lemma valid_ca_add_ca_deep X Y m n o l:
    valid_ca l ->
    size X <= size Y -> size Y + size l <= n -> size Y <= o -> o <= m ->
    valid_ca_aux o X Y -> valid_ca_aux n (add_ca_deep m l X) (add_ca_deep o l Y ++ l).
  Proof.
    move=> vl.
    elim: n X Y m o => //=++X.
    have H := list_induction _ _
  ...
  Lemma valid_ca_aux_make_lB0_empty_ca l1 l2 hd n:
    ...
    size l1 <= size l2 -> size l2 <= n ->
      valid_ca_aux n (make_lB0 l1 hd) l2.
  Proof.
    move=> Hhd.
    elim: n l1 l2 => //++l1.
    have H:= list_induction _ _
  ...
  Lemma ttt n bt l m:
    size l <= m -> m <= n ->
    valid_ca l ->
    min_suffix n bt (add_ca_deep m bt l).
  Proof.
    rewrite/valid_ca.
    elim: n m l => //=+++l.
    have H:= list_induction _ _
      (fun l => forall n : nat,
        (forall (m : nat) (l0 : seq (seq G)),
        size l0 <= m ->
        m <= n ->
        valid_ca_aux (size l0) l0 l0 ->
        min_suffix n bt (add_ca_deep m bt l0)) ->
        forall m : nat,
        size l <= m ->
        m <= n.+1 ->
        valid_ca_aux (size l) l l ->
        all (all (if_cut (fun x : seq (seq G) =>
          (x == [::]) || suffix bt x && min_suffix n bt (take (size x - size bt) x)))) 
            (add_ca_deep m bt l)).
    apply: H => //=; try by apply: is_list_inhab id _.
      move=>*; rewrite add_ca_deep_empty2//.
    move=> g _ gs Hgs n IH []//=m H1 H2 /andP[H3 H4].
    rewrite (Hgs _ _ m.+1)//?(ltnW H1)//; last first.
      rewrite -(valid_ca_mn1 1)//addn1//.
    clear Hgs H4;rewrite andbT.
    move: m n {l} gs H1 H2 H3 IH.
    have H:= list_induction _ _
      (fun g => forall (m n : nat) (gs : seq (seq G)),
        size gs < m.+1 ->
        m < n.+1 ->
        all (if_cut1 (fun alts => valid_ca_aux (size gs) alts alts && suffix alts gs)) g ->
        (forall (m0 : nat) (l0 : seq (seq G)),
        size l0 <= m0 ->
        m0 <= n ->
        valid_ca_aux (size l0) l0 l0 ->
        min_suffix n bt (add_ca_deep m0 bt l0)) ->
        all
          (if_cut
            (fun x : seq (seq G) =>
              (x == [::])
              || suffix bt x &&
                min_suffix n bt (take (size x - size bt) x)))
          [seq add_ca' bt (apply_cut1 (add_ca_deep m bt) x) | x <- g]).
    apply: H => //=; try by apply: is_list_inhab id _.
    move=> {}g _ gs Hgs m n gs1 H1 H2 /andP[H3 H4] IH.
    rewrite (Hgs _ _ gs1)//andbT.
    case: g H3 => //=l/andP[H5 H6].
    clear Hgs H4.
    rewrite size_cat size_add_ca_deep.
    ...
    rewrite IH//; last first.
      apply: leq_trans H7 H1.
    move/suffixP: H6 =>/=[w?]; subst.
    rewrite suffix_catr?orbT//suffix_refl//.
  Qed.

  Lemma valid_state_valid_ca_help A r n bt:
    valid_state A -> state_to_list A bt = r -> 
    (* valid_ca_aux (size r) r (r++bt) -> *)
      size r <= n -> min_suffix n bt r.
  Proof.
    move=> + <-; clear r.
    elim: A n bt => //=; try by move=>[].
    - move=> p [|t]//=[]//.
    - move=> A HA s B HB n bt.
      rewrite size_cat size_add_ca_deep size_cat.
      case:ifP => [dA vB|dA /andP[vA bB]].
        rewrite (state_to_list_dead dA)//==>H1.
        apply: ttt => //.
  Abort.

  Lemma valid_ca_aux_make_lB0_help g hd tl:
    all empty_ca hd ->
    forall (n m o0 : nat) (ys : seq (seq G)),
      (forall (m0 o : nat) (xs0 ys0 : seq (seq G)),
      size xs0 <= size ys0 ->
      size ys0 <= m0 ->
      m0 <= o ->
      valid_ca_aux n xs0 (ys0 ++ tl) -> valid_ca_aux n (add_deep tl m0 hd xs0) (make_lB0 (add_deep tl o hd ys0) hd ++ tl)) ->
      size ys < m.+1 ->
      m < o0.+1 ->
      all (if_cut1 (fun alts : seq (seq G) => valid_ca_aux n alts alts && suffix alts (ys ++ tl))) g ->
      all
        (if_cut1
          (fun alts : seq (seq G) =>
            valid_ca_aux n alts alts &&
            suffix alts (make_lB0 (add_deep tl o0 hd ys) hd ++ tl)))
        [seq add_deep_help add_deep tl m hd j | j <- g].
  Proof.
    move=> Hhd.
    have H:= list_induction _ _ (fun g =>
      forall (n m o0 : nat) (ys : seq (seq G)),
        ...
        size ys0 <= m0 ->
        m0 <= o ->
        valid_ca_aux n xs0 (ys0 ++ tl) ->
        valid_ca_aux n (add_deep tl m0 hd xs0) (make_lB0 (add_deep tl o hd ys0) hd ++ tl)) ->
        size ys < m.+1 ->
        m < o0.+1 ->
        all (if_cut1 (fun alts : seq (seq G) => valid_ca_aux n alts alts && suffix alts (ys ++ tl))) g ->
        all
          (if_cut1
            (fun alts : seq (seq G) => valid_ca_aux n alts alts && suffix alts (make_lB0 (add_deep tl o0 hd ys) hd ++ tl)))
          [seq add_deep_help add_deep tl m hd j | j <- g]).
    apply: H => //=; try by apply: is_list_inhab id _.
    move=> {}g _ gs Hgs n m o ys IH H2 H /andP[H3 H4].
    apply/andP; split; last first.
      rewrite Hgs//.
    clear gs Hgs H4.
  ...
  (* se sono vuoti, allora aggiungere hd non ha effetto e sono contento, altrimenti,
     ...
     con tl
  *)
  Lemma valid_ca_aux_make_lB0 xs ys hd tl n m o: 
    size xs <= size ys -> size ys <= m -> m <= o ->
    valid_ca_aux n xs (ys++tl) -> valid_ca tl -> all empty_ca hd ->
    valid_ca_aux n (add_deep tl m hd xs) (make_lB0 (add_deep tl o hd ys) hd ++ tl).
  Proof.
    move=> + + + + vtl Hhd.
    elim: n m o xs ys=>//=++++xs.
    have H1 := list_induction _ _
      (fun xs => forall n : nat,
        ...
        size ys <= m ->
        m <= o ->
        valid_ca_aux n xs0 (ys ++ tl) -> valid_ca_aux n (add_deep tl m hd xs0) (make_lB0 (add_deep tl o hd ys) hd ++ tl)) ->
        forall (m o0 : nat) (ys : seq (seq G)),
        size xs <= size ys ->
        size ys <= m ->
        m <= o0 ->
        ...
        all_tail
          (fun (xs0 : seq G) (ys0 : seq (seq G)) =>
          all (if_cut1 (fun alts : seq (seq G) => valid_ca_aux n alts alts && suffix alts ys0)) xs0)
          (add_deep tl m hd xs) (make_lB0 (add_deep tl o0 hd ys) hd ++ tl)).
    apply: H1 => //; try by apply: is_list_inhab id _.
      move=> n _ []//.
    clear xs.
    move=> g _ gs Hgs n IH /=m o[]//= y ys H1 H3 H4 /andP[H5 H6].
    case: m H3 H4 => //m H3 H4.
    case: o H4 => //=o H4.
    apply/andP; split; last first.
      have {Hgs}/=Hgs := Hgs n IH m.+1 o.+1.
    ...
    clear gs Hgs H1 H6.
    have /= := valid_ca_aux_make_lB0_help g hd tl _ n m o.+1.
    move=>->//.
    apply: ltnW H4.
  Qed.
  ...
  Proof.
    ...
    - move=> A HA s B HB n l/=.
      ...
      rewrite valid_ca_split; rewrite?size_cat//drop_size_cat//.
      rewrite HA1 valid_ca_mn//leq_addl//.
    - move=> A HA B0 _ B HB n l /=/and5P[oA vA aB]++vl.
      ...
      rewrite valid_ca_make_lB0_empty_ca2//?andbT/=.
        ...
        apply: valid_ca_aux_make_lB0_help => //.
