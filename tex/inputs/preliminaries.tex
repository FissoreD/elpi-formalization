...
and \texttt{X} for unification variable names.
  ...
  \begin{array}{c|c}
    syntax & types \\\hline
      \begin{array}{rl}
        ...
        tm     & ::= \predVar\ \vecL{tm} \mid \texttt{X} \label{eq:tm} \\
      ...
      \quad
      \begin{array}{rl}
        psign   & ::= \dtype{\relI}{ty^\ast}{ty^\ast} \ \label{eq:ty} \\
...
%
%
A goal $\g = \goalCmd{\prog}{g}{\vecL{\alt}}$ is a triple made of a program \prog,
an atom $g$ and a list of alternatives $\vecL{\alt}$ that we refer to as cut-to alternatives.
An alternative $\alt = (\subst, \vecL{\g})$ is a pair made of a substitution
and a list of goals. A substitution \subst is a mapping
from unification variables \texttt{X} to terms $tm$. We write $\EmptySubst$ the empty
substitution and $\mathrm{dom}\ \subst$ for the set of variables occurring in the domain or in the codomain of \subst.
We write $\EmptySubst$ for the only substition s.t. $\dom\ \EmptySubst = \emptyset$.
...
and we say $t$ is ground if $\forall \subst, \subst\ t = t$. \\
The operational semantics is given by a relation
$\run{} \subseteq \alt \times \vecL{\alt} \times \vecL{\alt} \times \subst$.
We write \runCmd{\vecL{\g}}{\alt}{\subst}{\alt'}{\subst'}
when a goal list $\vecL{\g}$ under a substitution \subst and alternatives \alt
terminates with a substitution $\subst'$ and a remaining list of
(still unexplored) alternatives $\alt'$.
The rules for \run{} are given in~\cref{fig:basic-interp}\todo{rule per fail?} where
use $::$ for list cons; $@$ for list concatenation; $\EmptyList$ is the empty list;
$[e|x \in \vec{x}]$ for list comprehension; $\subst \in \Sigma$; $\alts \in \alt$; $al \in \vecL{\alt}$; $g \in \g$; $gl \in \vecL{\g}$.
An initial query looks like \runCmd{[\goalCmd{\prog}{\pred\ \vecL{t}}{\EmptyList}]}{\EmptyList}{\EmptySubst}{\_}{\_}

Note that in this simple settings the
program never changes during execution, but it does in section~\ref{sec:hoas}
where the implication operator \elpiIn{=>} is introduced.
...
goals $gl$. All new goals 

The stop rule~\ref{rule:stop} terminates as there are no more goals to be
solved.

The unify rule (\ref{rule:unif}) invokes the \unify algorithm on
the two terms $t_1$ and $t_2$ obtaining the updated substitution \subst['].

This semantics amounts to a \textit{hard-cut} operator
...
preference for maintaining a concrete representation the choice points.
...
We say that $\unifiable\ t_1\ t_2$ if an only if
  $\exists\ \subst\ s.t.\ \unifyCmd{t_1}{t_2}{\EmptySubst}{\subst}$.
We say that $\nUnify\ t_1\ t_2$ if an only if
  $\forall \subst \ \neg \exists\ \subst[']\ s.t.\ \unifyCmd{t_1}{t_2}{\subst}{\subst[']}$.
\end{definition}
...
its discriminating argument $d$ to be ground in the call.
The reasoning goes as follows.

\begin{lemma}[Split transitivity] Given a substitution \subst and
two terms $t_1$ and $t_2$ such that $\nUnify~ t_1\ t_2$, there is no such a term
$t_3$ and substitutions $\subst_1$ and $\subst_2$
such that $\subst_1 t_1 = \subst_1 t_3\ \land\ \subst_2 t_2 = \subst_2 t_3$.
\end{lemma}
...
succeeds. We say $\wellModed\ (\pred\ \vecL{t})$ if $\ctx\ \pred = \dtype{\relI}{i_1 \ldots i_k}{\_}$
and $\ground\ t_j$ for $j \leq k$.
\end{definition}
...
The objective is to indentify predicates that repsent mathamtical
functions rather than relations.
The literature on determinacy analysis goes beyond this coarse categorization
by identifying functions with an empty domain~\cite{1996henderson} or
predicate that are observably functions in the sense that
...
or that leave choice points that certainly fail.
\todo{visto che abbiamo una bigstep la definizione di sotto non mi piace
perche include anche la divergenza}
\todo{incoltre se ucciadiamo tutti punti di scelta abbiamo
anche una garanzia sulla complessità}

\begin{definition}[Functionality (\isfunc)]\label{def:is-det}
  Given a program \prog and a term $t$,
  \isdet\ is defined as follows:
  $$\forall a\ \subst, \isFuncCmd{t}$$
\end{definition}
\begin{definition}[Determinacy (\isdet)]\label{def:is-det}
  Given a program \prog and a term $t$,
  \isdet\ is defined as follows:
...
Our objective is to let the progrmmer declare which predicates
must behave as functions, hence we augment the gramma for signatures
as follows:
...
%
\begin{align}
  ty & ::= data \mid [\detI \mid \relI]\ data^\ast \funsep data^\ast
  \label{eq:pred}
\end{align}
