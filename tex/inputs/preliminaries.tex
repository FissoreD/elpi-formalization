...
an atom $g$ and a list of cut-alternatives \alt. An alternative is a list of goals
paired with a substitution $\subst$. A substitution is a mapping from
unification variables to terms.
The operational semantics to our language is given by a relation
$\run{} \subseteq \g \times \alt \times \subst \times \alt \times \subst$.
We write \runCmd{\g}{\alt}{\subst}{\alt'}{\subst'}
when a goal $\g$ under a substitution \subst and alternatives \alt
terminates with a substitution $\subst'$ and a remaining list of
(still unexplored) alternatives $\mathcal{A}'$.
...
\begin{figure}[!h]
  ...
  \bigg[
    ...
    \big[\goalCmd \prog g ~\alt \mid g \in \vecL{g}\big]
    %\Big)
    %\bigg)
    \mid \clauseCmd{\pred}{\vec{u}}{\vecL{g}} \in \prog\ \pred
    \bigg]
  $$
...
Remark that
the cut-alternatives stored in each goal are a suffix
of the (global-)alternatives to that goal: If the atom in the goal
is a cut, then the global alternatives are shortened to the cut ones.
...
\begin{proof}
  ...
  most significant case to consider is ef{rule:call}. Let $\vec{c}$ denote the
  ...
  Let $\alt = a_1, \dots, a_s, \dots, a_n$, where $a_s$ is an alternative that
