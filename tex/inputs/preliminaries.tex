...
%
\begin{align}
  tm     & ::= \cut \mid \predVar\ \vecL{tm} \mid tm = tm \mid \lambda x.tm \label{eq:tm} \\
  clause & ::= \clauseCmd{\predVar}{\vecL{tm}}{\vecL{tm}}                  \label{eq:cl}  \\
...
we prefer to have a special case for it. Finally we have lambda-abstractions,
where the notation $\lambda x.tm$ is the abstraction of the variable $x$ inside
$tm$.
...
A program is a mapping from predicate names to clauses. These clauses are
...
$$
  ...
  \Bigg[\bigg(\subst,
    \Big(
    \big[(\prog, i_x = i_x', \alt) \mid x \in \text{len }t\big] @
    \big[(\prog, g, \alt) \mid g \in bs\big]
    \Big)\bigg) \mid \clauseCmd{\pred}{\vec{t'}}{bs} \in \prog\ \pred\Bigg]
$$
...
We can remark that the terms accepted by the interpreter are terms in the
canonical form, i.e. no lambda abstraction can appear in the head of a
application. Moreover, we intentionally keep out from \cref{fig:basic-interp}
application with head being variables. This essentially means that variables
are never evaluated, in this first part of the paper.  Variables will play an
important role
in \cref{sec:vars}.
...
\begin{definition}[Deterministacy (\isdet)]
  Given a program \prog, a predicate \pred, and a list of terms $\vec{t}$,
  \isdet\ is defined as follows:
  $$\forall a\ \subst, \isDetCmd{\vec{t}}$$
\end{definition}
...
\begin{definition}[Mutually-exclusive heads (\mutExclHeads)]
  ...
  $c_2 = \clauseCmd{\pred}{\vec{u}}{bo_2}$ we have
  \begin{align*}
    \forall \subst \exists n, \lnot (\exists \subst['], \unifyCmd{(\vec{t}_i)_n}{(\vec{u}_i)_n}{\subst}{\subst[']})
  \end{align*}
  \vspace{-2em}
  \label{def:mut-excl-head}
\end{definition}

That is, two clauses have mutually-exclusive heads if it does exists an
...
\begin{lemma}[Mutually-exclusive heads property]
  Given two clauses $c_1 = \clauseCmd{\predVar}{\vec{t}}{b_1}$ and $c_2 = \clauseCmd{\predVar}{\vec{u}}{b_2}$ then
  % forall vector of \textit{ground} terms $\vec{v}$,
  $$\mutExclHeads\ c_1\ c_2 \to \forall \subst_1\ \subst_2, \lnot (\exists \vec{v},
    \ground\ \vec{v} \to \unifyCmd{\vec{v}}{\vec{t}_i}{\EmptySubst}{\subst_1} \land
    \unifyCmd{\vec{v}}{\vec{u}_i}{\EmptySubst}{\subst_2})$$
  \vspace{-2em}
  \label{th:mut-excl-head}
\end{lemma}

\begin{proof}
  By contradiction, let $\vec{v}$ be a term unifying with both $\vec{t}_i$ and $\vec{u}_i$. This
  ...
  $\subst_1\ \vec{t}_i = \subst_2\ \vec{u}_i$. However, due to $\mutExclHeads\ c_1\ c_2$,
...
\begin{definition}[Mutual-exclusion (\mutExcl)]
  Given a program \prog, for all deterministic predicate \pred in \prog,
  the clauses in \pred\
...
\begin{lemma}[Mutually-exclusion property]
  Forall program \prog, such that $\mutExcl\ \prog$, and predicate \pred, at
  most one clause can be successfully applied fron any call to \pred with ground
  inputs.
  \label{th:mut-excl}
\end{lemma}
...
\begin{definition}[Deterministic term application (\detAtom)]
  A term application $\pred\ \vec{t}$ is if $t$ is a deterministic predicate.
\end{definition}

\begin{definition}[Deterministic premises (\detPrem)]
  Let \prog be a program and \pred be a deterministic predicate, then let
...
\begin{definition}[Determinacy checking (\detCheck)]
  For any program \prog and any deterministic predicate \pred,
  $$\mutExcl\ \prog\ \pred \land \detPrem\ \prog\ \pred$$
\end{definition}

\begin{theorem}[Determinacy checking property]
  Given a program \prog and a deterministic predicate \pred
  $$\wellModed\ \prog \to \detCheck\ \prog \to \forall \vec{t}, \ground\ \vec{t}_i\ \to \isdet\ \prog\ \pred\ \vec{t}$$
  \vspace{-20pt}
  \label{th:det}
\end{theorem}
