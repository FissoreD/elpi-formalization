...
and \texttt{X} for unification variable names. We write $\vecL{t}$ 
...
$[e|x \in \vec{x}]$ for list comprehension and $[e\ \mathrm{if}\ p|x \in \vec{x}]$ 
for list filtering.
...
We say that $\vars\ t \subset \texttt{X}$ is the set
of variables occurring in $t$. When $\vars\ t = \emptyset$ we way that
$t$ is \ground.
A substitution $\subst : \texttt{X} \to tm$ is a mapping from unification variables
to terms. 
...
\begin{proof}
  ...
  $\dom\ \subst_1 \cap \dom\ \subst_2 = \emptyset$,
  $\vars\ t_d \cap \vars\ u_d = \emptyset$ and since $\vars\ v = \emptyset$.
\end{proof}
...
called \emph{semidet} in \cite{1996henderson}. % (or that leave choice points that certainly fail ~\ref{def:is-func}).
...
We augment the grammar for types as follows:
...
\begin{definition}[Index-cut erasure $|\cdot|$]
  ...
  $|a| = (\subst,\ [ (\prog^!,\ g,\ \vecL{|al|})\ \mathrm{if}\ g \not\in I^! \ \mid\ (\prog^!,\ g,\ al) \in \vecL{gl}])$
\end{definition}
...
allternatively P! amounts to a semantics with an indexing that is as deep
as head split.

\paragraph{digression on inference vs checking}
...
\paragraph{digression on our definition}

This choice makes the definition quite strict but with a better computational
behavior, and in order to satisfy it we will use the semantics to justify some
optimizations to programs, such as inserting cuts (or better once)
that are not semantically needed since they result is observationally equal
programs. One motivation is that in our use case fast failure is important
and the absence of potential backtracking makes the behavior of code easier
to predict. The code above can have a very bad complexity if \texttt{mem}
is defined without a cut. In particular its complexity is not the sum
of the two premises, but rather their product. By flagging \texttt{mem} as
a function the static checker forces mem to contain the cut, or equivalently
its call to behave computationally to \texttt{once(mem 1 L)}. In this case
the complexity is linear in L.
