...
\begin{coqcode}
  ...
  Inductive atom := 
    | Call (p:pn) (i:tm) (o:tm) : atom  (*binary predicates taking integers*)
    | Cut : atom                        (*the cut operator*)
    | Unify (t1:tm) (t2:tm) : atom      (*unification between t1 and t2*)
...
The \textit{unify rule} (\ruleUnif) is applied when a unification $t_1 = t_2$
...
The \textit{cut rule} (\ruleBang) refers to goals where the first goal has the
...
The combination of the \ruleCall and \ruleBang rules is crucial for replicating
...
\begin{proof}
  ...
  \ruleCall when the heads $c_1$ and $c_2$ unify both with the current goal. If
...
\begin{definition}[Deterministic atom (\detAtom)]
  A atom is deterministic if it is a call deterministic-annotation predicate.
\end{definition}
...
\begin{definition}[Deterministic premises (\detPrem)]
  ...
  clause $c_i$,
  $$(\exists j, a_j = \cut \to \forall k, k > j \to \detAtom\ a_k) \lor (\forall j, \detAtom\ a_j)$$
  \vspace{-2em}
  \label{def:det-prem}
\end{definition}
...
\begin{theorem}[Determinacy checking property]
  Given a program \prog and a predicat \pred
  \begin{align*}
    &\wellModed\ \prog \to \detCheck\ \prog\ \pred \to \\
    &\forall i\ o, \ground i\ \to \runCmd{[\goalCmd{\prog}{\callCmd{p}{i}{o}}{\EmptyList}]}{\EmptyList}{\EmptySubst}{a}{\subst} \to a = \EmptyList    
  \end{align*}
  \vspace{-20pt}
  \label{th:det}
\end{theorem}
...
\begin{proof}
  ...
  interesting case is the \ruleCall. Due to \detCheck\ \prog \pred, we know, by
