...
%
%
A goal $\g = \goalCmd{\prog}{g}{\alt}$ is a triple made of a program \prog,
an atom $g$ and a list of alternatives \alt that we refer to as cut-to alternatives.
An alternative is a pair $(\subst, \vecL{\g})$ where \subst is a substitution
and $\vecL{\g}$ a list of goals. A substitution \subst is a mapping
...
$[e|x \in \vec{x}]$ for list comprehension.
...
\begin{figure}[!h]
  ...
  \ruleCallM{1}
  $$
  \mathcal{F}(\prog, \pred, \vec{t}, \g, \subst, \alt) :=
  \bigg[
    %\bigg(
    \subst,
    %\Big(
    \big[\goalCmd \prog (t = u) ~\alt \mid (t,u) \in \vecL{t,u}\big] @
    \big[\goalCmd \prog g ~\alt \mid g \in \vecL{g}\big] @ ~ \g
    %\Big)
    %\bigg)
    ~\bigg\rvert~
...
This rule goes hand in hand with~\ref{rule:call} that stores in the subgoals
the alternatives before calling the predicate. Indeed the function $\mathcal{F}$
creates a list of new alternatives, the former is directly evaluated while
