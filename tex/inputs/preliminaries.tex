...
from unification variables \texttt{X} to terms $tm$.\\
...
An initial query looks like \runCmd{[\goalCmd{\prog}{\pred\ \vecL{t}}{\EmptyList}]}{\EmptyList}{\EmptyList}{\_}{\_}
...
ensures that at call time inputs are ground and that the outputs are bound
to ground terms when the call ends. In turn a ground input is necessary
...
\begin{definition}[Mutually-exclusive heads (\mutExclHeads)]
  ...
  $c_1 = \clauseCmd{\pred}{t_1 \ldots t_n}{bo_1}$ and
  $c_2 = \clauseCmd{\pred}{u_1 \ldots u_n}{bo_2}$.
  Let $\ctx\ \pred = \dtype{\pred}{i_1 \ldots i_k}{\_}$\todo{!!}
  \begin{align*}
    \exists d \leq k\ s.t.\ \forall \subst \ \neg \exists\ \subst['] s.t.\ \unifyCmd{\vec{t}_d}{\vec{u}_d}{\subst}{\subst[']}
  \end{align*}
  \vspace{-2em}
...
its discriminating argument $d$\todo{d is a pos} to be ground. The reasoning goes as follows.
...
\begin{lemma}[Mutually-exclusive heads property]
  ...
  $$\hyperref[def:mut-excl-head]{\mutExclHeads}\ \ctx\ c_1\ c_2 \to \forall \subst_1\ \subst_2, \lnot (\exists \vec{v},
