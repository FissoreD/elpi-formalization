...
variables, constants or compound terms. A compound term is a predicate name
applied to two terms. The type of predicates is \\\elpiIn{tm -> tm -> prop}
where \elpiIn{tm} is the type of terms. The first argument of a predicate is
...
constants are indicated with lower case letters.
...
\texttt{unify} procedure. The notation \unifyCmd{t_1}{t_2}{\subst}{\subst[']} represents
the unification between the terms $t_1$ and $t_2$ from an intial substitution \subst
to a final substition \subst[']. A substitution is a mapping from variable name to its
...
It returns a new term where al variables in $t$ have been recursively been
...
sometimes it is also needed to express directed relations, where arguments are
...
the clauses implementing a determinate predicate respects this condition.
...
predicates annotated (or inferred) as determinate. Under the hypothesis to work
with program statically mode-checked, a necessary condition for a predicate
predicate to be determinate is that no two clauses can be applied successfully
...
\begin{definition}[Mutual-exclusion on heads]
  \begin{coqcode}
    Definition ~\customlabel{mutexcl}{\texttt{mutual\_exclusive}}\prog \pred\!\!~:
      ~$\forall$~ i (HG: ground i), 
        ~$!\exists$~ i' o bo, (~\clauseCmd{\pred}{\texttt{i' o}}{\texttt{bo}}~) \in ~\prog~p -> unify i i'
  \end{coqcode}   
  \label{def:mut-excl} 
\end{definition}
...
\begin{definition}[Determinate premises]
  Let \prog be a program and \pred a determinate-annotated predicate, then let
...
every determinate predicate not passing the determinacy check,
can be rewritten in a equivalent way
so that it is no more classified as non-determinate.

\begin{coqcode}
Definition ~\customlabel{detpred}{\texttt{determinate\_pred}}~(~\prog~: prog) (p: pn) :=
...
\begin{theorem}
  ...
  \begin{coqcode}
    Theorem ~\customlabel{is_detpred}{\texttt{determinate\_pred\_prop}}~(~\prog~: prog) (p: pn) :=
      forall (HM : well_moded ~\prog\!\!~) (HD: det_check ~\prog\!\!~),
          determinate_pred ~\prog~p
  \end{coqcode}    
\end{theorem}
