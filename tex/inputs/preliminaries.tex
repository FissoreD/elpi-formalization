...
%
\begin{align}
  tm     & ::= \cut \mid \predVar\ \vec{tm} \mid tm = tm \mid \lambda x.tm \label{eq:tm} \\
  clause & ::= \clauseCmd{\predVar}{\vec{tm}}{\vec{tm}}                  \label{eq:cl}   \\
  goal   & ::= \goalCmd{\prog}{tm}{\vec{alt}}                                                            \\
  alt    & ::= \texttt{subst} * \vec{goal}
\end{align}
...
A goal is a triple made of a program \prog, a term
...
the goal. A query is a list of goals (noted \g in the following) in conjunction, whereas alternatives
 (noted \alt in the following) represent a disjunction of goals.
% In particular,
...
%
$$run : goal \to \vec{alt} \to subst \to (\vec{alt} * subst)$$
%
