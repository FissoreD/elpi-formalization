...
We are interested in identifying predicates that leave no choice points~\ref{def:is-ded}
called \emph{semidet} in \cite{1996henderson},
(or that leave choice points that certainly fail ~\ref{def:is-func}).
...
\textcolor{red}{\textbf{TODO: \cite{nakamura1986} $\to$ differenza tra program logically deterministic
vs program operationally deterministic}}
...
\end{definition}
\begin{definition}[(Semi-)determinacy (\isdet)]\label{def:is-det}
  Given a program \prog and a term $t$,
  \isdet\ is defined as follows:
