...
Since programs can be encoded into atoms by asserting all clauses dynamically
atop the empty program (that validates the invariant vacuously), we can directly
relate the static check with the deterministic execution of atoms.

\begin{corollary}[Static check entails Deterministic run]
  % Given a context \ctx and an atom $t$, then
  \vspace{-1em}
...
Take the following predicate and consider the two (alternative)
contexts.
...
The signature of \elpiIn{silly} is accepted only
if \elpiIn{mem} is a function (the code on the right).
In this case the code of \elpiIn{silly} is operationally
equivalent to the following one where the worst-case complexity is clearly
the sum of its parts, rather than the product as it can be is \elpiIn{mem}
is allowed to give multiple times the same result (code on the left).

\begin{elpicode}
func do! (pred) ->.
do! P :- P, !.                   % encoding of if-then using Elpi's hard cut
