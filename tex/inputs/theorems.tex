...
predicates that are \emph{operationally deterministic}. Due to the lack of space, we leave the
...
%   \label{th:run-empty}
% \end{lemma}
The semantics of a signatures is given by recursion over the signature as follows.
...
\newcommand{\iprog}[1][\prog]{\ensuremath{\texttt{checked}(#1)}\xspace}
...
Note that $\iprog[\EmptyList]$ holds.

\begin{lemma}[key]
  Given a context \ctx and an atom $t$, then\todo{mh}
  $$\forall \prog, \iprog \land \checkCmd{\ctx}{\detI}{t}{\_}{\detI}
