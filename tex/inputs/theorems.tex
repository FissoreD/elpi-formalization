...
\section{Theorems}\label{sec:thm}
...
Taking into account the interpreter we have depicted in \cref{fig:basic-interp},
we can give the definition of determinacy.
...
In order to guarantee that a predicate behaves determinalistically in a given
program, we need to ensure some sufficient conditions. We start by giving some
definition of terms that are .
...
\begin{definition}[Mutually-exclusive heads (\mutExclHd) at index $d$]
  ...
  If $\ctx\ \pred = \dtype{\relI}{i_1 \ldots i_k}{\_}$,
  then we say that
  $\mutExclHd\ c_1\ c_2\ d$ holds iff $1 \leq d \leq k \land\ \lnot(\unifiable\ u_d\ v_d)$.
\end{definition}

As shown in \cite{1989Warren,1996Somogy}, ensuring determinacy also requires
verifying that calls to deterministic predicates receive ground input terms.
This is achieved through a static mode checker, which tracks the flow of ground
variables within a clause. Additionally, mode checking plays a crucial role in
guaranteeing the mutual exclusiveness of rules: if two input arguments do not
unify, no ground term can unify with both. However, since Elpi performs matching
rather than full unification on input terms, the groundness condition is not
necessary to ensure that at most one clause applies. The following lemma states
this property.
...
\begin{lemma}[Mutually-exclusive heads with \match property]
  ...
  $$\forall d, \mutExclHd\ \ctx\ c_1\ c_2\ d \to \forall \subst,
    \lnot (\exists t\ \subst_1\ \subst_2\ s.t.\ 
    \matchCmd{t}{u_d}{\subst}{\subst_1} \land
    \matchCmd{t}{v_d}{\subst}{\subst_2})
...
Since, in Elpi, we have the hard cut operator, overlapping clauses may exists,
if we can ensure that at most one of them can be used on a given query, the
other being cut away.
...
\begin{definition}[Mutual-exclusion (\mutExcl)]
  ...
  \vspace{-0.5em}
  $$((\exists d, \mutExclHeads\ \ctx\ c_1\ c_2\ d) \lor \mutExclCut\ c_1\ c_2)$$
  \vspace{-2em}
  \label{def:mut-excl}
\end{definition}

Mutual exclusion ensures that at most one clause is applied on a query, however,
this is not sufficient to have the guarantee that no new choice point is left
after the execution of a clause. We need therefore the following two definitions.

\begin{definition}[Deterministic term application (\detAtom)]
  Given a context \ctx\ and a term $t$, $t$ is a deterministic
  call if $t = \pred\ \vec{u} \land \ctx\ \pred = \dtype{\detI}{\_}{\_}$
\end{definition}
...
\begin{definition}[Deterministic premises (\detPrem)]
  ...
  $$(\exists j, b_j = \cut \land (\forall k, k > j \to \detAtom\ \ctx\ b_k)) \lor (\forall j, \detAtom\ \ctx\ b_j)$$
  \vspace{-2em}
  \label{def:det-prem}
\end{definition}

Since in Elpi we can dynamically load local clauses thank to the \impl\
constructor, we need to ensure that the new inserted clause does not hinder the
determinacy of the predicate being extented.

\begin{definition}[Local clauses with bang (\locExcl)]
  Given a context \ctx\ and a program \prog, forall clause $c$ in \prog and
  forall predicate \pred, forall subterm $s$ of $c$ such that
  $s~=~\piImplCmd{x}{(\clauseCmd{\pred}{\vec{v}}{t_1, \dots, t_n})}{\_}$, if
  $\ctx\ \pred = \dtype{\detI}{\_}{\_}$ then
  $$\exists i, t_i = \cut \land (\forall k, k > i \to \detAtom\ t_i)$$
  \vspace{-2em}
\end{definition}

This means that all local clauses should have a cut if they are clauses of a
...
\begin{definition}[Check clauses (\checkc)]
  Given a context \ctx\ and program \prog, forall clause $c$ in \prog,
  $$\checkc\ \ctx\ c$$
\end{definition}

\begin{definition}[Good call (\checkk)]
  Given a context \ctx\, program \prog and a term $t$,
  if $t = \pred\ \vec{u}$ and $\ctx\ \pred = \dtype{\_}{d_1, \dots, d_n}{\_}$
  then 
  $$\inferCmdArr{\ctx}{t}{(\_, \bool)} \to \bool = \top$$ 
\end{definition}
...
\begin{theorem}[Determinacy checking]
  Given a context \ctx\ and a program \prog, then for any predicate \pred\ and
  any list of terms $\vec{t}$,
  \begin{align*}
    &\mutExcl\ \ctx\ \prog \to \detPrem\ \ctx\ \prog \to \\
     &\quad   \locExcl\ \ctx\ \prog\ \to \checkc\ \ctx\ \prog \to\\
     &\quad\quad       \checkk\ \ctx\ \prog\ (\pred\ \vec{t}) \to \ctx\ \pred = \dtype{\detI}{\_}{\_} \to\\
     &\quad\quad\quad \isdet\ \prog\ (\pred\ \vec{t})
  \end{align*}
  \vspace{-2em}
  \label{th:det2}
\end{theorem}
