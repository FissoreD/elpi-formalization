...
\begin{definition}[Good call (\goodCall)]
  Given a context \ctx and a term $\pred\ \vec{u}\ \vec{v}$, then $\pred$ is well called if
  $$\ctx \pred = \dtype{\_}{\func}{\_} \Rightarrow \forall d, \inferCmdArr{\ctx}{u_d}{(\func_d',\top)} \Rightarrow \func_d' \mySub \func_d$$
\end{definition}
...
\begin{proof}
  ...
  performed by \( \mathcal{F} \) cannot use both \( c \) and \( c_n \) to solve
