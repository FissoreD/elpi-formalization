...
The first static check of all determinacy analysis is about the mutual
exclusion between the heads of rules
...
In particular the groudness of input arguments at call time is essential
to statically check the mutual exclusiveness of two rules: if two
input arguments do not unify, there is no ground terms that can unify
with both.
Since Elpi performs matching the groundness condition is not required: if
the goal is a variable it will not match any non variable input argument,
and two variable input arguments are not \nUnify.

\begin{lemma}[Mutually-exclusive heads with \match property]
  Given a context \ctx and two clauses $c_1$ and $c_2$ with heads,
  respectively,  $\predVar\ \vec{t}$ and $\predVar\ \vec{u}$ then
  % forall vector of \textit{ground} terms $\vec{v}$,
  $$\mutExclHeads\ \ctx\ c_1\ c_2\ d \to
    \lnot (\exists \vec{v}\ \subst_1\ \subst_2\ s.t.\ 
    \matchCmd{v}{t_d}{\subst}{\subst_1} \land
    \matchCmd{v}{u_d}{\subst}{\subst_2})$$
  \vspace{-2em}
...
We want to allow for some overlapping when one rule cuts the other
...
\begin{theorem}[Determinacy checking]
  ...
  \begin{align*}
    &\detCheckHO\ \prog\ \pred \to \isdet\ \prog\ (\pred\ \vec{t})
  \end{align*}
  \vspace{-2em}
  \label{th:det2}
\end{theorem}
