...
We use $\EmptyList$ is the empty list and $x :: xs $ for prepending an element $x$
...
using the combinator \fold defined by these equations:
$$
\fold\ \_\ \_\ \bot = \bot
\qquad
\fold\ (x :: xs)\ f\ a = \fold\ xs\ f\ (f\ x\ a)
\qquad
\fold\ \EmptyList\ \_\ a = a
$$

We asume a set $P$ of predicate names, a set $K$ of term constructors (e.g. nil
and cons), a set \X of variables and by convention we use capitals for
...
all input arguments and then output arguments and we separather those with $\funsep$,
omitting the symbol when both are absent. \detI{} stands for deterministic (the
precise meaning is given in~\ref{sec:thm}) while \relI{} for non-deterministic.
...
\newcommand{\syntaxFig}{
  ...
  \begin{array}{rlr}
    ...
    sig & ::= [\detI \mid \relI]\ \vecL{ty} \funsep \vecL{ty} & \mathrm{predicate\ signature} \\
...
The user provides a context \ctx{} that assigns to each predicate symbol \pred{} a
signature $sig$. For example, looking back at section~\ref{sec:examples},
...
of free variables occurring in $t$. When $\vars\ t = \emptyset$ we way that
$t$ is \ground.
...
We write $\prog~\pred$ for the clauses for predicate \pred in $\mathcal{I}$
and $(x,h) + \prog$ to add an extra clause $h$ to $\mathcal{I}$ and
name $x$ to $\aleph$. We write $x \# \prog$ to find a name fresh in $\aleph$.
...
\newcommand{\runFig}{
  ...
  $$
  $$
  \mathcal{H}(\vecL{tu},\ ol,\ \subst) = \fold\ \vecL{tu_o}\ \unify\ (\fold\ \vecL{tu_i}\ \match\ \subst)
  $$
...
to the next alternative (backtracks to the most recent choice point) is
any, otherwise rule ~\ref{rule:callabort} terminates.
A peciluarity of Elpi is that input arguments are \emph{matched} against
...
Rule~\ref{piimpl} loads into the current program the new clause $h$
...
problems. The choice simplifies the formal threatment of determinacy we
provide in~\ref{sec:thm} and in particular it matches the \mutExclHeads condition.
