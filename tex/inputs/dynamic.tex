...
and \X for variable names. We write $\vecL{t}$ 
for $t_1 \ldots t_n$ (a possibly empty list of $t$s).
%and \texttt{c} for data types names.
In the syntax we single out the cut operator since it has a dedicated treatment
in the operational semantics. All other constructs are seen as predicate calls,
i.e. the usual $t_1 = t_2$ is encoded as $eq~t_1~t_2$ for a predicate $eq$
with a single clause $eq~X~X$.
  ...
  \begin{array}{c|c}
    syntax & types \\\hline
      \begin{array}{rl}
        P & ::= p, q, \ldots \\
        K & ::= c, f, \ldots \\
        \X & ::= \mathrm{X}, \mathrm{Y}, \ldots \\
        \A & ::= \cut \mid P\ \vecL{\T} \mid \X\ \vecL{\T} \mid \piImplCmd[ty]{\X}{\CL}{\A} \!\!\!\!\label{eq:atom} \\ % \mid \T = \T \label{eq:atom} \\
        \T & ::= K\ \vecL{\T} \mid P \ \vecL{\T} \mid \X\ \vecL{\T} \mid \lambda \X\!\!: ty \bs\ \T\label{eq:tm} \\
        \CL & ::= \clauseCmd{P}{\vecL{\T}}{\vecL{\A}} \nonumber %\label{eq:cl} 
      \end{array} 
      \quad
      &
      \quad
      \begin{array}{rl}
        \V & ::= \mathrm{A}, \mathrm{B}, \ldots \\
        \!\!\!sig & ::= [\detI \mid \relI]\ \vecL{ty} \funsep \vecL{ty} \ \label{eq:ty} \\
        ty   & ::= \texttt{exp} \mid sig \mid \V \\ %\texttt{c}\ data^\ast \label{eq:data}
        ~ & ~ \\
        ~ & ~ \\
        ~ & ~ 
      \end{array}  
...
all terms \T have type \texttt{exp},
and a context \ctx{} assigns to each predicate symbol \pred{} a predicate signature $sig$.
...
\fold\ \_\ \_\ \bot = \bot
\quad
\fold\ ((t,u) :: l)\ f\ x = \fold\ l\ f\ (f\ t\ u\ x)
\quad
\fold\ \EmptyList\ \_\ x = x
$$
...
such that if $\matchCmd{t_1}{t_2}{\subst}{\subst[']} \not= \bot$ then $\subst['] t_1 = \subst['] t_2$
...
i.e. \match{} does not assign variables in $t$ but only in the term $p$ that acts as a pattern.
...
the one given in~\cite{1990Vink} extended to HO\todo{chiarire}. The choice to use operational semantics
...
\subsection{Digression on  the eagher H}
