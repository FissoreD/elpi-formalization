...
then \subst[']  is the most general extension of  $\subst$
such that $\subst t = \subst['] p$ and $\subst[']t = \subst t$ and $\subst[']$ ,
...
\newcommand{\runFig}{
  ...
  $$
  \!\! \mathcal{F}(\prog, \pred\ \vecL{t}, gl, \subst, a) :=
  \bigg[
    %\bigg(
    \subst['],
    %\Big(
    %\underbrace{
      \big(\big[(\prog, g, a) \mid g \in \vecL{g}\big] @\ gl\big)
    %}_{\mathrm{premises}} @ ~ gl
    \ \kIF\ \mathcal{H}(\vecL{tu}, \subst) = \subst['] \not= \bot
    %\Big)
    %\bigg)
    ~\bigg\rvert~
    (\clauseCmd{\pred}{\vecL{u}}{\vecL{g}}) \in \prog\ \pred
    \bigg]
  $$
  $$
