...
\newcommand{\syntaxFig}{
  ...
  \begin{array}{rlr}
    ...
    \A & ::= \cut \mid \CALL\ \vecL{\T} \mid \piImplCmd[\texttt{exp}]{\X}{\CL}{\A} & \mathrm{atom} \\
    \T & ::= K\ \vecL{\T} \mid \CALL\ \vecL{\T} \mid \lambda \X\!\!: ty \bs\ \T & \mathrm{term}\\
    ...
    ty   & ::= \texttt{exp} \mid \func \mid \V & \mathrm{type} \\
  \end{array}
  $$
\end{subfigure}
}
...
The operational semantisc we present is essentially a big-step version of
the one given in~\cite{1990Vink} extended to the higher-order
constructs of $\lambda$Prolog and the input-matching behavior
specific to Elpi.
...
rather than a denotational one (as in~\cite{2011king}) stems from our
preference for maintaining a concrete representation of the current choice
points and is a prerequisite for the precise notion of
function we pick in section~\ref{sec:thm}.
