...
The non terminal \CALL groups symbols ($c,d,\ldots$) that can be called: either predicate names
or variables.
...
Looking back at the examples in Section~\ref{sec:examples}
we have that
$\ctx~\texttt{likes} = \dtype{\relI}{\expI}{\expI}$,
that
$\ctx~\texttt{likes!} = \dtype{\detI}{\expI}{\expI}$,
and also $\ctx\ \texttt{true} = \relI$.

In the syntax of atoms \ATOM we single out the \cut operator since it has a
dedicated treatment in the operational semantics. We also glue together the
\elpiIn{pi} and \elpiIn{=>} in the same grammar production for simplicity.
Terms include predicate calls, functor applications and $\lambda$-abstraction.
