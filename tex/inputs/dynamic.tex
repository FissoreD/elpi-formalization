...
dedicated treatment in the operational semantics. We note $\Ainst \in \A$.
...
$[e|x \in \vec{x}]$ for list comprehension and $[e\ \kIF\ p|x \in \vec{x}]$ 
...
$f : A \to B \to C$ and $\vecL{tu} : list (A*B)$.
...
the function $\mathcal{F}$. This function is in charge of creating the\todo{dire che F le restituisce in ordine}
