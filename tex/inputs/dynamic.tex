...
We assume a set $P$ of predicate names, %\todo{pradicate with piimpl as arg?}
a set $K$ of functors (term constructors),
a set of type variables \V
and a set of term variables \X.
By convention we use capitals for
unification variables and small letters for bound variables.
\CALL is the union of $P$ and \X.

The programmer provides a context \ctx{} that assigns a
signature $\func$ to each predicate. By convention predicates take
...
(i.e. $\func$ degenerates to \F).
The determinacy marker \detI{} stands for deterministic (the
precise meaning is given in~\cref{sec:thm}) while
\relI{} for non-deterministic. Be convention
we use \func, \func['], $\func[_1] \ldots$ to denote signatures
or determinacy markers.
Looking back at section~\ref{sec:examples}
we have that
...
In the syntax of atoms \A we single out the cut operator since it has a
dedicated treatment in the operational semantics.
The usual =/2 operator can be
encoded as a regular predicate \texttt{eq} with a single clause
\elpiIn{(eq X X :- .)} and signature
$\ctx\ \texttt{eq} = \dtype{\detI}{}{A\ A}$: a deterministic predicate\todo{should add var to static check}
with two outputs of the same type $A$.\todo{input,output?}

We write \(\vecL{t}\) to denote \(t_1, \ldots, t_n\), representing a (possibly
empty) list of \(t\)s. Similarly, we write \(\vecL{tu}\) to denote the
...
$\ctx\ \pred = \dtype{\func}{{ty}_1 \ldots {ty}_k~}{~{ty}_{k+1} \ldots {ty}_n}$ we
write $\vecL{t_i}$ for the input arguments ${t}_1 \ldots {t}_k$
and $\vecL{t_o}$ for the output arguments ${t}_{k+1} \ldots {t}_n$.

$\lambda$ and \texttt{pi} are binders in the $\lambda$-calculus sense. In our syntax
we impose the user to explicitly type binders and the body of $\lambda$-abstractions,
the body is never a paritial application, i.e. $lambda$-term are always in the $\eta$-long form.
Note that this piece of information can be inferred by the type-checker,
but we prefer to keep our presentation simple. We write
$t[x/y]$ for the usual, capture avoiding, operation of replacing the variable $y$
with $x$ inside $t$.
We say that $\vars\ t \subseteq \mathcal{P}(\X)$\todo{\prog is program?} is the set
...
\newcommand{\syntaxFig}{
  ...
  \begin{array}{rlr}
    ...
    \CALL & ::= P \mid  \X & \mathrm{call\ head}\\
    \F & ::= \detI \mid \relI & \mathrm{determinacy} \\
    \A & ::= \cut \mid \CALL\ \vecL{\T} \mid \piImplCmd[ty]{\X}{\CL}{\A} & \mathrm{atom} \\
    \T & ::= K\ \vecL{\T} \mid \CALL\ \vecL{\T} \mid \lambda \X\!\!: ty \bs\ \T\!\!: \base & \mathrm{term}\\
    \CL & ::= \clauseCmd{P}{\vecL{\T}}{\vecL{\A}} & \mathrm{clause} \\
    \V & ::= \mathrm{A}, \mathrm{B}, \ldots & \mathrm{type\ variable} \\
    \func & ::= \F\ \vecL{ty} \funsep \vecL{ty} & \mathrm{predicate\ signature} \\
    \base & ::= \expI \mid \F & \mathrm{base\ determinacy} \\
    ty   & ::= \base \mid \func \mid \V & \mathrm{type} 
  \end{array}
  $$
\end{subfigure}
}
...
A substitution $\subst \in \Sigma $ is a mapping from unification variables
\X to terms $tm$. 
...
We assume a function $\unify$ of type $tm \times tm \times \Sigma \to \Sigma + \B$
such that if $\unifyCmd{t_1}{t_2}{\subst}{\subst[']} \not= \bot$ then $\subst['] t_1 = \subst['] t_2$
and $\subst[']$ is the most general extension of $\subst$\todo{extension=unifier?}
as in~\cite{1991miller-pf}.
We assume a function $\match$ of type $tm \times tm \times \Sigma \to \Sigma + \B$
...
i.e. \match{} does not assign variables in $t$ but only in $p$ that acts as a pattern.\todo{consumer?}
...
a single node \texttt{exp} for regular data or \V{} for polymorphic data.
%\todo{what about \V?}
...
for list filtering. We use the list processing combinators \fold and \map defined by the following equations:
$$
\begin{array}{lll}
...
\noindent
By a slight abuse of notation, we assume that the currification operation for
the function \( f \) is applied whenever necessary. For example, $\map\ f\
\vecL{tu} = \map\ (\lambda x.f\ (\text{fst}\ x)\ (\text{snd}\ x))\ \vecL{tu}$ if
$f : A \to B \to C$ and $A$ is not the product type.
...
We write $x \# \prog$ to find a name fresh in $\aleph$.
We write $\prog~\pred$ for the clauses of predicate \pred in $\mathcal{I}$
and $(x,h) + \prog$ to add
the name $x$ to $\aleph$ and the extra clause $h$ on top of $\mathcal{I}$,
i.e. $h$ has the histest priority in the program.
...
is a triple made of a program, an atom $g$ and a list of
...
The operational semantics of Elpi is given by a relation
$\run{} \subseteq \alt \times \vecL{\alt} \times (\vecL{\alt} \times \Sigma ~+~ \bot)$.
...
predicate but also eliminates alternative clauses for subgoals that appear
earlier in the clause premises. 
...
points. This choice is also essential for the precise notion of function we
define in Section~\ref{sec:thm}.
