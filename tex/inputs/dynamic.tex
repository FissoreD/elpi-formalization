...
rule out predicates inside data (like lists or predicates).
...
and we collapse all non-predicate types to a single node \texttt{exp}. 
...
substitution s.t. $\dom\ \EmptySubst = \emptyset$.
...
the name $x$ to $\aleph$ and prepend the extra clause $h$  to $\mathcal{I}$
($h$ has the highest priority in the program).
...
The operational, big step, semantics of Elpi is given by a function
