...
The non terminal \CALL groups symbols that can be called: either predicate names or variables.

The programmer provides a context \ctx{} that assigns a
signature $\SIGN$ to each predicate name. By convention predicates take
...
dedicated treatment in the operational semantics, and we glue together the
\elpiIn{pi} and \elpiIn{=>} in the same grammar production for simplicity.
Terms include predicate calls, functor applications and the binder $\lambda$.
For simplicity we impose the body of $\lambda$-abstractions to be explicitly typed
with a base type (if the body is a predicate, then it must be fully applied).
Moreover, as this paper does not
really focus on type checking
(the interested reader can refer to~\cite{1992nadathur}) we collapse all
non-predicate types to a single node \texttt{exp}. 
We also rule out predicates inside data
% such as list of predicates, 
...
and $\vecL{t_o}$ for the output ones ${t}_{k+1} \ldots {t}_n$.
We write
...
\newcommand{\syntaxFig}{
  ...
  \begin{array}{rlr}
    ...
    \DATA & ::= c, f, \ldots & \mathrm{data\ constructor}\\
    \VAR & ::= \mathrm{X}, \mathrm{Y} \ldots x, y, \ldots & \mathrm{variable}\\
    \CALLHD & ::= \PRED \mid  \VAR & \mathrm{callable}\\
    \ATOM& ::= \cut \mid \CALLHD\ \vecL{\TERM} \mid \piImplCmd[\TYPE]{\VAR}{\CLAUSE}{\ATOM} & \mathrm{atom} \\
    ...
    \SIGN & ::= \F\ \vecL{\TYPE} \funsep \vecL{\TYPE} & \mathrm{predicate\ signature} \\
...
We omit the conversion bewteen the (isomorphic) types $A \times B \to C$
and $A \to B \to C$, e.g. we may write $\map\ f\ \vecL{tu}$ instead of $\map\ (\lambda x.f\ (\text{fst}\ x)\ (\text{snd}\ x))\ \vecL{tu}$.


We assume a function $\unify$ of type $\TERM \times \TERM \times \Sigma \to \Sigma + \B$
...
We assume a function $\match$ of type $\TERM \times \TERM \times \Sigma \to \Sigma + \B$
