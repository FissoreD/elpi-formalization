...
In \cref{fig:static-check} we present the derivation system for the the static
checker for determinacy. \ctx\ stands for a context and is a mapping from
variable and predicate names to their determinacy signature. Variables
signatures are added during the checking operation. In the system we use some
auxiliary functions. 1) \infer which takes a context and a term and returns the
determinacy of the term. The entry point of the
checker is \ref{rule:check-clause}.
