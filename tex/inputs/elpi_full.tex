...
allowing to check what is the determinacy of a term. This check relation is not
...
We note \ctx\ to be a mapping from variable names to their
determinacy type (dtype). Each variable \predVar\ appearing in a term $t$ has a
dtype in \ctx\ since either \predVar\ is the name of a predicate and its dtype
can be retrived from its signature, or \predVar\ is a quantified variable and
its dtype is derived from the type inferred by the type checker. In the latter
case, the typechecker infers the worst determinacy from the type, i.e. if
...
The determinacy checker (the \checkk) for terms takes a context \ctx\, an expected determinacy
\func\ and a term $t$. It returns the inferred determinacy of $t$ and an updated
version of \ctx[']\ where the values of some quantified variables in \ctx\ may
have been updated. The simplest rule is \ref{rule:check-cut}: for any \ctx and
and expected determinacy \func, the determinacy of \cut\ is \detI with an
unchanged context.
...
We can now present the static checker for determinacy. It is depicted in
\cref{fig:static-check} and uses the rules in
\cref{fig:det-inference,fig:det-assume} as auxiliary relations.

We assume that the typechecker annotates each term with its corresponding type.
This association between terms and their determinacy types (dtypes) is stored in
a mapping called \ctx.

When the typechecker encounters a variable, its dtype is approximated to the
maximum dtype allowed for the given type. For example, in the query
\elpiIn{X 3}, the variable \elpiIn{X} is assigned the determinacy \relI\
[\expI]\ \EmptyList, representing the most general approximation for its type.

\paragraph{Infer}
The \infer\ relation in \cref{fig:det-inference} aims to produce the
dtype of a term (potentially applied) given a
mapping \ctx. Rule \ref{rule:infer-hd} handles variables or predicate names by
retrieving their determinacy from the context.

Rules \ref{rule:infer-cut} and \ref{rule:infer-unif} are straightforward: both
the \cut\ operation and unification are considered deterministic terms.
...
Finally, rule \ref{rule:infer-app} addresses term applications. It verifies that
the input’s determinacy is less than or equal to the expected
determinacy. If this condition holds, the determinacy of the term corresponds to
the determinacy of its head. For simplicity, we focus on binary applications,
such that the first and second parameters are an input and an output.

\paragraph{Assume}
The \assume\ relation takes a mapping \ctx\, a determinacy type \func\, and a
term $t$. The output is a new mapping \ctx[']\ where the variables in
$t$ have been updated wrt \func.

The basic case of this derivation system is the rule \ref{rule:assume-hd} which
takes a variable or predicate name \predVar\ as input. It updates by setting the
\predVar\ to the given determinacy if \predVar\ is a variable or if it is a
predicate with the same determinacy in \ctx.

Rules \ref{rule:assume-cut} and \ref{rule:assume-unif} are intuitive: no change
has to be performed in \ctx.

Rule \ref{rule:assume-piimpl} adds in the local context the type of the binder
$x$ and keeps calling \assume\ in the term under the implication.

\textbf{TODO: the variable x should be removed from \ctx otherwise it is returned}

Rule \ref{rule:assume-app} is particularly interesting as it deals with the
application of a term to two arguments, $t_1$ and $t_2$, which we consider as
its input and output, respectively. Our goal is for this term to assume it has
the determinacy \func. This is valid if the inferred dtype of the head $t$ is
$\func[']\ [\func_i]\ [\func_o]$, such that $\func['] \mySub \func$. In other
words, the term we are assuming to be have a certain determinacy already
possesses the appropriate determinacy. The only requirement is that its input,
$t_1$, matches the determinacy expected by the function's signature in the input
position.


\paragraph{Static check}
In \cref{fig:static-check} we present the derivation system for the determinacy
static checker. The \checkc\ routine aims to verify that a clause respects the
signature, i.e. the output terms and its body have the expected determinacy type
whereas the \checkk\ routine takes a list of terms returning if their execution
is deterministic or relational and a new mapping from terms to their
determinacy.

\textbf{TODO: dire che la regola per $=_m$ è la stessa che per unif}
