...
We can now present the inference rules for determinacy (in
\cref{fig:det-inference}) and those for the static checker (in
\cref{fig:static-check}).

TODO: dire che lavoriamo con predicati e applicazioni binarie

Determinacy inference aims to determine the determinacy of a term given a
mapping \ctx\ from predicate names to their determinacy type. Rules
\ref{rule:infer-cut} and \ref{rule:infer-unif} are intuitive: the \cut\ and the
unification are deterministic predicate. Rule \ref{rule:infer-piimpl} works with
the \coqIn{PiImpl} constructor. It adds in \ctx\ a mapping going from the local
variable $x$ to the determinacy type deduced from the type of $x$. Finally
\ref{rule:infer-app} deals with terms applications. It checks that the input has
a determinacy which is \mySub\ then the exepected one. If it is the case the
determinacy of the term is the determinacy of its head. Otherwise, the predicate
is wrongly called and therefore the call will behaves has a relation.

From \infer\ we define three auxiliary functions: given a mapping \ctx\ and a
predicate name \predVar, if ``\inferCmdArr{\ctx}{\predVar}{D\ [D_i]\ [D_o]}''
then ``$\getF\ \ctx\ \predVar = D$'', ``$\getI\ \ctx\ \predVar = D_i$'' and
``$\getO\ \ctx\ \predVar = D_o$''.
