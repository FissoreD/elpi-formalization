...
(\cite{1991miller-pf}), noted \pf. This extension allows the declaration of
anonymous functions together with the possibility of dynamically modifying the
program by inserting fresh variables and clauses. $\lambda$-abstraction are
often used to represent higher-order abstract syntax (HOAS \cite{1988pfenning})
which is intensively used in meta-programming to represent terms of the
target language. For example, \elpi + CITE
...
we add three constructors to \coqIn{tm}:
\begin{coqcode}
  | Impl (c:clause) (b:tm)
  | Pi (x:vn) (b:tm)
  | Lam (x: vn) (b:tm)
...
The first constructor is noted with the syntax \implCmd{c}{b} and introduces
the clause \texttt{c} locally in the scope of an atom \texttt{b}, by convention
this new inserted clause has the highest priority wrt the clauses existing in
the scope of \coqIn{b}. The \coqIn{pi} constructor is noted \piCmd{x}{b}
introduces the fresh variable nominal variable \coqIn{x} in \coqIn{b}.
Finally the \coqIn{Lam} constructors
is used to define anonymous functions, the notation ``\lamCmd{x}{B}''
stands for the $\lambda$-abstraction binding a local variable $x$ in the scope of
$B$.
...
To account for the new constructors in the language, we extend our interpreter.
The updated rules are shown in \cref{fig:full-interp}.  
...
The notation $H + \ctx \stackrel{\mathrm{def}}{=} H + (\prog, \env)$ is the insertion of the clause \clause 
in the program \prog so that \clause has
the highest priority in \prog. The notation
$x \uplus \ctx \stackrel{\mathrm{def}}{=} H \uplus (\prog, \env)$
is the insertion of the local variable $x$ inside the set of local binders \env
provided that $x \notin \env$. The notation $B[x\bs T]$ produces a new terms
where all the free occurrences of $x$ in $B$ have been replaced with the term
$T$.
...
The \unify and \match procedures must now consider \env\ to perform \pf\
unification, which is required when a term is a local variable.

Finally the \ruleLam makes a $\beta$-reduction step and keep solving the goal.

\subsection{Determinacy checker with HOAS}

The insertion of the new rules make the determinacy checker to change
accordingly. In particular, the \ruleImpl\ inserts a new rule, this rule may
break \cref{def:mut-excl} and \ruleLam\ may create a $\beta$-reduced term which
may break \cref{def:det-prem}.

In order to address the first problem, we need to ensure that the dynamic
insertion of new clause $H$ in the program is such that $H$ successfully applies
on a goal then all the other clause fails. To ensure this propery, we check
that the body of each local clause has a \cut as a premise. This condition
is sufficient to make \cref{th:mut-excl} to work again.
...
deterministic and has a $\beta$-reduction after the last call, then the reduced
term is still a call to a deterministic predicate. This check can be performed
by looking to the 
