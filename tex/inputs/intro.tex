...
function and if the first list is ground. We want tho author of this code
to be able to ascribe a more precise signature to the code above, namely:
...
\noindent The first one is accepted since \elpiIn{likes!} is a function, hence also
...
introduce is expressive enough to encapsulate the desired ``tail cut'' behavior
into a predicate: The signature of \elpiIn{once}
...
meal Gs M :- map (once likes) Gs Ds. % ok, "once likes" is a function
\end{elpicode}
% meal Gs M :- map (g\d\once (likes g d)) Gs Ds. % ok, "once likes" is a function
...
meal Gs M :- id likes F, map Gs F M.     % nope
\end{elpicode}
...
is. Of the outputs are considered functional only if the predicate is
well called.
