...
a dialect of \(\lambda\)Prolog, well suited for manipulating syntax
...
particularly relevant in the context of the \emph{Rocq platform}, where code
...
% a determinacy analysis. 
While the literature offers numerous studies on determinacy analysis (e.g., \cite{1989Warren, 1996Somogy, 2011king}), none are directly
...
should be understood as the standard Prolog atom \elpiIn{map(F, L, R)}.
...
and one output. The first argument is a predicate with an input
...
we will give a more precise definition in \cref{sec:thm}.
...
considered to be a function for the analysis of the surrounding code.
...
likes! G D :- likes G D, !.       % commit to the first solution
...
The cut operator of Elpi lets one turn a relation into a function
...
\noindent In line 20 the output of \elpiIn{commit} is a function \elpiIn{F} that
...
is. The example at line 22 is rejected since \elpiIn{id} is miscalled: the outputs
...
is the local version of Prolog's \texttt{assert/1}).
...
\item can be applied to a pre-existing code bases since it tracks
