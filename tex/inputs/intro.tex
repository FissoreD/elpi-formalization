...
% is typically used to implement overloading~\cite{wadler1996-hs,sozeau2008-tc}
% where 
the solution to a query provides the formal meaning of the overloaded symbol
and it is of paramount importance that this solution is unique, ruling out ambiguities.
...
many works on the subject~(among them \cite{1989Warren,1996Somogy,2011king}), but none of these
works can be applied to Elpi due to its higher-order nature.
%, inherited from $\lambda$Prolog. %, 
% and its non-standard notion of input.
...
... % same guest may like more than one dish ...
...
hence it behaves like function.
...
\begin{elpicode}
kind tm type.                % type of $\lambda$-terms
...
clause is the second one that crosses the binder in \elpiIn{F} by performing two
operations:
1) it creates a fresh symbol \elpiIn{x} via the \elpiIn{pi} operator, and
2) it asserts a dynamic clause to copy \elpiIn{x} to itself via the \elpiIn{=>} operator.
