...
of type \elpiIn{A} and an output of type \elpi{B}, while the other two
arguments are respectively of type \elpiIn{list A} and \elpiIn{list B}.
The better signature we describe in this paper additionally states that
if the higher order argument is a function, then also \elpiIn{map} is.
While we will be more precise in section~\ref{sec:thm},
in the reasoning above ``function'' just means single-valued relation.
