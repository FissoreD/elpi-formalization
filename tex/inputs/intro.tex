...
As of typday Elpi comes with a quite standard type checker~\cite{1992nadathur} but
...
with cut that poses the bases for our definition of \emph{functional} and
paves the way to a mechanization of the determinacy checking algorithm.
...
\noindent The syntax ``\elpiIn{func name inputs -> outputs}'' asserts that name is
...
be added to an existing code base where \elpiIn{map} is potentially
...
In the examples below we give three (alternatives) 1-line
...
meal Gs M :- map likes! Gs Ds.     % a function
meal Gs M :- map likes Gs Ds, !.   % also a function
meal Gs M :- map likes Gs Ds.      % not really a function
\end{elpicode}

\noindent All three implementation validate the signature above, but only the
first two validate the one below.

\begin{elpicode}
func meal list guest -> list dish.
\end{elpicode}

\noindent The first one is accepted since \elpiIn{like!} is a function, hence also
\elpiIn{map likes! Gs Ds} is. The second implementation miscalls \elpiIn{map}
but then commits to the first solution, hence it is a function.
The third one is rejected since it miscalls \elpiIn{map} without
compensating for it.

The cut operator is the immediate way to impose functionality on a relation
by committing to its first result, as in the body of \elpiIn{like}, but it
not necessary to explicitly use it.\\
The language of predicate signatures we introduce is expressive enough to
encapsulate its behavior.
For example the signature of \elpiIn{once}
states that the higher order argument \elpiIn{P} can be any predicate, not
necessarily functional, but still \elpiIn{once P} acts as a function.

\begin{elpicode}
func once (pred) -> .
once P :- P, !.

meal Gs M :- map (once likes) Gs Ds. % ok
\end{elpicode}

\noindent  The determinacy analysis we
present tracks functionality of the outputs of (well) called predicates,
that can be themselves predicates.
    
\begin{elpicode}
func commit (pred) -> (func).
commit P (once P).

meal Gs M :- commit likes F, map Gs F M. % ok
\end{elpicode}

\noindent Here the output of \elpiIn{commit} is a function \elpiIn{F} that
in turn makes \elpiIn{map Gs F M} produce a single value for \elpiIn{M}. Moreover
\elpiIn{commit} is a function, so the whole body of \elpiIn{meal} also is.


The real business of Elpi is the manipulation of higher order data via the so
called $\lambda$-tree syntax (also known as HOAS~\cite{1988pfenning,Miller2018MechanizedMR}).
...
represents the first projection, where \elpiIn{x\_} is the built in
...
at run time: Since \elpiIn{x} is fresh it does not unify
