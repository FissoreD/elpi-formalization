...
\begin{definition}[Mutual exclusion \mutExclAll]
  ...
  b) $\forall h' = (\clauseCmd{p}{\vecL{u}}{\_}) \in \prog$
     there exists a $d \leq k$ such that $\forall \subst, \unifyCmd{t_d}{u_d}{\subst}{\bot}$ % \land \subst \neq \bot$
  % c) $\forall h' = (\clauseCmd{p}{\vecL{t'}}{\_}) \in \prog$ $\bigwedge \vecL{t_i = x} \bigwedge \vecL{t'_i \not = X}$.
  \label{def:mut-excl}
\end{definition}
...
\elpiIn{pi} quantified variable, hence different from any symbol that can
occur in the goal or in the other clauses.
Moreover we
point out that \ref{rule:call} \emph{matches} the input arguments
of the goal against the head of the clauses.
In turn this dispenses with the need for groudness of input
arguments, a prerequisite of most determinacy analysis algorithms
