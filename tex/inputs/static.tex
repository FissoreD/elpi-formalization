...
The \assume\ procedure is meant to strenghten the signature
...
strongest between the one in the context and the expected one. 
% For example,
% if \elpiIn{X = Y, X = commit likes F} with $\ctx\ \text{\elpiIn{Y}} = \relI$.
% The first unification updates the determinacy of \elpiIn{X} to \relI.
% The second unification strengthens \elpiIn{X} signature to \detI, since
% \elpiIn{commit likes F} is a function. This update is justified by the fact that
% at runtime, unification will succeed iff  
