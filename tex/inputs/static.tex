...
update the variable's signature to align with the post condition of the predicate
call.

We suppose the context \ctx is pre-loaded with weak signatures
for each variable: It is up to the determinacy analysis to \emph{assume}
...
Case b) addresses mutually exclusive heads.
We recall that rule \ref{rule:piimpl} generates a fresh symbol for the
\elpiIn{pi}-quantified variable, ensuring it is distinct from any symbol that can
appear in the goal or in other clauses.
...
Consider the following code performing the fusion of two calls to \elpiIn{map} (as in \cite{wadler1989}).

\begin{elpicodeln}[29]
  func compose (func A -> B), (func B -> C), A -> C.
  compose F G X Z :- F X Y, G Y Z.

  ~\PYG{k+kd}{typeabbrev}\PYG{+w}{ }\PYG{n+nf}{(flist A B)}\PYG{+w}{ }\PYG{k+kd}{(}\PYG{k+kd}{func}\PYG{+w}{ }\PYG{k+kt}{list}\PYG{+w}{ }\PYG{k+kt}{A}\PYG{+w}{ }\PYG{k+kd}{\PYGZhy{}\PYGZgt{}}\PYG{+w}{ }\PYG{k+kt}{list}\PYG{+w}{ }\PYG{k+kt}{B}\PYG{k+kd}{)}.\PYG{+w}{             }\PYG{c}{\PYGZpc{} type abbreviation}~
...
For brevity, we shorten the signature
\elpiIn{~\PYG{k+kd}{func}\PYG{+w}{ }\PYG{k+kt}{list}\PYG{+w}{ }\PYG{k+kt}{X}\PYG{+w}{ }\PYG{k+kMode}{\PYGZhy{}\PYGZgt{}}\PYG{+w}{ }\PYG{k+kt}{list}\PYG{+w}{ }\PYG{k+kt}{Y}~}
to $\func$
and we say $t \infunc$ when a term $t$ has signature stronger or equal to
$\func$.
% $\dtype{\detI}{\expI}{\expI}$.

Considering the work that \checkc performs line 34:
...
In turn, this requires \elpiIn{compose A B} $\in$
\elpiIn{~\PYG{k+kd}{func}\PYG{+w}{ }\PYG{k+kt}{X}\PYG{+w}{ }\PYG{k+kMode}{\PYGZhy{}\PYGZgt{}}\PYG{+w}{ }\PYG{k+kt}{Y}~}.
...
second clause extracts the sub-term \elpiIn{F} from it. The
in-depth analysis performed by \assume ensures the correctness of the
signature for arguments in \textit{input} positions of the outputs,
such as \elpiIn{F}.
