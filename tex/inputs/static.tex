...
We assume each rule to have been typechecked by an algorithm that
traverses all clauses and in particular all variables in \X\ that
...
to the variables the output of \maximize\ of its declared signature.
That function is defined by mutual recursion with its dual \minimize\ 
and in the definition below we denote one of them
with $f$ and $g$ the other one:
...
\begin{equation}
  f\ s =
  \begin{cases}
    \dtype{\relI}{(\vecL{g\ a})}{(\vecL{f\ b})} & \text{if } s = \dtype{\func}{a}{b} \\
    \expI                                                       & \text{if } s = \expI
  \end{cases}
  \label{eq:maximize}
\end{equation}
