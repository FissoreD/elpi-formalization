...
The checking performs dataflow analsysis. It stores in \ctx the predicate
signature of each variable and it uses it to test if it satisfies the
precondition of predicate call and possibly refines it to
the signature of a post condition a predicate call.
...
The \infer\ procedure is responsible for inferring the signature of a term. 
It returns a pair containing its signature and a boolean value
indicating whether all of its sub-terms representing a subterm which i predicate calls are correct. Since predicate calls are part
of the term syntax, they can also be miscalled.  
...
the variable signature must be the strongest between the one in the context and
the expected one.  
...
The \checkk\ procedure combines both \infer\ and \assume. Its purpose is to
