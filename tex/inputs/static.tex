...
It is one of our objectives that the determinacy checker should accept
...
extend the intuitive inclusion of mathematical functions
...
The checking process performs dataflow analysis. It stores the predicate
signature of each variable in \ctx\ and uses it to verify whether the variable
satisfies the precondition of a predicate call. Additionally, it may refine the
variable's signature to match the postcondition of the predicate call.

Refinements use the ``dual'' operators \(\minT\) and \(\maxT\).  
In their mutual definition below, we denote one of them as \( f \) and the other
as \( g \):\todo{f per max è sbagliata}
%
\begin{equation}
  f\ s_1\ s_2 =\ 
  \begin{cases}
    %\ \dtype{\detI}{(\vecL{g\ a\ c})}{(\vecL{f\ b\ d})} & \text{if } s_k = \dtype{\relI}{a}{b}\ \land\ s_{3-k} = \dtype{\detI}{c}{d} \\
    \ \dtype{\detI}{(\map\ g\ \vecL{ac})}{(\map\ f\ \vecL{bd})} & \text{if } s_k = \dtype{\relI}{\vec a}{\vec b}\ \land\ s_{3-k} = \dtype{\detI}{\vec c}{\vec d} \\
    %\ \dtype{D}{(\vecL{g\ a\ c})}{(\vecL{f\ b\ d})}     & \text{if } s_1 = \dtype{D}{a}{b}\ \land\ s_2 = \dtype{D}{c}{d}             \\
    \ \dtype{D}{(\map\ g\ \vecL{ac})}{(\map\ f\ \vecL{bd})}     & \text{if } s_1 = \dtype{D}{\vec a}{\vec b}\ \land\ s_2 = \dtype{D}{\vec c}{\vec d}             \\
...
%   \maxT\ f_1 f_2 =
%   \begin{cases}
%     \dtype{\detI}{(\vecL{\minT}\ a\ c)}{(\vecL{\maxT}\ b\ d)} & \text{if } f_k = \dtype{\relI}{a}{b} \land f_{3-k} = \dtype{\detI}{c}{d} \\
...
% We construct the transitive closure of \mySub to form a directed acyclic graph
% (DAG) of signatures, where an arc from \( s_1 \) to \( s_2 \) indicates that \(
% s_1 \mySub s_2 \). The \minT (respectively, \maxT) function between two
% signatures, if it exists, determines their lowest common predecessor
% (respectively, successor). We will later discuss the impact of these functions.
% Notably, the \minT relation plays a key role in \ref{rule:assume-hd}.

% To ensure the correctness of the static checker, each variable must be mapped to
% a signature in \ctx. The signature \( s \) assigned to a variable \( v \) is
% computed using the \maximize function, which derives a signature from the type
% \( ty \) inferred by the type checker. Specifically, \(\maximize\ s = s'\) is
% defined such that \(\forall s'', \maxT\ s\ s'' = s'\), meaning that each
% variable is assigned the weakest determinacy within its class. Due to space
% constraints, we omit the description of the \maximize function. However, it can
% be easily defined in a manner similar to the \maxT function.


% Below we define the functions \maximize and \minimize.

We assume each clause has been typechecked by an algorithm that
traverses terms including variables in \X\ that
represent predicates, e.g. the higher-order parameter of \elpiIn{map}.
Since we want to keep typecheck separated from determinacy checking,
the former identified \relI\ with \detI\ and in particular assigns
...
dual \minimize. In the definition below we denote one of them
with $f$ and $g$ its dual:
%
% $$
...
\begin{equation}
  f\ s =\ 
  \begin{cases}
    \ \dtype{\relI}{(\vecL{g\ a})}{(\vecL{f\ b})} & \text{if } s = \dtype{D}{a}{b} \\
    \ \expI                                                       & \text{if } s = \expI
  \end{cases}
  \label{eq:maximize}
\end{equation}
...
\checkk\ (\cref{fig:static-check}) if a rule matches a declared signature.\todo{in sig of deduce: \A and also tm}
%
$$\infer : \ctx \to \A \to sig \times \B$$
$$\dapp : \ctx \to \vecL{sig} \to \vecL{sig} \to \vecL{tm} \to \B \to sig \times sig \times \B$$
...
The \infer\ procedure is responsible for inferring the signature of a term. It
returns a pair containing its signature and a boolean value indicating whether
all of its sub-terms representing predicate calls are correct. Since predicate
...
The rule \ref{rule:infer-app}, combined with the \dapp\ procedure, infers the signature
of a term application when the head of the term is a variable or a predicate.
This routine also accounts for partial applications.  

For example, consider the term $t =\ $\elpiIn{map likes!}. Its inferred
...
% 
$$\assume : sig \to tm \to \ctx \to \ctx$$  

The \assume\ procedure aims to assume a term $t$ to have an expected determinacy
\func. It takes and updates a context \ctx\ where
variable mappings may change according to \func. The rule
...
its current signature in \ctx\ and \func. The use of \minT\ is justified by the
fact that \assume\ represents runtime unification. For unification to succeed,
...
Finally, \ref{rule:assume-app} performs a fold\todo{quasi fold} over the input arguments of the
predicate being called, under the assumption that the determinacy of the
predicate is smaller than the received one.  
% 
$$\checkk : \A \to (\ctx \times sig) \to (\ctx \times sig)$$  

\checkTermFig  

The \checkk\ procedure (in \cref{fig:static-check}) combines both \infer\ and \assume. Its purpose is to
...
The final step of the static checking procedure concerns the validation of
clauses. In \ref{rule:check-clause}, we begin by assuming that the input terms
have the determinacy specified in the signature. Then, we analyze the premises
