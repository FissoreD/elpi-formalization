...
% \section{Determinacy checking for Higher Order predicates} % Prolog with higher-order variables in prop}
% \label{sec:vars}
...
% %account in the \ref{rule:unif} and \ref{rule:match} rules, 
% used in term unifications, however, a query like
% \elpiIn{p = X, X 1 3} was not accepted by the run procedure. The only
% application we authorized was caputered by calls to terms with rigid heads.
% % Here we extend terms application and replace it as follows
...
% % \newcommand{\sbeta}{\ensuremath{\mathcal{S}_\beta}}
...
% %   %   \vspace{-2em}
% %   % \begin{align*}
% %   %   \sbeta\ \subst\ (\lambda x.b)\ (a::\vec{as}) &= \sbeta\ \subst\ (b [x\bs a])\ \vec{as}\\
% %   %   \sbeta\ \subst\ (\pred\ \vec{t})\ \vec{a} &= \pred\ \vec{t}\ \vec{a}
% %   % \end{align*}
...
% applied to $\vec{t}$ is $\beta$-reduced (thanks to the hereditary substitution
% principle) to a canonical value in the form of a predicate name applied
% a new vector of terms $\vec{u}$.

% This change significantly impacts our static determinacy checker: variables can
% now be passed as arguments and may have a propositional type, meaning
% they can be called within clause bodies. Therefore, additional properties can be
% enforced by the checker. For example, users may want to ensure that a
% predicate's output is verified to be a deterministic predicate.
...
% provide more expressive type signatures for constants. The non-terminal $pred$
% from \cref{eq:pred} changes as follows:
% as follows:
% %
% \begin{align}
%   ty & ::= data \mid [\detI \mid \relI]\ ty^\ast \funsep ty^\ast \label{eq:pred1}
% \end{align}

% The arguments of a predicate, now, are no more objects living in the datatype,
% but they can also have propositional type. In the following a predicate with $0$
% argument will be note as \detI\ (resp. \relI) instead of $\detI\ []\ []$ (resp.
% $\relI\ []\ []$).
...
% type signatures, the \elpi's checker ensures that a predicate behaves as a
% function and returns output with the declared determinacy in the signature
% if they are called with inputs having the expected determinacy.

% Thanks to this certification, for example, if \elpiIn{f} be a binary relation, then the query ``\elpiIn{make-det (f 1 X) F, do F}'' is
...
% \subsection{Comparing predicate signatures}

It is one of our objective that the determinacy checker should accept
predicates that are \textit{wrongly} called so that we can, incrementally,
make the predicate signatures more precise and also avoid code duplication\footnote{require multiple copies of the same code just because they differ in signature}.


% FINQUI

% For
% example, \elpiIn{do (f 1 X)} should be accepted as a call, however, since the\todo{cambiare f con queen}
% argument of \elpiIn{do} has not the expected determinacy, we loose the guarantee
% that the call behaves deterministically. Similarly, in the query \elpiIn{idF (f 1 X)
%   R}, \elpiIn{idF} is \textit{wrongly} called and therefore we have no guarantee that \elpiIn{R} is
...
% is a function: \elpiIn{A} could be a relation and calling it in the body of a
% predicate expected to behave deterministically, may create choice points.

% To address all of these issues we implement a static checker which labels variables
% with their determinacy type (dtype). Labels are simpler instantces of \cref{eq:ty}:
% all terms living in the datatype will be labelled with \expI. The checker
% rejects the clauses of a predicate if the labels that have been updated in a
% given context \ctx\ do not respect the signature of the predicate.
...
% be always passed in place of a relation. 

We define below the partial ordering test between two predicate signature $f_1$ and $f_2$.
\begin{equation}
  f_1 \mySub f_2 =
  \begin{cases}
...
The \mySub extend the intuitive inclusion of mathematical functions
into relations to the recursive nature of predicate signatures, and rejects the
converse inclusion. The definition assumes type checking as in~\cite{1992nadathur}:
we only compare signatures that agree in their arity and number of inputs (hence outputs)
and we ignore the case in which datatypes are compared to predicate signatures.
...
\begin{definition}[Meaning of a signature (\sem{\cdot})]
$$
\begin{array}{rl}
\sem{\dtype{\func_1}{\func_2}{\func_3}}\ \pred & =
  \forall t\ u, \sem{\func_2}\ t \Rightarrow \sem{\func_1}\ (\pred\ t\ u) \land \sem{\func_3}\ u\\
\sem{\detI}\ \pred & = \isdet\ \pred \\
\sem{\expI}\ \pred & = \top 
\end{array}
$$
\end{definition}
If the preconditions of a signature are satisfied then the call is
functional and the post conditions on the out hold.
The order relation \mySub is such that $f_1 \mySub f_2 = \sem{f_1} \Rightarrow \sem{f_2}$,
hence the contravariance, the post of 11 are stronger than the ones of f2, while
the pre must be in the converse relation.
\todo{is odet is defined later}

% The \mySub\ relation is intentionally incomplete, as we assume that the program
% has undergone type-checking before any determinacy verification. This guarantees
% that no comparison between, for instance, expressions and predicates can
% occur—such a comparison would result in a fatal type error. Moreover, when
% comparing predicates $p_1$ and $p_2$, we assume that their input lists have the
% same length, as do their output lists: predicates with differing input or output
% list lengths are considered incomparable.
...
% outputs of $t_1$ should be \mySub\ than the outputs $t_2$ and
...
% outputs $t_1$.
% The idea is similar to a subtyping relation where inputs should be
% contravariant and outputs covariant.

% For example, let's consider the signatures of \elpiIn{make-det}
...
\section{Determinacy checking algorithm}

The algorithm is a dataflow analsysis algorith a bit like mode checking.
It stores in \ctx the predicate signature of each variable: it uses it to
check if it satisfies the precondition of a call and possibly refines if
the signature if the post condition of a call holds.

These updates use the following two operators

%
\begin{equation}
  \minT\ f_1 f_2 =
  \begin{cases}
    \dtype{\detI}{(\vecL{\maxT}\ a\ c)}{(\vecL{\minT}\ b\ d)} & \text{if } f_k = \dtype{\relI}{a}{b} \land f_{2-k} = \dtype{\detI}{c}{d} \\
    \dtype{D}{(\vecL{\maxT}\ a\ c)}{(\vecL{\minT}\ b\ d)}     & \text{if } f_1 = \dtype{D}{a}{b} \land f_2 = \dtype{D}{c}{d}             \\
    % \relI\ (\vecL{\maxT}\ a\ c)\ (\vecL{\minT}\ b\ d) & \text{if } f_1 = \relI\ a\ b \land f_2 = \relI\ c\ d \\
...
\begin{equation}
  \maxT\ f_1 f_2 =
  \begin{cases}
    \dtype{\detI}{(\vecL{\minT}\ a\ c)}{(\vecL{\maxT}\ b\ d)} & \text{if } f_k = \dtype{\relI}{a}{b} \land f_{2-k} = \dtype{\detI}{c}{d} \\
    \dtype{D}{(\vecL{\minT}\ a\ c)}{(\vecL{\maxT}\ b\ d)}     & \text{if } f_1 = \dtype{D}{a}{b} \land f_2 = \dtype{D}{c}{d}             \\
    % \relI\ (\vecL{\minT}\ a\ c)\ (\vecL{\maxT}\ b\ d) & \text{if } f_1 = \relI\ a\ b \land f_2 = \relI\ c\ d \\
    \expI                                                     & \text{if } f_1 = \expI \land f_2 = \expI
  \end{cases}
  \label{eq:max}
\end{equation}

\newcommand{\maximize}{\texttt{maximize}\xspace}
\newcommand{\minimize}{\texttt{minimize}\xspace}

% Below we define the functions \maximize and \minimize.

We assume all variables appear in \ctx with a signature upfront, generated from
the type checker as follows: TODO explain \maximize and \minimize.
...
\begin{equation}
  \maximize\ f =
  \begin{cases}
    \dtype{\relI}{(\vecL{\minimize}\ a)}{(\vecL{\maximize}\ b)} & \text{if } f = \dtype{\_}{a}{b} \\
    \expI                                                       & \text{if } f = \expI
  \end{cases}
  \label{eq:maximize}
\end{equation}
...
\begin{figure}
  ...
  % \fbox{\inferAppFig}
  \fbox{\assumeFig}
  \caption{Handling of pre and post conditions}
  \label{fig:full-det-check-terms}
\end{figure}

$$\infer : \ctx \to \A \to sig \times \bool$$
$$\dapp : \ctx \to \vecL{sig} \to \vecL{sig} \to \vecL{tm} \to \bool \to sig \times sig \times \bool$$

The \infer\ procedure is in charge of inferring the signature of an input
argument. Since predicate calls are part of the term syntax, these ``calls''
can as well be miscalled. In that case the procedure returns, as the second
value, $\bot$.

EXAMPLE: lambda and partial application
...
% takes a context \ctx and a term $t$ and returns the dtype of
% $t$ and a boolean. The boolean tells if the term is well dtyped, the only interesting
...
% Based on the \mySub, \minT\ and \maxT\ relations we present a system of rules
% allowing to \infer\ (\cref{fig:det-inference}) or \assume\ (\cref{fig:det-assume})
% the signature of a predicate and finally to \checkk\ (\cref{fig:static-check})
% if a rule matches a declared signature.
...
\detI \mySub d$. 
% Rule \ref{rule:assume-piimpl} goes under the implication with a
% recursive call charging in the context the local binder $x$. This same binder is
% removed from the result in the output. 
Finally the rule for term application
...
\begin{figure}
  ...
  \label{fig:full-det-check-rules}
\end{figure}
...
\subsection{Digression: inference v.s. checking}
