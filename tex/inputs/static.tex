...
make the predicate signatures more precise.\todo{??}
...
\newcommand{\semi}[1]{\ensuremath{[\![#1]\!]_i}}
\newcommand{\semt}[1]{\ensuremath{[\![#1]\!]_t}}
The intuition of a 
...
does not matter).\todo{is-odet prende un programma e un termine}
...
% $$
% \end{definition}
\newcommand{\predt}{\ensuremath{\pred{}\ \vec{t}}}
% \begin{definition}[Meaning of a signature $(\sem{\func}\predt)$ in a given program \prog]\label{def:sem}
  \begin{definition}[Meaning of a signature $(\sem{\func}\ \pred)$ given a context \ctx and a program \prog]\label{def:sem}
  % $$
  % \begin{array}{rll}
  %   \sem{\predt} & = \bigwedge \vecL{\semi{t_i}} \Rightarrow \semt{\func, \predt} \land \vecL{\semi{t_o}} & \mathrm{with\ } \ctx\ \pred{} = \dtype{\func}{\_}{\_}\\
  %   \semi{\predt} & = \bigwedge \vecL{\semi{\func_i, t_i}} & \mathrm{with\ } \ctx\ \pred{} = \dtype{\_}{\func_i}{\_} \\
  %   \semt{\detI, \predt} & = \isdet \prog (\predt) & \\
  %   \semt{\relI, \predt} & = \top &
  % \end{array}
  % $$
  $$
  \begin{array}{rl}
    \sem{\dtype{\func}{\func_i}{\func_o}}\ \pred & = \forall t_i\ t_o, \bigwedge \vecL{\semi{\func_i}\ t_i} \Rightarrow \semt{\func}\ (\predt) \land \bigwedge \vecL{\semi{\func_o}\ t_o}  \\
    \semi{\func}\ (\predt) & = \bigwedge \vecL{\semi{\func_i}\ t_i} \qquad \mathrm{with\ } \ctx\ \pred{} = \dtype{\_}{\func_i}{\_} \\
    \semt{\detI}\ (\predt) & = \isdet\ \prog\ (\predt)  \\
    \semt{\relI}\ (\predt) & = \top 
  \end{array}
  $$
...
\assume\ (\cref{fig:det-assume}) the signature of a term (or atom) and finally to
...
a term (or atom) and a boolean value indicating whether
it contains miscalled predicates.
%  Since predicate
% calls are part of the term syntax, they can also be miscalled.  

The rules for \cut\ (\ref{rule:infer-cut}) and functors (\ref{rule:inferK}) are
...
call to \infer. $\lambda$-abstractions are handled by \ref{rule:infer-lam} and
are transformed to clauses that must be validated by the \checkc\ routine (in \cref{fig:static-check}). In this\todo{split \ref{rule:infer-lam}}
case, we ensure that both the output and the term beneath the spine of
$\lambda$s conform to the expected type; otherwise, we return the boolean $\bot$.
...
updating a context \ctx\ according to \func. The rule
...
sense of~\cref{def:sem}, the determinacy of an atom by checking the
preconditions and deducing new knowledge from the postconditions. The \checkc\
procedure verifies the validity of a clause.
