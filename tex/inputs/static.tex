...
\begin{equation*}
  ...
  \begin{cases}
   ...
   \ \top                                                          & \text{else if } s_1 = \expI\ \land\ s_2 = \expI
  \end{cases}
  % \label{eq:sub-def}
\end{equation*}

Given two signatures \func[_1] and \func[_2], if $\func[_1] \mySub \func[_2]$,
then \func[_1] is stronger than or equal to \func[_2], and \func[_2] is weaker
...
$p$, let $\func[_i']$ (resp. \func[_o]) represent the signatures of its input
(resp. output) arguments. If the preconditions are satisfied (i.e.,
$\vecL{\func[_i'] \mySub \func[_i]} = \top$), then the postconditions can be
assumed. That is, we assume that the output arguments are pointwise stronger
or equal to the signature \func[_o], and the entire application has determinacy
\func.
...
if preconditions are met, it
updates the signature of the variable to match the postconditions 
...
\begin{equation}
  ...
  \begin{cases}
    ...
    %       \ \ \ \func_1 \neq \func_2
    % \end{array} \\
    \text{else if }s_1 = \dtype{\_}{a}{b}\ \land\ s_2 = \dtype{\_}{c}{d}\\
    \ \expI                                                     & \text{else if } s_1 = \expI\ \land\ s_2 = \expI
  \end{cases}
  \nonumber
  \label{eq:min}
\end{equation}
...
of variables (see rule \ref{rule:assume-hd}) to a given signature by updating the context via the $\min$ operator.
