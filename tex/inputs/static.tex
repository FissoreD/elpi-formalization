...
\begin{definition}[Mutual exclusion \mutExclAll]
  ...
  holds:\\
  a) $\cut \in \vecL{bs}$\\
  b) $\forall r' = (\clauseCmd{p}{\vecL{t'}}{\_}) \in \prog$
     there exists a $d \leq k$ such that $\lnot \exists\ \subst\ s.t.\ \unifyCmd{t_d}{t'_d}{\EmptySubst}{\subst}$ % \land \subst \neq \bot$
  % c) $\forall r' = (\clauseCmd{p}{\vecL{t'}}{\_}) \in \prog$ $\bigwedge \vecL{t_i = x} \bigwedge \vecL{t'_i \not = X}$.
  \label{def:mut-excl}
\end{definition}

Case a) is justified by the fact that \ref{rule:piimpl} gives precedence to the
newly asserted clause, hence the hard cut in its body discards all other clayses.
Case b) accounts for mutually exclusive heads.
We recall that \ref{rule:piimpl} generates a fresh symbol for the
\elpiIn{pi} quantifid variable.
