...
\begin{equation*}
  ...
  \begin{cases}
   ...
   \ (\forall i, c_i \mySub a_i)\ \land\ (\forall j, b_j \mySub d_j) &
    \!\!\!
    \begin{array}{l} \text{if } s_1 = \dtype{\func}{a}{b}\ \land\ s_2 = \dtype{\func}{c}{d} \ \lor    \\
     \ \ \  s_1 = \dtype{\detI}{a}{b}\ \land\ s_2 = \dtype{\relI}{c}{d} 
    \end{array}    \\
...
the variable meets the precondition of a predicate call. Additionally, it can
update the variable's signature to align with the postcondition of the predicate
call.

We suppose the context \ctx is pre-loaded with weak signatures
for each variable: It is up to the determinacy analsysis to \emph{assume}
...
\begin{equation}
  ...
  \begin{cases}
    ...
    \ \dtype{d}{(\map\ u_{\overline{d}}\ \vecL{ac})}{(\map\ u_d\ \vecL{bd})} & \text{if } s_k = \dtype{\relI}{\vec a}{\vec b}\ \land\ s_{3-k} = \dtype{\detI}{\vec c}{\vec d} \\
    %\ \dtype{D}{(\vecL{g\ a\ c})}{(\vecL{f\ b\ d})}     & \text{if } s_1 = \dtype{D}{a}{b}\ \land\ s_2 = \dtype{D}{c}{d}             \\
    \ \dtype{\func}{(\map\ u_{\overline{d}}\ \vecL{ac})}{(\map\ u_d\ \vecL{bd})}     & \text{if } s_1 = \dtype{\func}{\vec a}{\vec b}\ \land\ s_2 = \dtype{\func}{\vec c}{\vec d}             \\
...
\infer\ (\cref{fig:det-inference}) and \checkk\ (\cref{fig:static-check}).

\assumeFig

\paragraph{The \assume\ procedure} strenghtens the signature
of variables to a given signature by updating the context via the $\min$ operator.
Since the syntax allows for arbitrary terms in predicate postion,
\assume traverses terms recursively until it finds the variables \ref{rule:assume-hd}.
When the term represents data (e.g. it is of type \expI) the context is left
untouched (see rules \ref{rule:assume-cut}, \ref{rule:assume-expr} and \ref{rule:assume-exprl}).
...
%Rule \ref{rule:infer-piimpl} introduces a new mapping in the
...
a fresh predicate name $p$ so to reuse the code \checkc\ to check regular clauses (see \cref{fig:static-check}).
...
\paragraph{The \checkk\ procedure} is in charge of veryfying the determinacy of
an atom.
A cut is clearly determinate (rule \ref{rule:check-cut}).
A predicate call is determinate if the the premises of its signature
...
output arguments is strenghtened. When a predicate is miscalled
...
output arguments in the head. It concludes by veryfying that the inferred
...
\begin{definition}[Mutual exclusion \mutExclAll]
  Given a program \prog and a clause $r = \clauseCmd{p}{\vecL{t}}{\vecL{bs}}$
  where $\ctx\ \pred = \dtype{\detI}{i_1 \ldots i_k}{\_}$, we say that
  $\mutExclAll\ \prog\ r$ holds iff one of the following conditions
  holds:\\
  a) $r$ is inserted on top of \prog and $\cut \in \vecL{bs}$, or\\
  b) $\forall r' = (\clauseCmd{p}{\vecL{u}}{\_}) \in \prog$
     there exists a $d \leq k$ such that $\forall \subst, \unifyCmd{t_d}{u_d}{\subst}{\bot}$ % \land \subst \neq \bot$
  % c) $\forall r' = (\clauseCmd{p}{\vecL{t'}}{\_}) \in \prog$ $\bigwedge \vecL{t_i = x} \bigwedge \vecL{t'_i \not = X}$.
  \label{def:mut-excl}
\end{definition}

Case a) is justified by the hard-cut semantics combined with the the highest
priority of $r$ in \prog (see rule \ref{rule:cut}).
Case b) accounts for mutually exclusive heads.
We recall that \ref{rule:piimpl} generates a fresh symbol for the
\elpiIn{pi} quantifid variable, hence different from any symbol that can
occur in the goal or in the other clauses.
Moreover we
poit out that \ref{rule:call} \emph{matches} the input arguments
of the goal against the head of the clauses.
In turn this dispenses the need for groudness of input
arguments, a prerequisite of most determinacy analsys algorithms
(see for example~\cite{1989Warren,1996Somogy}). For example imagine all
input arguments in the goal are variables: condition b) grants that there
is at most one clause with variables in all input positions, and that
would be the only one that can match (unify with the goal without instantiating it).
...
Consider
the following program optimizing function composition (as in \cite{wadler1989}).

\begin{elpicodeln}[29]
  func compose (func A -> B), (func B -> C), A -> C.
  compose F G X Y :- F X R, G R Y.
  typeabbrev (flist A B) (func list A -> list B).     % type abbreviation
...
For brevity, we shorten the signature \elpiIn{func list X -> list Y} to $\func$
and we say $t \infunc$ when a term $t$ has signature stronger or equal to
$\func$.
% $\dtype{\detI}{\expI}{\expI}$.

Considering the work that \checkc performs line 32:
it \assume{}s (resp. \infer{}s) that
\textit{inputs} (resp. \textit{outputs}) have the expected signature and that
the body is functional.
More precisely, it \assume{}s that \elpiIn{map A}\infunc and \elpiIn{map B}\infunc. 
In order to be able to \infer that
\elpiIn{map C} \infunc, we need 
\elpiIn{C} \infunc.
The checking phase on the premise allows to \infer that \elpiIn{C} \infunc 
if \elpiIn{id} is well-called.
In turn, this requires \elpiIn{compose A B} $\in$ \elpiIn{func A -> B}.
   \elpiIn{A} and \elpiIn{B} have the determinacy
\func. This is the reason why \assume recurses on the 
\textit{inputs} of \elpiIn{map A} and \elpiIn{map B} in order to
\infer that \elpiIn{A}\infunc and \elpiIn{B}\infunc.
...
As a result of the deep analysis performed by \assume on the outputs of
well-called predicates. The following clause

\begin{elpicode}
  whd_copy_list TL RL :- fuse (map whd) (map copy) F, F TL RL. 
  whd_copy T R :- fuse (map whd) (map copy) (map F), F T R. 
\end{elpicode}

\noindent can be considered functional since the variable \elpiIn{F} 
occurs in \textit{input} position of the output of \elpiIn{fuse} and therefore
is assumed to be a binary function for the final call \elpiIn{F A B}.
