...
% \end{elpicode}
As an example, consider a predicate compose with the following signature
\begin{elpicode}
  func compose (func A -> B), (func B -> C) -> (func A -> C)
\end{elpicode}

\noindent which takes two binary functions and returns a new function which is
their composition. We may wish to assume that \elpiIn{compose likes X Y} is a
function. Since the first argument of compose has signature
$\dtype{\relI}{\expI}{\expI}\not\mySub \dtype{\detI}{\expI}{\expI}$, then not
assumption is done on the variable \elpiIn{X}.
...
Case a) is justified by the hard-cut semantics (see rule \ref{rule:cut}).
...
that the inferred determinacy of their \textit{inputs} is at most \mySub, with
respect to their expected signature.  
...
signatures have \textit{inputs} that validate their expected signature. Consider
the following rule:  

\begin{elpicode}
  compose (map A) (map B) (map C) :- compose A B C.
\end{elpicode}

\checkc assumes that the clause is called with the correct arguments. Therefore,
we can \assume that its \textit{inputs} have the expected signature, which is
\dtype{\detI}{\expI}{\expI}. Consequently, \elpiIn{map A} (resp. \elpiIn{map B})
...
This validation of clauses allows us to make assumptions about the
\textit{outputs} of atoms if their \textit{inputs} respect the predicate’s
signature. For example, in a query like \elpiIn{compose (map copy) (map copy)
(map L)}, the variable \elpiIn{L} can be safely considered a binary function.
Note that the assumption is made on the \textit{input} of the \textit{output}
argument.  
