...
are transformed to clauses that must be valided by the \checkc routine. In this
case, we ensure that both the output and the term beneath the spine of
$\lambda$s conform to the expected type; otherwise, the checker raises an error.
...
signature. For example: \elpiIn{X = (x\y\ commit x y), X like F, F paul Z}.
