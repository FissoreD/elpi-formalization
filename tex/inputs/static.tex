...
\begin{definition}[Mutual exclusion \mutExclAll]
  ...
  $\mutExclAll\ \prog\ r\ v$ holds iff one of the following conditions
  holds:\\
  a) $h$ has highest priority in \prog and $\cut \in \vecL{bs}$, or\\
  b) exists a $d \leq k$ such that $t_d = v$ and
       $\forall h' = (\clauseCmd{p}{\vecL{u}}{\_}) \in \prog$
       $u_d = f \vec{w} \lor u_d = q \vec{w}$ ($u_d$ not a variable), or\\
...
Case c) addresses mutually exclusive heads for teplevle clauses, i.e. clauses
...
since they can be asserted multiple times: in case b) that the hypothetical
