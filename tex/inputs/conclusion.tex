...
\begin{table}[b]
  ...
  \begin{tabular}{c|c|c|c|c|c}
    Paper         & Mode check & Hard Cut & HO programs        & Wrong calls           & Open program    \\
    \hline
    Warren et al. & required    & \cmark   & \xmark          & \xmark          & \xmark \\
    Mercury       & required     & \xmark   & \nicefrac{1}{2}$\ddagger$ & \xmark$\dagger$ & \xmark \\
    % Mixtus        & \cmark     & \cmark   & \xmark          & \nicefrac{1}{2} & \xmark  & \xmark \\
    RedAlert      & required     & \cmark   & \xmark          & \xmark          & \xmark \\
    Elpi          & no need     & \cmark   & \cmark          & \cmark          & \cmark
  \end{tabular}
  \end{center}
  $\dagger$ multiple signatures for the sema predicate, one must perfectly match\\
  $\ddagger$ TODO explain what is missing: partial application\\
...
than logic ones, hence we strieve for simplicty,
