...
in our language: for example \textit{fail} is represented as a predicate with no
...
We would also point out that our semantics is the concrete semantics of the Elpi
...
Elpi targets less technically includes users hence we strieve for simplicty,
defining fewer determinacy classes and immediately classifying wrong calls as
non-deterministic. For instance, \elpiIn{foldr} has only the signature
\elpiIn{func foldr (func L A -> A) (list L) A -> A}. This design choice aligns
with our goal of providing static guarantees on programs rather than producing
efficient, specialized, machine code depending on the pred usage.
