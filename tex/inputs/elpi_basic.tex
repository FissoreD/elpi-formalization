...
%
$$
\mathcal{F}(\prog, p, i, o, s, a) := [(s, (\Cons{(\prog, i = i', a)}\Cons{(\prog, o = o', a)}{[(\prog, g, a) \mid g \in bs]})) \mid p\ i'\ o'\ \text{\elpiIn{:-} } bs \in \prog\ p]
$$
...
Mode checking is an essentially ingredient from which determinacy checker can be
...
In order to satisfy this (necessary by not sufficient) condition, we need that
all the clauses of a determinate predicate $p$ are mutually exclusive, i.e. for
any pair of clauses $p\ \vec{x} \text{ \elpiIn{:-} } b1$ and $p\ \vec{y} \text{
\elpiIn{:-} } b2$ there exists an input position $i$ such that $\forall \sigma,
\sigma\ t_1 \neq \sigma\ t_2$.  Equivalently, no two clauses can exist without
at least one overlapping input term.

Thank to the (hard-)cut operator, we can relax a bit this strong condition, by
saying that two clauses that are non-mutually exclusive for a determinate
predicate may exists, only if the chronological antecedent has a cut in it. This
ensures that if we reach the cut operator in the first clause, the second clause
is not considered as a choice point. Conversely, if one of the premises before
the cut fails, then the second clause will be tried. In both situations, the two
clauses cannot be applied simultaneously to the same predicate call.

In \cite{1996henderson}\todo{mercury non ha cut}, mode checking combined with
mutual exclusive clause test, ensures that at most one clause can be applied to
any predicate call. This is because, thanks to mode checking, we know that
output arguments become ground terms after a call. Since output terms are
ground, they can be used as new input in future queries guaranteeing that any
call to a determinate predicate gives at most one clause unifying with it.\\
It is quite simple to have non-determinism without mode checking: a non gruond
output (in a classic prolog engine without dynamic mode checking) can used in an
input position allowing two potentially mutually exclusive clauses to be applied
on the same query, resulting in multiple solutions for the same query.

\todo{AAA}SPIEGARE COME FUNZIONA IL NOSTRO EXCLUSIVE-CLAUSE modulo CUT CHECKING
considerando VARIABILI, TERMINI RIGIDI e UVAR keyword.


The second, but no less important, condition for a predicate to be determinate
is that for each clause, the premises after the last cut must involve calls to
functional predicates, otherwise multiple solutions may be produced.

\todo{BBB}SPIEGARE COME FUNZIONA IL NOSTRO: all premises after the last cut are
call to determinate predicates.

From now on determinacy checking ensures that clauses have been tested for
mutual-exclusion modulo cut, and that all premises after the last cut are
call to determinate predicates.
...
%
\begin{lemma}
  For each determinate predicate $p$, after mutual-clause modulo cut checking,
...
\begin{proof}
  ...
  head of $g$ then adding leading cut in its body does not affect the behavior
  of the program. This completes the proof.
\end{proof}

\Cref{th:all-cut} helps us to give a common structure to all the clause $c$ of
a determinate predicate: we are free to assume that $c$ always has at least one
cut.

\begin{lemma}
  All clauses have cut $\to$ al massimo una regola si applica
  \label{th:only-one-clause}
\end{lemma}

In our scenario, we show that static mode checking is not needed if the dynamic
input-output modes of \elpi are used at runtime. 
%
\begin{theorem}
