...
The procedure \runCmd{\g}{\alt}{\subst}{\alt[']}{\subst[']} relates a list of
goals \g, a list of alternatives \alt and an initial substitution \subst with a
new substitution \subst['] and a new list of alternatives \alt[']. The list of goals
should be understood as a list of goal put in cunjunction, whereas the
alternatives represent a disjunction of goals. In the derivation system, we use
...
%
$$
\mathcal{F}(\prog, p, i, o, \subst, \alt) := 
  [(\subst, (
      \Cons{(\prog, i =_i i', \alt)}
        \Cons{(\prog, o =_o o', \alt)}
          {[(\prog, g, \alt) \mid g \in bs]})) \mid \clauseCmd{p}{i'\ o'}{bs} \in \prog\ p]
$$

It takes a list of clauses, two terms, i.e. $i$ and $o$ representing the two
arguments of the predicate call a substitution $\subst$ and a list of alternatives \alt. For each clause
...
The \textit{fail rule} (\ruleFail) consumes the list of alternatives if the
first goal fails. A failure occurs if the goal is a call to a predicate
with no clauses in the associated program or if a unification in the head
cannot be performed starting from the given substitution.
% In fact, this
...
either $i$ or $o$, it calls respectively the \unify or the \match (see \cref{sec:elpi-modes}) algorithm
between the two terms $t_1$ and $t_2$ from the substitution \subst and
returns the updated substitution \subst[']. Finally the \run\ procedure is called the
list of remaining goals \g and the new substitution \subst['].

The \textit{cut rule} (\ruleBang) refers to goals where the first conjunct is is
the \cut atom. This is where the cut-alternatives \alt become interesting.
The rule tells that the list of goal \g can be launched using \alt as the
new list of alternatives.\todo{explain better}
...
of each body of $\prog p$ to the current alternatives \alt. If a clause $b_i$ in
...
mode, we give them a different interpretation. 
...
Our interpretation of modes follows the concept of read and write as
explained in \cite{1991ait-wam}. Essentially, in a call, a term in input position is an
object received in read mode. Therefore, if the entry
term is a variable and the corresponding term in the head of the clause is a
rigid term, unification fails. This is because the \unify procedure of the
interpreter cannot write inside it.

On the other hand, it is always possible to unify a non-flexible input term with
a variable if that variable appears in the head of the clause. Finally, no
constraints are imposed on output terms, which can be read and written at any
time.
...
\begin{definition}[The \match procedure]
  ...
  \coqIn{t}, and let \coqIn{is_var(t)} be a function that determines whether the
  term \coqIn{t} is flexible. Then:
  \begin{coqcode}
...
In \cite{1996henderson}, mode checking (with groundness checking) ensures that
at most one clause can be applied to any given predicate. Note the hypotheses
referred to as \coqIn{HG} in \cref{def:mut-excl} and \ref{detpred}.
...
\begin{definition}[Mutual-exclusion on heads in \elpi]
  \begin{coqcode}
    Definition ~\customlabel{emutexcl}{\texttt{emutual\_exclusive}}\prog \pred\!\!~:
      ~$\forall$~ i, ~$!\exists$~ i' o bo, (~\clauseCmd{\pred}{\texttt{i' o}}{\texttt{bo}}~) \in ~\prog~p -> exists ~\subst\!\!~, ~\matchCmd{i}{i'}{\EmptySubst}{\subst}~ 
  \end{coqcode}   
  \label{def:emut-excl} 
\end{definition}
