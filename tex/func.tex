...
\title{Functionality analysis for higher order logic programs}
...
\keywords{Mode Analysis, Determinacy Analysis, Higher Order, Logic Programming}
...
We want to say more about the behavior

\begin{elpicode}
func map (func A -> B), list A -> list B.
\end{elpicode}

This is HOAS, we want out analysis to accept this code that adds a
dynamic rule
...
\begin{elpicode}
func comp (func A -> B), (func B -> C) -> (fun A -> C).
...
Finally, we want to take advantage of cut
\begin{elpicode}
func once (pred) -> (func).
once P :- P, !.
...
func make-even int -> int.
map-fist X :- even X, !, Y is X + 2.
map-fist X :- odd X, !, Y is X + 1.
\end{elpicode}
...
spec map True ... =
∀c0, (∀c1, ∀c2, func (c0 c1 c2)) → ∀c1, ∀c2, func (map c0 c1 c2) 
\end{verbatim}
...
% main language. Place a figure in the margin.
